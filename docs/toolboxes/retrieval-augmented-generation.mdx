...
- **Use agent tools** when you're building an agent that needs retrieval as a tool. For example, see [File search tool for agents](/toolboxes/file-search). To build a full RAG app with complete control instead, use the Foundry SDK.
...
- [File search tool for agents](/toolboxes/file-search)
