...
Start with the [web search tool](/toolboxes/web-search). Learn more about the differences between web search and Grounding with Bing Search (or Grounding with Bing Custom Search) in the [web grounding overview](/toolboxes/web-overview).
...
- Azure RBAC roles:
  ...
  - **Foundry User** on the Foundry project to create and run agents. For more information, see [Role-based access control for Microsoft Foundry](../../../concepts/rbac-foundry).
...
| --- | --- | --- |
| General Web Search | Your agent needs current information from the public web without a separate Bing resource or project connection. | [Add web search directly to a prompt agent](/toolboxes/web-search#add-web-search-directly-to-an-agent). |
| Domain-restricted Bing Custom Search | You want the Web Search API surface, but results must come from domains configured in your Bing Custom Search instance. | [Configure domain-restricted Web Search](/toolboxes/web-search#domain-restricted-search-with-bing-custom-search). |
| Deep research | Your `o3-deep-research` agent needs multi-step research and synthesis. | [Use direct Web Search for deep research](/toolboxes/web-search#deep-research-with-web-search). |
...
<Frame>
  <img src="/images/website-citations.png" alt="Screenshot of Bing search results showing website and search query citations." />
</Frame>
...
- [Web search tool](/toolboxes/web-search) - Use web search without configuring Bing tool parameters
...
- [Connect OpenAPI tools to agents](/toolboxes/openapi) - Integrate custom APIs with your agents
- [What is Toolbox in Foundry?](/get-started/toolbox-overview) - Explore all available agent tools in Foundry Agent Service
