...
You can customize Code Interpreter behavior at runtime, such as specifying which files to include or adjusting tool parameters per request, by using [structured inputs](/agents/structured-inputs).
...
This sample creates the code-interpreter toolbox, then uses [`FoundryChatClient`](/agents/responses-api) from the Microsoft Agent Framework and connects to the toolbox MCP endpoint using [`FoundryToolbox`](https://github.com/microsoft-foundry/foundry-samples/tree/main/samples/python/hosted-agents/agent-framework/responses/04-foundry-toolbox). Set the `FOUNDRY_PROJECT_ENDPOINT` and `FOUNDRY_MODEL` environment variables, and sign in with `az login`.
...
For most agents, add the code interpreter tool through a [toolbox](/get-started/toolbox-overview) and attach the toolbox to your agent as an MCP tool. The Java SDK doesn't yet expose a toolbox creation API, so create the toolbox by using one of the currently supported methods (Python, REST API, C#, TypeScript, or the [Foundry portal](/toolboxes/toolbox)). Once the toolbox is created, reference its MCP endpoint from your Java agent as an `McpTool`. The following example attaches the code-interpreter toolbox MCP endpoint to the agent.
...
Add Code Interpreter by creating a toolbox, and then attach the toolbox to your agent as an MCP tool. For more information, see [What is a toolbox?](/get-started/toolbox-overview)
...
For the current list of supported regions and models for Code Interpreter, see [Best practices for using tools in Microsoft Foundry Agent Service](/agents/tool-best-practice).
...
| Python package not available. | Code Interpreter has a fixed set of packages. | Code Interpreter includes common data science packages. For custom packages, use [Custom code interpreter](/toolboxes/custom-code-interpreter). |
...
For examples of conversation and file cleanup patterns, see [Web search tool](/toolboxes/web-search) and [File search tool for agents](/toolboxes/file-search).
...
If you need more control over the sandbox runtime or you need a different isolation model, see [Custom code interpreter tool for agents](/toolboxes/custom-code-interpreter).

## Related content

- [Best practices for using tools in Microsoft Foundry Agent Service](/agents/tool-best-practice)
- [Custom code interpreter tool for agents (preview)](/toolboxes/custom-code-interpreter)
