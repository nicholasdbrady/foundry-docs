...
For the full toolbox creation workflow, see [Curate intent-based toolbox in Foundry](/toolboxes/toolbox#create-a-toolbox-version).
...
The recommended way to add Work IQ is through a toolbox, then attach the toolbox to your agent as an MCP tool. See [What is a toolbox?](/get-started/toolbox-overview)
...
Next, [connect the toolbox to a hosted agent](/agents/use-toolbox-hosted-agent). For a complete C# implementation that registers Work IQ toolbox tools with Agent Framework, see the [Teams activity sample](https://github.com/microsoft-foundry/foundry-samples/tree/main/samples/csharp/hosted-agents/agent-framework/teams-activity).
...
After you create `work-iq-toolbox`, follow [Use a toolbox with a hosted agent](/agents/use-toolbox-hosted-agent) to connect it to Agent Framework, run it locally, and deploy it to Foundry Agent Service. The hosted integration preserves the per-request context that Work IQ needs to act on behalf of each signed-in user.
...
<Frame>
  <img src="/images/edit-connection-portal.png" alt="Screenshot of the Foundry portal showing authentication settings for a Work IQ connection." />
</Frame>
...
- [Connect to an A2A agent endpoint from Foundry Agent Service](/toolboxes/agent-to-agent)
