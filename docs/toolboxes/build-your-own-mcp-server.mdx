...
Agent Service connects to publicly accessible MCP server endpoints and [can be configured](/toolboxes/model-context-protocol#public-and-private-mcp-server-endpoints) to use private MCP server endpoints.
...
For Agent Service authentication patterns (for example, key-based authentication, Microsoft Entra identities, and OAuth identity passthrough), see [MCP server authentication](/toolboxes/mcp-authentication).
...
For detailed configuration steps (including project connections and approval workflows), see [Connect to Model Context Protocol servers (preview)](/toolboxes/model-context-protocol).
...
- **MCP server connection fails**: Confirm the server URL is reachable from Agent Service and uses the MCP webhook path (`/runtime/webhooks/mcp`). For public endpoints, verify the URL is publicly accessible. For private endpoints, verify your [Standard Agent Setup with private networking](/toolboxes/model-context-protocol#public-and-private-mcp-server-endpoints) is configured correctly. Check the Function App logs in Azure portal for errors.
