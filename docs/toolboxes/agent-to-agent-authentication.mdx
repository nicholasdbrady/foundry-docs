...
- The **Foundry User** role or higher on your project. This role grants permissions to create project connections and configure agents. For details, see [Role-based access control in the Foundry portal](../../concepts/rbac-foundry).
...
For more information about the agent identity lifecycle, see [Agent identity concepts in Microsoft Foundry](/agents/agent-identity).
...
[**Foundry Agent Consumer** role](../../concepts/rbac-foundry#agent-scope-role-assignments)
...
[Grant access to the A2A endpoint](/agents/enable-agent-to-agent-endpoint#grant-access-to-the-a2a-endpoint).
...
1. **Create a project connection** in the Foundry portal. The connection stores the A2A endpoint URL, authentication method, and credentials.
   ...
   - For A2A-specific configuration, see [Add an A2A agent endpoint to Foundry Agent Service](/toolboxes/agent-to-agent).
...
- [Add an A2A agent endpoint to Foundry Agent Service](/toolboxes/agent-to-agent): Step-by-step guide to configure an A2A tool for your agent.
- [Agent identity concepts in Microsoft Foundry](/agents/agent-identity): Learn how agent identities work and their lifecycle.
