...
To learn more on optimizing tool usage, see [best practices](/agents/tool-best-practice).
...
- **Workflows**: Orchestrate multiple Foundry agents declaratively using sequential, group chat, or human-in-the-loop patterns. See [Build a workflow in Microsoft Foundry](/agents/workflow).

For a complete mapping of classic agent tools to their replacements in the new API, see [Agent tool availability](/agents/migrate#agent-tool-availability).
...
- For a Foundry agent target, use its [A2A base path](/agents/enable-agent-to-agent-endpoint#verify-the-agent-card), enable incoming A2A, and grant the calling identity the **Foundry Agent Consumer** role or higher on the target project. For the role requirement, see [Configure authentication for incoming requests](/agents/enable-agent-to-agent-endpoint#configure-authentication-for-incoming-requests).
- Foundry agent targets support generally available A2A protocol version 1.0 and preview version 0.3. Version 1.0 uses JSON-RPC. Only text modality is supported, and streaming responses aren't supported. For details, see [A2A limitations](/agents/enable-agent-to-agent-endpoint#limitations).
...
For details about supported authentication approaches, see [Agent2Agent (A2A) authentication](/toolboxes/agent-to-agent-authentication).
...
[A2A task and context retention](/agents/enable-agent-to-agent-endpoint#understand-a2a-task-and-context-retention).

For other endpoints, if the endpoint requires authentication to read its agent card, set `send_credentials_for_agent_card` to `true` in the A2A tool definition. Otherwise, Agent Service fetches the agent card anonymously by default. For more information, see [Credentials for the agent card request](/toolboxes/agent-to-agent-authentication#credentials-for-the-agent-card-request).
...
<ZonePivot group="csharp__python__rest-api" options={[{"id": "python", "title": "Python"}, {"id": "csharp", "title": "C#"}, {"id": "rest-api", "title": "REST API"}]} defaultValue="python" />

<ZoneContent group="csharp__python__rest-api" value="python" options={[{"id": "python", "title": "Python"}, {"id": "csharp", "title": "C#"}, {"id": "rest-api", "title": "REST API"}]} values={["python", "csharp", "rest-api"]} defaultValue="python">
...
<ZoneContent group="csharp__python__rest-api" value="csharp" options={[{"id": "python", "title": "Python"}, {"id": "csharp", "title": "C#"}, {"id": "rest-api", "title": "REST API"}]} values={["python", "csharp", "rest-api"]} defaultValue="python">
...
For a maintained .NET Agent Framework integration, see [Use a toolbox with a hosted agent](/agents/use-toolbox-hosted-agent).

---

</ZoneContent>
<ZoneContent group="csharp__python__rest-api" value="rest-api" options={[{"id": "python", "title": "Python"}, {"id": "csharp", "title": "C#"}, {"id": "rest-api", "title": "REST API"}]} values={["python", "csharp", "rest-api"]} defaultValue="python">
...
The recommended way to add an A2A tool is through a toolbox, then attach the toolbox to your agent as an MCP tool. See [What is a toolbox?](/get-started/toolbox-overview)
...
<ZonePivot group="java__typescript" options={[{"id": "typescript", "title": "TypeScript"}, {"id": "java", "title": "Java"}]} defaultValue="typescript" />

<ZoneContent group="java__typescript" value="typescript" options={[{"id": "typescript", "title": "TypeScript"}, {"id": "java", "title": "Java"}]} values={["typescript", "java"]} defaultValue="typescript">
...
<ZoneContent group="java__typescript" value="java" options={[{"id": "typescript", "title": "TypeScript"}, {"id": "java", "title": "Java"}]} values={["typescript", "java"]} defaultValue="typescript">
...
You can expose your Foundry agent as an A2A endpoint directly by enabling the A2A protocol on the agent. For step-by-step instructions, see [Enable incoming A2A on a Foundry agent](/agents/enable-agent-to-agent-endpoint).
...
1. [Register the agent in Foundry Control Plane](../../../control-plane/register-custom-agent), and select **A2A** as the protocol.
...
For authentication setup, see [Agent2Agent (A2A) authentication](/toolboxes/agent-to-agent-authentication).
...
1. Deploy the server and [register it in Foundry Control Plane](../../../control-plane/register-custom-agent).
...
- [Agent2Agent (A2A) authentication](/toolboxes/agent-to-agent-authentication)
- [Register and manage custom agents](../../../control-plane/register-custom-agent)
- [Best practices for tools](/agents/tool-best-practice)
