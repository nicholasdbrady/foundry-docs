...
- An AI model deployed in your project (for example, `gpt-5.4`). Confirm that both the model and project region support Browser Automation in [Tool support by region and model](/agents/limits-quotas-regions#tool-support-by-region-and-model).
...
After you run a sample, verify the tool was called by using tracing in Microsoft Foundry. For guidance on validating tool invocation, see [Best practices for using tools in Microsoft Foundry Agent Service](/agents/tool-best-practice). If you use streaming, you can also look for `browser_automation_preview_call` events.
...
The following Python example demonstrates how to create an AI agent with browser automation capabilities. Select **Prompt Agents** to use the Azure AI Projects SDK to create a server-side prompt agent, or **Hosted Agents** to use the Agent Framework [`FoundryChatClient`](/agents/responses-api) to build an ephemeral, in-process agent.
...
The recommended way to add Browser Automation is through a toolbox, then attach the toolbox to your agent as an MCP tool. See [What is a toolbox?](/get-started/toolbox-overview)
...
This tool uses a Playwright workspace resource to run browser sessions. Review the Playwright workspace documentation for pricing and usage details. For guidance on optimizing tool usage, see [Best practices for using tools in Microsoft Foundry Agent Service](/agents/tool-best-practice).
...
- Use tracing in Microsoft Foundry to confirm whether a tool call occurred. For guidance, see [Best practices for using tools in Microsoft Foundry Agent Service](/agents/tool-best-practice).
...
Review the [transparency note](../../../responsible-ai/agents/transparency-note) when using this tool. The Browser Automation tool is a tool that can perform real-world browser tasks through natural language prompts, enabling automated browsing activities without human intervention.

Review the [responsible AI considerations](../../../responsible-ai/agents/transparency-note) when using this tool.

## Related content

- [Best practices for using tools in Microsoft Foundry Agent Service](/agents/tool-best-practice)
- [Computer use tool for agents](/toolboxes/computer-use)
