...
You can customize file search behavior at runtime, such as specifying which vector store to use per request, by using [structured inputs](/agents/structured-inputs).
...
The following code sample shows how to add the file search tool to a toolbox and attach the toolbox to an agent. You need to upload files and create a vector store before running this code. Select **Prompt Agents** to use the Azure AI Projects SDK to create a server-side prompt agent, or **Hosted Agents** to use the Agent Framework [`FoundryChatClient`](/agents/responses-api) to build an ephemeral, in-process agent.
...
This sample creates the file-search toolbox with the Azure AI Projects SDK, then uses [`FoundryChatClient`](/agents/responses-api) from the Microsoft Agent Framework and connects to the toolbox MCP endpoint by using [`FoundryToolbox`](https://aka.ms/foundry-toolbox-maf). Set the `FOUNDRY_PROJECT_ENDPOINT` and `FOUNDRY_MODEL` environment variables, and sign in by using `az login`.
...
The recommended way to add file search is through a toolbox, then attach the toolbox to your agent as an MCP tool. See [What is a toolbox?](/get-started/toolbox-overview)
...
| -------- | ---------------- |
| Search existing Azure AI Search indexes | [Azure AI Search tool](/toolboxes/ai-search) |
| Search the public web for current information | [Web search tool](/toolboxes/web-search) |
...
For background concepts and lifecycle guidance (readiness, deletion behavior, and expiration policies), see [Vector stores for file search](/toolboxes/vector-stores).
...
- Features and availability vary by region. See [Microsoft Foundry region support](/models/region-support).
...
- [Azure AI Search tool](/toolboxes/ai-search) - Search existing Azure AI Search indexes from your agents
- [Web search tool](/toolboxes/web-search) - Enable agents to search the public web
- [Vector stores for file search](/toolboxes/vector-stores) - Understand vector store lifecycle and expiration
- [Structured inputs](/agents/structured-inputs) - Parameterize agent definitions at runtime
