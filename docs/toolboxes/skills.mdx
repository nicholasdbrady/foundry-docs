...
For the full toolbox workflow — including connections, versioning, and the `azd ai toolbox skill add`, `azd ai toolbox skill list`, and `azd ai toolbox skill remove` commands — see the [Attach skills to a toolbox](/toolboxes/toolbox#attach-skills-to-a-toolbox) section in the toolbox article. Changes from the imperative `azd` skill commands don't take effect for MCP clients until you promote the new version with `azd ai toolbox publish`.
...
Skills support private networking. For setup instructions, see [Set up private networking for Foundry Agent Service](../virtual-networks).
...
| Skills API operations can't reach the project. | Review your network setup. For private networking, see [Set up private networking for Foundry Agent Service](../virtual-networks). |
