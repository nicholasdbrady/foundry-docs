...
- **RBAC**: Grant the **Foundry User** role on the Foundry project to each identity that applies to your scenario:
  ...
  For step-by-step instructions to assign the **Foundry User** role to an agent identity, see [Assign permissions to the agent identity](/agents/agent-identity#assign-permissions-to-the-agent-identity).
...
- **Azure Developer CLI**: [Install the Azure Developer CLI](https://learn.microsoft.com/azure/developer/azure-developer-cli/install-azd) (`azd` 1.27.1 or later) and the unified Foundry CLI extension bundle:
...
You can add the following tools to a toolbox. This table shows SDK and tooling support for each tool and whether the tool can also be attached directly to an agent (outside a toolbox). For how each tool's traffic flows when your project uses network isolation, see [Network isolation for a toolbox](/toolboxes/toolbox-network-isolation).

| Tool | In a toolbox | Direct tool integration | Python SDK | REST API | .NET SDK | JavaScript SDK | Azure Developer CLI | Foundry Toolkit |
| ---- | ------------ | ----------------------- | ---------- | -------- | -------- | -------------- | --- | --------------- |
| [Model Context Protocol (MCP)](/toolboxes/model-context-protocol) | ✅ Yes | ✅ Yes | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [Web search](/toolboxes/web-search) | ✅ Yes | ✅ Yes | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [Azure AI Search](/toolboxes/ai-search) | ✅ Yes | ✅ Yes | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [Code interpreter](/toolboxes/code-interpreter) | ✅ Yes | ✅ Yes | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [File search](/toolboxes/file-search) | ✅ Yes | ✅ Yes | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [OpenAPI](/toolboxes/openapi) | ✅ Yes | ✅ Yes | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | No |
| [Agent-to-agent (A2A)](/toolboxes/agent-to-agent) | ✅ Yes | ✅ Yes | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | No |
| [Browser automation](/toolboxes/browser-automation) | ✅ Yes | ✅ Yes | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | No |
| [Fabric IQ](/toolboxes/fabric-iq) | ✅ Yes | ✅ Yes | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [Work IQ](/toolboxes/work-iq) | ✅ Yes | ✅ Yes | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [Tool search](/toolboxes/tool-search) | ✅ Yes | ❌ No | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [Skills](/toolboxes/skills) | ✅ Yes | ❌ No | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | No |

Tool availability also depends on your project's region and model. Before you deploy a toolbox, verify that your target region supports the tool types you plan to use. See [Tool support by region and model](/agents/limits-quotas-regions#tool-support-by-region-and-model).
...
<Frame>
  <img src="/images/toolbox-vscode-create.png" alt="Screenshot of Foundry Toolkit showing the toolbox name, description, tools, and Publish action." />
</Frame>
...
3. Author a toolbox YAML that references one or more existing connections by name. The YAML never embeds credentials:
   ...
   At least one of `connections`, `skills`, or `tools` must be non-empty. Skill references must point to skills that already exist in the same Foundry project; see [Use skills in Foundry](/toolboxes/skills) to create them with `azd ai skill create`. For end-to-end Tool Search setup details, see [Use tool search](/toolboxes/tool-search).
...
- MCP tools include a `_meta.tool_configuration` block containing runtime settings such as `require_approval`. See [Enforce tool approval](/agents/use-toolbox-hosted-agent#enforce-tool-approval).
...
- **Hosted agent** (your own code running in Foundry Agent Service): see [Use a toolbox with a hosted agent](/agents/use-toolbox-hosted-agent) for Agent Framework, LangGraph, Visual Studio Code, and Azure Developer CLI integration patterns and runtime approval requirements.
...
Foundry MCP Server (preview) exposes toolbox management as MCP tools, so you can retrieve, version, update, and delete toolboxes from an MCP client such as GitHub Copilot in Visual Studio Code. To set up the server, see [Get started with Foundry MCP Server (preview)](/developer-tools-and-integrations/get-started).
...
For the full tool reference, see [Available tools and example prompts for Foundry MCP Server](/developer-tools-and-integrations/available-tools).
...
Attach [skills](/toolboxes/skills) to a toolbox version to make them available to agents through the toolbox MCP endpoint. Each skill reference specifies the skill name and an optional version. Omit `version` to use the skill's `default_version`; pin a `version` string to use an immutable snapshot.
...
For full setup instructions, usage examples, and limitations, see [Reminder tool for self-scheduling agents](/toolboxes/reminder-tool).
...
- [Connect agents to Model Context Protocol servers](/toolboxes/model-context-protocol)
- [Available tools and example prompts for Foundry MCP Server](/developer-tools-and-integrations/available-tools)
- [Add MCP server authentication](/toolboxes/mcp-authentication)
- [Web search tool](/toolboxes/web-search)
- [Azure AI Search tool](/toolboxes/ai-search)
- [Guardrails overview](../../../guardrails/guardrails-overview)
- [Manage skills](/toolboxes/skills)
