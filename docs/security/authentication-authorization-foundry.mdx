...
- A Microsoft Foundry resource with a [custom subdomain](https://learn.microsoft.com/azure/ai-services/cognitive-services-custom-subdomains) configured.
- Understanding of [Azure RBAC concepts](https://learn.microsoft.com/azure/role-based-access-control/overview).
- To assign roles, you need the **Owner** role or **User Access Administrator** role at the appropriate scope.
- (Optional) The [Azure CLI](https://learn.microsoft.com/cli/azure/install-azure-cli) or [Azure SDK for Python](https://pypi.org/project/azure-identity/) installed for programmatic authentication.
- (Optional) Python packages for code samples: `pip install azure-identity requests`

## Control plane and data plane

Azure operations divide into two categories: control plane and data plane. Azure separates resource management (control plane) from operational runtime (data plane). Therefore, you use the control plane to manage resources in your subscription and use the data plane to use capabilities exposed by your instance of a resource type. To learn more about control plane and data plane, see [Azure control plane and data plane](https://learn.microsoft.com/azure/azure-resource-manager/management/control-plane-and-data-plane).
...
<Frame>
  <img src="/images/foundry-rbac-control-data-separation.png" alt="Diagram illustrating separation of control plane and data plane operations with associated RBAC surfaces." />
</Frame>
...
Limitations: Higher initial setup complexity. Requires understanding of role-based access control (RBAC). For more on RBAC in Foundry, see [Role-based access control for Microsoft Foundry](/security/rbac-foundry).
...
For more information on enabling keyless authentication, see [Configure key-less authentication with Microsoft Entra ID](/security/configure-entra-id).
...
**Reference**: [DefaultAzureCredential](https://learn.microsoft.com/python/api/azure-identity/azure.identity.defaultazurecredential) | [azure-identity library](https://learn.microsoft.com/python/api/overview/azure/identity-readme)
...
**Reference**: [Rotate API access keys](../../ai-services/rotate-keys.md)
...
<Frame>
  <img src="/images/role-mapping-diagram.png" alt="Diagram mapping built-in roles to control plane actions and data plane actions in Foundry." />
</Frame>
...
For high-level guidance on setting up Entra ID authentication in Foundry, see [Configure key-less authentication](/security/configure-entra-id).

1. Ensure your Microsoft Foundry resource has a custom subdomain configured. See [Custom subdomains](https://learn.microsoft.com/azure/ai-services/cognitive-services-custom-subdomains). A custom subdomain is required for token-based authentication.
...
**Reference**: [Assign Azure roles](https://learn.microsoft.com/azure/role-based-access-control/role-assignments-portal) | [Role-based access control for Foundry](/security/rbac-foundry)
...
| Custom subdomain required | Resource uses a regional endpoint instead of a custom subdomain | Configure a [custom subdomain](https://learn.microsoft.com/azure/ai-services/cognitive-services-custom-subdomains) on the Foundry resource. Token-based auth requires a custom subdomain. |

## Related content

- [Role-based access control for Foundry](/security/rbac-foundry)
- [Configure key-less authentication with Microsoft Entra ID](/security/configure-entra-id)
- [Rotate API access keys](../../ai-services/rotate-keys.md)
- [Azure built-in roles (AI + machine learning)](https://learn.microsoft.com/azure/role-based-access-control/built-in-roles#ai-+-machine-learning)
