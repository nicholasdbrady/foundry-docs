...
</Warning>   > If clients support in-product messaging, notify users they're in a standby environment. Prior conversation history is unavailable, and new conversations *are lost* after failback.
...
</Warning>   > This step permanently deletes all state created during the failover period. There's no recovery or merge capability for this data.
...
[Resource and data loss recovery strategies](/security/agent-service-operator-disaster-recovery)
