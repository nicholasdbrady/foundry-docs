...
</Info>    > The parameter names in this example must match the policy definition you assign. If they differ in your tenant, update the JSON keys to match the policy definition parameters.
...
</Note>     > By using the [**audit**](https://learn.microsoft.com/azure/governance/policy/concepts/effect-audit) option, you can configure the policy to log information to your own compliance dashboard.
...
<Frame>
<img src="/images/gpt-5-model-card.png" alt="Screenshot of model catalog showing a model card with the publisher name highlighted." />
</Frame>
...
</Info>        > The model ID value must be an exact match for the model. If the model ID isn't an exact match, the policy doesn't work as expected.
