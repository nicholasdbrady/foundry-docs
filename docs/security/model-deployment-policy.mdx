...
<Tabs>
  <Tab title="Azure CLI">
    ...
    </Warning>    > The parameter names in this example must match the policy definition you assign. If they differ in your tenant, update the JSON keys to match the policy definition parameters.
  ...
  <Tab title="Azure portal">
    ...
    1. In the **Parameters** tab, clear **Only show parameters that need input or review** to see all fields:
       ...
       - **Allowed Models Publishers**: Enter a list of publisher names in quotes, separated by commas. Here's an example that shows where to find a publisher name:
         ...
         1. You find publisher name on the model card as shown in the following screenshot. For example, in this case it's `OpenAI`.
            ...
            ![Screenshot of model catalog showing a model card with the publisher name highlighted.](/images/gpt-5-model-card.png)
    ...
    </Warning>        > The model ID value must be an exact match for the model. If the model ID isn't an exact match, the policy doesn't work as expected.
