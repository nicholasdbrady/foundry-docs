...
<Frame>
  <img src="/images/network-security-perimeter-diagram.png" alt="Diagram that shows a Foundry resource inside a network security perimeter boundary, with inbound rules filtering external traffic and outbound rules controlling egress to external services." />
</Frame>
...
For the full set of required actions and permissions (profiles, associations, access rules, diagnostic settings), see [Azure RBAC permissions required for network security perimeter](https://learn.microsoft.com/azure/private-link/network-security-perimeter-role-based-access-control-requirements).
...
Reference: [az network perimeter associable-resource-type list](https://learn.microsoft.com/cli/azure/network/perimeter/associable-resource-type)
...
For portal screenshots and a detailed walkthrough, see [Assign an Azure OpenAI account to a network security perimeter](https://learn.microsoft.com/azure/ai-foundry/openai/how-to/network-security-perimeter#assign-an-azure-openai-account-to-a-network-security-perimeter). The same portal flow applies to Foundry resources.
...
Reference: [az network perimeter association create](https://learn.microsoft.com/cli/azure/network/perimeter/association#az-network-perimeter-association-create)

For CLI (for automation) and full creation steps, see the NSP quickstarts (CLI or PowerShell):
- [Create a network security perimeter (CLI)](https://learn.microsoft.com/azure/private-link/create-network-security-perimeter-cli)
- [Create a network security perimeter (PowerShell)](https://learn.microsoft.com/azure/private-link/create-network-security-perimeter-powershell)
...
Start in Learning mode to observe potential denies. Switch to Enforced mode once you define the required inbound and outbound rules. For more details, see [NSP access modes](https://learn.microsoft.com/azure/private-link/network-security-perimeter-concepts).
...
Reference: [az network perimeter association update](https://learn.microsoft.com/cli/azure/network/perimeter/association#az-network-perimeter-association-update)

## Enable logging

Configure diagnostic settings on the NSP resource to send `allLogs` to Log Analytics, Storage, or Event Hubs.

For detailed steps, see [Diagnostic logs for Network Security Perimeter](https://learn.microsoft.com/azure/private-link/network-security-perimeter-diagnostic-logs).
...
For examples of log fields you can filter on, such as `MatchedRule` or `Profile`, see [Add an Azure OpenAI service to a network security perimeter](https://learn.microsoft.com/azure/ai-foundry/openai/how-to/network-security-perimeter#enable-logging-network-access).
...
- [A walkthrough of configuring inbound and outbound rules](https://learn.microsoft.com/azure/ai-foundry/openai/how-to/network-security-perimeter#add-an-access-rule-for-your-azure-openai-service) using Azure OpenAI NSP (which applies to Foundry data‑plane scenarios).
- Azure CLI reference (access rules): [az network perimeter profile access-rule](https://learn.microsoft.com/cli/azure/network/perimeter/profile/access-rule)
...
1. Open [Microsoft Foundry](https://ai.azure.com/?cid=learnDocs) and perform a model deployment or chat test. Success indicates required traffic is permitted.
1. If blocked, revert to Learning mode or add rules and retry.

## Troubleshooting

- If the portal experience doesn't show your Foundry resource as associable, confirm that Foundry is supported for NSP association in your region and review the supported resource types: [Network security perimeter concepts](https://learn.microsoft.com/azure/private-link/network-security-perimeter-concepts).
- If you don't see logs after enabling diagnostics, confirm that you selected `allLogs` and that your destination is supported: [Diagnostic logs for Network Security Perimeter](https://learn.microsoft.com/azure/private-link/network-security-perimeter-diagnostic-logs).
- If Learning mode looks correct but Enforced mode blocks access, return to Learning mode and add the minimum inbound and outbound rules needed for your scenario: [Azure OpenAI NSP guidance](https://learn.microsoft.com/azure/ai-foundry/openai/how-to/network-security-perimeter).
...
For more information, see [Network security perimeter concepts](https://learn.microsoft.com/azure/private-link/network-security-perimeter-concepts).

## View and manage your configuration

Use REST or CLI to audit and reconcile:
- REST reference (perimeter core): [Network security perimeter REST API](https://learn.microsoft.com/rest/api/network-security-perimeter/)
- (Example) Profile and association operations (CLI): [Azure CLI network perimeter commands](https://learn.microsoft.com/cli/azure/network/perimeter)
...
- [Role-based access control for Foundry](/security/rbac-foundry)
- [Network security perimeter concepts](https://learn.microsoft.com/azure/private-link/network-security-perimeter-concepts)
- [Azure OpenAI NSP article](https://learn.microsoft.com/azure/ai-foundry/openai/how-to/network-security-perimeter)
- [Azure CLI network perimeter reference](https://learn.microsoft.com/cli/azure/network/perimeter)
- [What is Foundry Agent Service?](/agent-service/overview)
