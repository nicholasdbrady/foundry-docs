...
sidebarTitle: "How to configure a private link for Microsoft Foundry projects"
---
...
</Info>    > Don't use the 172.17.0.0/16 IP address range for your virtual network. This range is the default subnet range used by the Docker bridge network on-premises.

## Securely connect to Foundry

To connect to Foundry secured by a virtual network, use one of these methods:

* [Azure VPN Gateway](https://learn.microsoft.com/azure/vpn-gateway/vpn-gateway-about-vpngateways) - Connect on-premises networks to the virtual network over a private connection on the public internet. Choose from two VPN gateway types:
...
* [Azure Bastion](https://learn.microsoft.com/azure/bastion/bastion-overview) - Create an Azure virtual machine (a jump box) in the virtual network, then connect to it through Azure Bastion using RDP or SSH from your browser. Use the VM as your development environment. Because it's in the virtual network, it can access the workspace directly.
...
</Note>    > In the portal UI, the target to which you create the private endpoint might be labeled as an "account" or "resource". Select your Foundry project resource when prompted.
...
</Note>    > The portal refers to the private endpoint target as an "account" or "resource". Choose your Foundry resource as the target.
...
<Frame>
<img src="/images/foundry-portal-firewall.png" alt="Screenshot of the firewalls and virtual networks tab with the all networks option selected." />
</Frame>
...
For Agent Service network isolation scenarios (including network injection, end-to-end isolation, and limitations), see [How to use a virtual network with the Azure AI Agent Service](https://learn.microsoft.com/azure/ai-services/agents/how-to/virtual-networks). 

## End-to-end secured networking for Foundry Agent Service and evaluations

If you're building agents or running evaluations and you want end-to-end network isolation, use the guidance in [How to use a virtual network with the Azure AI Agent Service](https://learn.microsoft.com/azure/ai-services/agents/how-to/virtual-networks). That article includes required DNS zones, a reference architecture, and known limitations.

<Frame>
  <img src="/images/agent-eval-networking.png" alt="Diagram of the recommended network isolation for Foundry." />
</Frame>
...
<Frame>
  <img src="/images/hub-spoke-network.png" alt="Diagram of the hub and spoke network isolation for Foundry projects and agents." />
</Frame>

## Next steps

- [Create a Foundry project](/setup/create-projects)
- [Learn more about Foundry](/overview/what-is-foundry)
