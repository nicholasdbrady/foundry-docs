...
If your Foundry resources use Azure Private Links, the hosted Foundry MCP Server can't reach them through the public endpoint. Disable Private Link, use SDKs/REST APIs, or use a [custom MCP server with private networking](/toolboxes/model-context-protocol#public-and-private-mcp-server-endpoints) through Foundry Agent Service.
