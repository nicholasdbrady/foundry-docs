...
- A [Microsoft Foundry project](../../how-to/create-projects) created in one of the [supported regions](https://learn.microsoft.com/en-us/azure/foundry-classic/how-to/deploy-models-serverless-availability#region-availability) for Claude models. Claude models are currently available in **East US 2** and **Sweden Central** only.
...
- Access to [Azure Marketplace](../../foundry-models/how-to/configure-marketplace) to deploy Foundry Models from partners.
...
Before configuring Claude Code, deploy the available [Claude models](/models/models-from-partners#anthropic) that Claude Code needs. Claude models in Foundry are available for [global standard deployment](/models/deployment-types#global-standard). Claude Code uses different models for different tasks:

<Info>

Claude models in Microsoft Foundry are currently in preview. Model availability might change. Check the [Foundry Models from partners](/models/models-from-partners#anthropic) page for the latest list of available models.
...
1. Follow the instructions in [Deploy Microsoft Foundry Models in the Foundry portal](/models/deploy-foundry-models) to deploy a Claude model, such as Opus 4.6.
...
| Deployment creation fails in the Foundry portal | Verify you have **Contributor** or **Owner** role on the resource group, and your subscription has [Azure Marketplace access](../../foundry-models/how-to/configure-marketplace) enabled. |
