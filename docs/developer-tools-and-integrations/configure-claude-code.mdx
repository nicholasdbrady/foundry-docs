...
- A [Microsoft Foundry project](../../how-to/create-projects) created in one of the [supported regions](/models/models-from-partners#region-availability-by-deployment-type) for Claude model deployments.
- **Contributor** or **Owner** role on your Foundry resource group. For more information, see [Azure RBAC roles](https://learn.microsoft.com/azure/role-based-access-control/built-in-roles).
- Access to [Azure Marketplace](/models/models-from-partners#permissions-required-to-subscribe-to-models-from-partners-and-community) to deploy Foundry Models from partners.
...
- (Optional) [Azure CLI](https://learn.microsoft.com/cli/azure/install-azure-cli) installed with `az login` completed for Microsoft Entra ID authentication.
...
Before configuring Claude Code, deploy the available [Claude models](/models/models-from-partners#anthropic) that Claude Code needs. Most Claude models support global standard deployment. For more information on supported deployment types, see [Claude models in Microsoft Foundry](/models/claude-models#deployment-types-and-regions).

Claude Code uses different models for different tasks:

<Info>

Availability of Claude models in Microsoft Foundry might change. Check the [Foundry Models from partners](/models/models-from-partners#anthropic) page for the latest list of available models.
...
1. Follow the instructions in [Deploy and use Claude models in Microsoft Foundry](/models/use-foundry-models-claude#deploy-claude-models) to deploy a Claude model, such as Opus 4.6.
...
To use Model Router with Claude Code, first deploy the supported Claude models, then deploy Model Router and enable them through [model subset configuration](/models/model-router#optional-route-to-a-model-subset).
...
For more restrictive access, create a custom role scoped to the specific data actions your team needs. For guidance on defining custom roles, see [Role-based access control for Microsoft Foundry](https://learn.microsoft.com/azure/ai-foundry/concepts/rbac-azure-ai-foundry).
...
Run the [az cognitiveservices account deployment delete](https://learn.microsoft.com/cli/azure/cognitiveservices/account/deployment#az-cognitiveservices-account-deployment-delete) command for each deployment:
...
| Rate limit exceeded (HTTP 429) | Check your quotas in the Foundry portal under **Manage** > **Quota**. Review [Claude model quotas and rate limits](/models/claude-models-quotas-limits), and consider adjusting your token limits per the [Claude Code configuration documentation](https://code.claude.com/docs/en/model-config). |
...
| Deployment creation fails in the Foundry portal | Verify you have **Contributor** or **Owner** role on the resource group, and your subscription has [Azure Marketplace access](/models/models-from-partners#permissions-required-to-subscribe-to-models-from-partners-and-community) enabled. |
...
- [Data, privacy, and security for Claude models](../../responsible-ai/claude-models/data-privacy)
- [Microsoft Foundry Models quotas and limits](/models/quotas-limits)
- [Monitor model usage and costs](https://learn.microsoft.com/en-us/azure/foundry-classic/how-to/costs-plan-manage)
- [Configure Claude Desktop for Microsoft Foundry](/developer-tools-and-integrations/configure-claude-desktop)
