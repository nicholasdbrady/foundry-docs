...
Create, test, and deploy [hosted Foundry Agent workflows](../concepts/hosted-agents) by using the [Microsoft Foundry Toolkit for Visual Studio Code extension](https://aka.ms/foundrytk). The toolkit supports agent creation from templates, local testing and debugging with the Agent Inspector for visualization and trace support, and direct deployment to Foundry Agent Service from VS Code. Hosted workflows let multiple agents collaborate in sequence, each with its own model, tools, and instructions.
...
- A [supported region](../concepts/hosted-agents#region-availability) for Hosted agents.
