...
[Create an agent](/developer-tools-and-integrations/create-agent-visual-studio-code).
...
- [Select a Foundry project](/developer-tools-and-integrations/set-up-foundry-project-visual-studio-code) with
  a deployed model. Use a
  [supported hosted-agent region](/agents/hosted-agents#region-availability).
- Permission to use the model and deploy hosted agents. For source-code
  ...
  [Hosted agent permissions](/agents/hosted-agent-permissions).
...
- [Azure CLI](https://learn.microsoft.com/cli/azure/install-azure-cli) for the local authentication steps
  in this article.
- For container deployment, the registry and image access required by
  [Azure Container Registry setup](/agents/hosted-agent-permissions#azure-container-registry-setup).
...
[service limits and availability](/agents/hosted-agents#limits-pricing-and-availability)
...
[Choose a creation route](/developer-tools-and-integrations/create-agent-visual-studio-code#choose-a-creation-route).
...
<Frame>
<img src="/images/hosted-agent-sample-gallery.png" alt="Screenshot of the hosted-agent sample gallery with Basic Hosted Agent selected, workflow samples, and language, framework, and protocol filters." />
</Frame>
...
<Frame>
<img src="/images/create-hosted-agent-project.png" alt="Screenshot of the Create tab showing workspace folder, folder name, model deployment, and Create controls, with local paths hidden." />
</Frame>
...
1. In a terminal, change to the source directory that contains the `.csproj`
   ...
   Reference: [dotnet restore](https://learn.microsoft.com/dotnet/core/tools/dotnet-restore).
...
Reference: [Sign in with Azure CLI](https://learn.microsoft.com/cli/azure/authenticate-azure-cli-interactively).
...
or [credential chains for .NET](https://learn.microsoft.com/dotnet/azure/sdk/authentication/credential-chains).
...
<Frame>
  <img src="/images/local-agent-inspector.png" alt="Screenshot of Agent Inspector connected to localhost on port 8088 with the Responses protocol, tool calls, a latency waterfall, and a run timeline." />
</Frame>
...
and [dotnet run](https://learn.microsoft.com/dotnet/core/tools/dotnet-run).
...
[Choose a hosted-agent protocol](/agents/hosted-agents#which-protocol-should-i-use).
...
[Platform-injected environment variables](/agents/deploy-hosted-agent#platform-injected-environment-variables).
...
<Frame>
<img src="/images/open-hosted-agent-deployment.png" alt="Screenshot of Deploy to Microsoft Foundry under Build in the Foundry Toolkit Developer Tools section." />
</Frame>
...
<Frame>
<img src="/images/code-remote-deployment.png" alt="Screenshot of Basics with Code deployment, Remote package mode, and New agent selected, with the agent name hidden." />
</Frame>
...
<Frame>
<img src="/images/review-hosted-agent-deployment.png" alt="Screenshot of Review + Deploy showing Python 3.14 as an example, a hidden entry point, CPU and memory, and Deploy controls." />
</Frame>
...
requirements, see [Deploy from source code](/get-started/deploy-hosted-agent-code).
For the runtime support policy, see
[Supported hosted-agent runtimes](/get-started/deploy-hosted-agent-code#supported-runtimes).
...
Review [container requirements](/agents/deploy-hosted-agent#container-requirements)
and [private networking guidance](../../agents/how-to/virtual-networks)
...
follow [Migrate from the hosted-agent preview](/agents/migrate-hosted-agent-preview).
...
<Frame>
<img src="/images/open-hosted-agent-playground.png" alt="Screenshot of Hosted Agent Playground under Build in the Foundry Toolkit Developer Tools section." />
</Frame>
...
<Frame>
<img src="/images/hosted-agent-playground.png" alt="Screenshot of the remote hosted-agent playground with a response, session details, and inspection tabs, with agent and session identifiers hidden." />
</Frame>
...
[hosted-agent tracing](/agents/quickstart-tracing-hosted-agent)
and [hosted-agent evaluation](/agents/quickstart-evaluate-hosted-agent).
...
[current agent endpoint and publishing model](/agents/migrate-agent-applications).
...
| Authentication or authorization fails. | Check the local credential and project access. Review [hosted agent permissions](/agents/hosted-agent-permissions) for deployment and runtime identity requirements. |
...
- [Create a prompt agent](/developer-tools-and-integrations/create-prompt-agent-visual-studio-code)
