...
- **No network isolation** — Foundry MCP Server uses the public endpoint `https://mcp.ai.azure.com`. Resources behind Azure Private Links aren't accessible. For private MCP connectivity, [build your own MCP server](/toolboxes/build-your-own-mcp-server) and connect it to Agent Service with [private networking](/toolboxes/model-context-protocol#public-and-private-mcp-server-endpoints).
...
| Private endpoint not reachable | Foundry resources use Azure Private Links that the hosted Foundry MCP Server can't reach. | Remove private endpoint restrictions, use SDKs/REST APIs, or use a [custom MCP server with Agent Service private networking](/toolboxes/model-context-protocol#public-and-private-mcp-server-endpoints). |
...
- Learn how to [build your own MCP server](/toolboxes/build-your-own-mcp-server)
