...
| ----- | ----------- | ------------------- |
| Install | Install `azd` and the Foundry extensions. | [Set up your developer environment](/developer-tools-and-integrations/install-cli-sdk) |
| Scaffold | Initialize a hosted agent from a template or your existing code, or create a prompt-based voice agent. | [Quickstart: Deploy a hosted agent](/agents/quickstart-hosted-agent) or [Quickstart: Create a prompt voice agent](/get-started/prompt-voice-agent) |
| Define | Configure the agent, model deployment dependencies, protocols, tools, and environment in `azure.yaml`. | [Author azure.yaml for hosted agents](/agents/author-azure-yaml) |
| Develop | Write agent logic, add tools using a toolbox, and test locally. | [Toolbox overview](/get-started/toolbox-overview) |
| Deploy | Provision infrastructure and deploy to Foundry. | [Deploy a hosted agent](/agents/deploy-hosted-agent) |
| Operate | Monitor logs, manage versions, and automate runs. | [Manage hosted agents](/agents/manage-hosted-agent) |
| Evaluate | Measure agent quality and improve the prompt. | [Run agent evaluations with the azd CLI](/evaluation/azure-developer-cli-evaluation) |
...
build a [voice agent with a hosted agent](/agents/build-voice-agent) and
use the `invocations_ws` protocol.

To keep conversation logic in a hosted text agent while Voice Live handles audio, use the [hosted voice wrapper workflow](/agents/deploy-hosted-voice-agent). The wrapper and target are separate services in the same `azure.yaml` project. This flow doesn't replace the existing custom `invocations_ws` audio-pipeline flow.

Before using the public-preview voice CLI options, check your installed extension as described in the [voice agent quickstart prerequisites](/get-started/prompt-voice-agent#prerequisites).
...
For the full specification, see [Hosted agent runtime contract](/agents/hosted-agent-contract).
...
`azd ai agent invoke` doesn't implement voice conversations for prompt-based voice agents or hosted voice wrappers. For the CLI behavior and testing guidance, see [Voice agent limitations](/agents/invoke-hosted-agent#voice-agent-limitations).
...
For details, see [Run agent evaluations with the azd CLI](/evaluation/azure-developer-cli-evaluation) and [Optimize agent prompts](/agents/prompt-optimizer).
...
[Quickstart: Create a prompt voice agent](/get-started/prompt-voice-agent).
