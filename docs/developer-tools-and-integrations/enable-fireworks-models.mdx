...
sidebarTitle: "Fireworks models on Microsoft Foundry"
---
...
* **Experiment with the latest open-source models** often before they're available [directly from Azure](/models/models-sold-directly-by-azure).
* **Import and deploy custom model weights** (bring your own model, or BYOM) onto Fireworks' on-demand GPU-backed infrastructure. For more information, see [Import custom models on Microsoft Foundry with Fireworks](/developer-tools-and-integrations/import-custom-models).
* **Scale up** using [Provisioned throughput](/models/provisioned-throughput).
...
* A [Foundry resource](https://learn.microsoft.com/azure/ai-foundry/how-to/create-azure-ai-resource) with a [Foundry project](../../how-to/create-projects).
* An Azure identity with the **Subscription Owner** or **Subscription Contributor** role to enable the feature.
* To deploy models, you need the **Foundry Owner** role on the Foundry project. For more information, see [Azure built-in roles](https://learn.microsoft.com/azure/foundry/concepts/rbac-foundry#permissions-for-each-built-in-role).
...
Data Zone Standard and Data Zone Provisioned deployments of models via Fireworks on Foundry are available in the following [Azure regions](https://learn.microsoft.com/azure/reliability/regions-list):
...
Global Provisioned throughput deployments of base and custom models are available in all global [Azure regions](https://learn.microsoft.com/azure/reliability/regions-list) except for Azure Government cloud environments. All catalog models that support Global Provisioned throughput also support Data Zone Provisioned throughput.
...
1. On the model page, select **Deploy**. For more information on deployment options, see [Deploy Foundry Models in the portal](/models/deploy-foundry-models).
...
1. In the deployment window, configure the following settings:
   ...
   * **Token Plan**: Select **Pay-per-token** -> Datazone Standard or Global Standard, depending on the model, or **Provisioned Throughput** -> Datazone or Global. For more information, see [Deployment types](/models/deployment-types#deployment-type-comparison).
...
All catalog models support the [OpenAI/v1 API](https://aka.ms/openai/v1) for Chat Completions API and the [Foundry SDK](../develop/sdk-overview#foundry-sdk) and endpoint for accessing the Responses API.
...
For step-by-step instructions, see [Import custom models into Foundry](/developer-tools-and-integrations/import-custom-models).
...
Foundry provides the tools and best practices for performing your own [risk and safety evaluations](/evaluation/safety-evaluations-transparency-note) of models.
...
LoRA support is in public preview. See [import custom Fireworks modesl](/developer-tools-and-integrations/import-custom-models) for details.

### How do I import and deploy a custom model?

Custom model import uses a CLI-first workflow with the [Azure Developer CLI](https://learn.microsoft.com/azure/developer/azure-developer-cli/install-azd). For step-by-step instructions, see [Import custom models into Foundry](/developer-tools-and-integrations/import-custom-models).

### How is Fireworks on Foundry billed?

Fireworks models deployed through Foundry support both [pay-per-token](https://aka.ms/fireworks-pricing) and [provisioned throughput](/models/provisioned-throughput) offers.
...
The Responses API is supported via the Foundry Projects API and SDK. Make sure to point your client to your project's API endpoint or use the [Foundry SDK](../develop/sdk-overview#foundry-sdk).
...
* [Import custom models into Foundry](/developer-tools-and-integrations/import-custom-models)
* [Deploy Foundry Models in the portal](/models/deploy-foundry-models)
* [Foundry Models from partners and community](/models/models-from-partners)
* [Foundry model catalog overview](/models/models-sold-directly-by-azure)
* [Deployment types](/models/deployment-types)
* [Provisioned throughput concepts](/models/provisioned-throughput)
