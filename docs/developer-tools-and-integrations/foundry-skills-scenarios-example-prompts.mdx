...
Use the prompts in this article with a coding agent host that has the [Microsoft Foundry Skill installed](/get-started/use-microsoft-foundry-skill), such as GitHub Copilot in Visual Studio Code, Copilot CLI, Codex, or Claude Code. The prompts are grouped by scenario so you can find the right workflow without moving across multiple quickstarts.
