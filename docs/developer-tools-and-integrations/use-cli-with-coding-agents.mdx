...
- Optional: a deployed hosted agent when you need to invoke an agent endpoint. For setup, see [Deploy a hosted agent](deploy-hosted-agent).
...
Use this shape when one repository's CI needs to call an agent owned by a different repository, or when an MCP server fronts several agents and only knows their endpoint URLs. For the full set of `invoke` options, see [Invoke a hosted agent](invoke-hosted-agent).
...
For secrets that shouldn't live in a local `.env` file, store them in a Foundry project connection and reference them with a `${{connections.<name>.credentials.<field>}}` placeholder. See [Run a hosted agent locally](run-hosted-agent-locally) for the full local-run surface.
...
- [Set up CI/CD for hosted agents with the Azure Developer CLI](set-up-ci-cd-cli) for patterns that run `azd ai` in pipelines.
- [Invoke a hosted agent](invoke-hosted-agent) for full `azd ai agent invoke` options, including `--agent-endpoint`.
