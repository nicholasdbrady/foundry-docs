...
1. Find the remote MCP server that you want to connect to, such as the GitHub MCP server. Create or update a Foundry agent with an MCP tool by using the following information:
    ...
    ![Screenshot of the dialog for creating an MCP tool.](/images/agent-mcp-tool-dialog.png)

1. In the **Allowed tools** dropdown list, choose which tools the MCP server can use.

    ![Screenshot of the list of allowed tools in the dialog for creating an MCP tool.](/images/agent-select-mcp-tool.png)
...
1. Select the authentication method for the MCP server tool and proceed.
    ...
    ![Screenshot of the MCP server tool's authentication prompt in the agent playground.](/images/mcp-tool-authentication.png)

1. Select the approval preference for the MCP server tool and proceed.

    ![Screenshot of the MCP server tool's approval preference in the agent playground.](/images/mcp-tool-approval-preference.png)

1. If you chose **Ask every time** for your approval preference, you need to approve or reject the tool call.

    ![Screenshot of the MCP server tool's approval prompt in the agent playground.](/images/mcp-approve-tool.png)
...
1. The agent uses the model and the MCP server tools that you configured in the agent designer to retrieve the information. The source of the information appears in the section for agent annotations.
    ...
    ![Screenshot of the agent playground with agent annotations highlighted in Visual Studio Code after use of the MCP server tool.](/images/mcp-tool-response.png)
...
1. Select the agent that you want to delete, and then select **Delete**.
   ...
   ![Screenshot of the Foundry portal with the Delete command for a selected agent.](/images/delete-agent.png)
