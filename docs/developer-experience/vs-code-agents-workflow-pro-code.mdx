...
Create, test, and deploy [hosted Foundry Agent workflows](/agents/development/hosted-agents) by using the [Microsoft Foundry for Visual Studio Code extension](https://marketplace.visualstudio.com/items?itemName=TeamsDevApp.vscode-ai-foundry). Hosted workflows let multiple agents collaborate in sequence, each with its own model, tools, and instructions.
...
- The project's managed identity with the [Azure AI User](https://aka.ms/foundry-ext-project-role) and [AcrPull](https://learn.microsoft.com/azure/role-based-access-control/built-in-roles/containers#acrpull) roles assigned. Also assign the `acrPull` role to the managed identity of the Foundry project where you plan to deploy the hosted agent.
- A [supported region](/agents/development/hosted-agents#region-availability) for hosted agents.
...
- [Hosted agent concepts](/agents/development/hosted-agents)
