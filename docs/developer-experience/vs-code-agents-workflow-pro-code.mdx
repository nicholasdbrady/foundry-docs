...
---
Create, test, and deploy [hosted Foundry Agent workflows](/agent-service/hosted-agents) by using the [Microsoft Foundry for Visual Studio Code extension](https://marketplace.visualstudio.com/items?itemName=TeamsDevApp.vscode-ai-foundry). Hosted workflows let multiple agents collaborate in sequence, each with its own model, tools, and instructions.
...
- A [supported region](/agent-service/hosted-agents#region-availability) for hosted agents.
...
- [Hosted agent concepts](/agent-service/hosted-agents)
