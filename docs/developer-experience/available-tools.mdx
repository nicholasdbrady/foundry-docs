...
Before using these tools, complete the [Foundry MCP Server setup](/developer-experience/get-started).
...
Each tool is classified as **read** (retrieves information) or **write** (creates, updates, or deletes resources). Write operations affect live resources and billing immediately. Review the [security best practices](/developer-experience/security-best-practices) before running write operations.
...
For more information, see [Role-based access control for Microsoft Foundry](/security/rbac-foundry).
...
- **No network isolation** — Foundry MCP Server uses the public endpoint `https://mcp.ai.azure.com`. Resources behind Azure Private Links aren't accessible. For private MCP connectivity, [build your own MCP server](/agent-service/build-your-own-mcp-server) and connect it to Agent Service with [private networking](/agent-service/model-context-protocol#public-and-private-mcp-server-endpoints).
...
| --- | --- | --- |
| Access denied | Insufficient Azure RBAC role on the Foundry project or account. | Assign at least **Reader** for read tools or **Contributor** for write tools. See [RBAC for Microsoft Foundry](/security/rbac-foundry). |
...
| Private endpoint not reachable | Foundry resources use Azure Private Links that the hosted Foundry MCP Server can't reach. | Remove private endpoint restrictions, use SDKs/REST APIs, or use a [custom MCP server with Agent Service private networking](/agent-service/model-context-protocol#public-and-private-mcp-server-endpoints). |

For more troubleshooting guidance, see [Foundry MCP Server security and best practices](/developer-experience/security-best-practices#troubleshooting).

## Related content

- Get started with [Foundry MCP Server](/developer-experience/get-started)
- Learn how to [build your own MCP server](/agent-service/build-your-own-mcp-server)
- Review [security best practices for MCP servers](/developer-experience/security-best-practices)
