...
Global provisioned throughput deployments of base and custom models are available in all public [Azure regions](https://learn.microsoft.com/azure/reliability/regions-list) with the exception of Azure Government cloud environments.
...
1. In the deployment window, configure the following settings:
   ...
   * **Deployment type** â€” Select **Data Zone Standard** or **Global provisioned throughput**. For more information, see [Deployment types](/models/capabilities/deployment-types#deployment-type-comparison).
   ...
   * **Guardrails** â€” Select **DefaultV2** or **Default** guardrail configuration. By default, models are assigned the Microsoft.DefaultV2 guardrail. For more information, see [Use guardrails to set boundaries on model outputs](/guardrails/guardrails-overview).
