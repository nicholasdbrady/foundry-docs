...
<Frame>
  <img src="/images/runtime-components.png" alt="Diagram that shows an agent and conversation providing input to a response, which calls tools and returns output for the next turn." />
</Frame>
...
If you use a coding agent like GitHub Copilot to design how agents, conversations, and responses work together, the [Microsoft Foundry Skill](/get-started/use-microsoft-foundry-skill) can help apply these components to your app's workflow.
...
For additional agent types (hosted), see [Agent development lifecycle](/agents/development-lifecycle).
...
For the full list of available tools and how to add them to a Toolbox, see the [Toolbox overview](/get-started/toolbox-overview). For best practices, see [Best practices for using tools](/agents/tool-best-practice).
...
For examples that show how conversations and responses work together in code, see [Create and use memory in Foundry Agent Service](/toolboxes/memory-usage).
...
For details about response modes and how to consume outputs, see [Responses API](/models/responses).
...
For conceptual details, see [Memory in Foundry Agent Service](/toolboxes/what-is-memory). For full implementation guidance, see [Create and use memory](/toolboxes/memory-usage).
...
- **Be careful with non-Microsoft services**. If your agent calls tools backed by non-Microsoft services, some data might flow to those services. For related considerations, see the [Toolbox overview](/get-started/toolbox-overview).

## Limits and constraints

Limits can depend on the model, region, and the tools you attach (for example, streaming availability and tool support). For current availability and constraints for responses, see [Responses API](/models/responses).
...
- [Create and use memory in Foundry Agent Service](/toolboxes/memory-usage)
