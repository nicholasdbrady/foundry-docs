...
Event triggers use project connections to authenticate to the external system. For example, the `github_issue` trigger relies on a GitHub connector connection that Foundry provisions in your account's connector namespace. For more information, see [Add managed MCP servers powered by connector namespaces](/toolboxes/connectors).
