...
Event triggers use project connections to authenticate to the external system. For example, the `github_issue` trigger relies on a GitHub connector connection that Foundry provisions in your account's connector namespace. For more information, see [Add managed MCP servers powered by connector namespaces](/tools-and-knowledge/connectors).
...
Routines start an agent from an external trigger. A hosted agent can also schedule *itself* to run again later by calling a built-in reminder tool that you expose through a [toolbox](/agents/toolbox). The agent decides during a run that it needs to follow up, chooses the delay, and the service re-invokes the agent on the same conversation when the delay elapses. Use this pattern for agent-driven follow-ups, such as checking back on a long-running task. For steps, see [Automate agents with routines](/agents/use-routines#let-an-agent-schedule-its-own-reminders).
