...
- **Trigger types.** The supported triggers are `timer` (one-shot), `schedule` (cron-based recurring), and `github_issue` (event-based). The agent-scheduled [reminder tool](/toolboxes/reminder-tool) is available only for hosted agents.
