...
- A deployed hosted agent. To deploy one, see [Deploy a hosted agent](/agents/deploy-hosted-agent).
- The Azure Developer CLI Foundry extensions, for the CLI steps. See [Install the Azure Developer CLI Foundry extensions](/developer-tools-and-integrations/install-cli-foundry-extensions).
...
Think of it as one agent serving many private workspaces. Each session also gets a private `$HOME` filesystem in its own sandbox, isolated by default because each user gets their own session. If you instead place several users in one session, that sandbox is shared - see [Multiplex multiple users in one hosted agent session](/agents/multiplex-session-users). For more about the session model, see [Hosted agents in Foundry Agent Service](/agents/hosted-agents#isolation-model).
...
<ZonePivot group="azd__csharp__javascript__python__rest" options={[{"id": "azd", "title": "Azure Developer CLI"}, {"id": "rest", "title": "REST"}, {"id": "python", "title": "Python"}, {"id": "csharp", "title": "C#"}, {"id": "javascript", "title": "JavaScript"}]} defaultValue="azd" />

<ZoneContent group="azd__csharp__javascript__python__rest" value="azd" options={[{"id": "azd", "title": "Azure Developer CLI"}, {"id": "rest", "title": "REST"}, {"id": "python", "title": "Python"}, {"id": "csharp", "title": "C#"}, {"id": "javascript", "title": "JavaScript"}]} values={["azd", "rest", "python", "csharp", "javascript"]} defaultValue="azd">
...
<ZoneContent group="azd__csharp__javascript__python__rest" value="rest" options={[{"id": "azd", "title": "Azure Developer CLI"}, {"id": "rest", "title": "REST"}, {"id": "python", "title": "Python"}, {"id": "csharp", "title": "C#"}, {"id": "javascript", "title": "JavaScript"}]} values={["azd", "rest", "python", "csharp", "javascript"]} defaultValue="azd">
...
<ZoneContent group="azd__csharp__javascript__python__rest" value="python" options={[{"id": "azd", "title": "Azure Developer CLI"}, {"id": "rest", "title": "REST"}, {"id": "python", "title": "Python"}, {"id": "csharp", "title": "C#"}, {"id": "javascript", "title": "JavaScript"}]} values={["azd", "rest", "python", "csharp", "javascript"]} defaultValue="azd">
...
<ZoneContent group="azd__csharp__javascript__python__rest" value="csharp" options={[{"id": "azd", "title": "Azure Developer CLI"}, {"id": "rest", "title": "REST"}, {"id": "python", "title": "Python"}, {"id": "csharp", "title": "C#"}, {"id": "javascript", "title": "JavaScript"}]} values={["azd", "rest", "python", "csharp", "javascript"]} defaultValue="azd">
...
<ZoneContent group="azd__csharp__javascript__python__rest" value="javascript" options={[{"id": "azd", "title": "Azure Developer CLI"}, {"id": "rest", "title": "REST"}, {"id": "python", "title": "Python"}, {"id": "csharp", "title": "C#"}, {"id": "javascript", "title": "JavaScript"}]} values={["azd", "rest", "python", "csharp", "javascript"]} defaultValue="azd">
...
Within delegation, the platform doesn't fence one delegated end user from another. It enforces a hard boundary only between delegated and non-delegated callers - it lets any of your delegated users into a session your app created. Give each user their own session ID; if you route two users to the same session, they can see each other's data. To deliberately share one session, see [Multiplex multiple users in one hosted agent session](/agents/multiplex-session-users).
...
To see isolation end to end, deploy the [note-taking agent sample](https://github.com/microsoft-foundry/foundry-samples/tree/main/samples/python/hosted-agents/bring-your-own/responses/notetaking-agent), which stores notes per session under `$HOME`: each identity's notes land in a separate session file that only that identity can list or download through the [Session Files API](/agents/manage-hosted-sessions#session-file-operations).

## View sessions across users

By default, each caller sees only their own sessions. An administrator or automation that holds the **Foundry User** role on the project can list and manage every session on the agent, regardless of which identity created it. To manage sessions, see [Manage hosted agent sessions](/agents/manage-hosted-sessions).
...
- If you remain on protocol 1.0.0 for now, see [Pass isolation keys to a hosted agent](/agents/pass-isolation-keys) for how isolation keys work.
- To upgrade, see [Migrate hosted agents](/agents/migrate-hosted-agent-preview).
...
- [Manage hosted agent sessions](/agents/manage-hosted-sessions) to list, inspect, and delete sessions, including across users.
- [Multiplex multiple users in one hosted agent session](/agents/multiplex-session-users) when several users intentionally share one session.
- [Agent identity concepts in Microsoft Foundry](/agents/agent-identity) to understand how agent and user identities work.
- [Invoke a hosted agent with the Azure Developer CLI](/agents/invoke-hosted-agent) for every invoke variant and option.
