...
</Info>    > **You MUST specify `--platform linux/amd64` when building your Docker image.** Hosted agents run on Linux AMD64 infrastructure. Images built for other architectures (such as ARM64 on Apple Silicon Macs) will fail to start with container runtime errors.
