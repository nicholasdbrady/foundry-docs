...
- A [Foundry project](/setup/create-projects) with an agent version you tested and want to publish
- The following role assignments:
  ...
  - For details, see [Role-based access control in the Foundry portal](/security/rbac-foundry).
...
- If your agent uses tools that access Azure resources, plan to reassign any required permissions after publishing. A published agent application uses its own agent identity separate from your project identity. For details, see [Agent identity concepts in Microsoft Foundry](/agents/development/agent-identity) and [Publish and share agents in Microsoft Foundry](/agents/development/publish-agent).
...
<Frame>
<img src="/images/publish-agent.png" alt="Screenshot of the Publish option for an agent version in Microsoft Foundry." />
</Frame>
...
</Warning>   > Don't include secrets, API keys, or sensitive information in any metadata fields. These fields are visible to users.
...
<Frame>
  <img src="/images/agent-store.png" alt="Screenshot of the agent store showing sections such as Your agents and Built by your org." />
</Frame>
...
1. If your agent uses tools that access Azure resources, verify the published agent identity has the required role assignments. See [Agent identity concepts in Microsoft Foundry](/agents/development/agent-identity) for details on reassigning permissions.
...
| Agent works in Foundry but fails after publishing | Agent identity missing required role assignments | The published agent uses its own identity. Reassign RBAC permissions to the new agent identity for any Azure resources the agent accesses. See [Agent identity concepts](/agents/development/agent-identity). |
...
- [Publish and share agents in Microsoft Foundry](/agents/development/publish-agent) — Learn about agent applications, identity, and other publishing options.
- [Publish an agent to Agent 365](/agents/development/agent-365) — Publish to Microsoft Agent 365 for enterprise-wide distribution.
- [Agent identity concepts in Microsoft Foundry](/agents/development/agent-identity) — Understand how agent identity works after publishing.

## Related content

- [Role-based access control in the Foundry portal](/security/rbac-foundry)
- [Microsoft Foundry Playgrounds](/models/capabilities/concept-playgrounds)
