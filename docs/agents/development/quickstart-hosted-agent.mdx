...
sidebarTitle: "Quickstart: Deploy your first hosted agent"
---
...
<Check>Set up an agent sample project with Foundry tools</Check>
<Check>Test the agent locally</Check>
<Check>Deploy to Foundry Agent Service</Check>
<Check>Interact with your agent in the playground</Check>
<Check>Clean up resources</Check>
## Prerequisites

Before you begin, you need:

* An Azure subscription - [Create one for free](https://azure.microsoft.com/free/)
* A [Microsoft Foundry project](/get-started/quickstart-create-foundry-resources) with:
  ...
  * (Optional) An [MCP tool](/agents/tools/model-context-protocol), if you have one you want to use.
...
* (Optional) [Azure CLI](https://learn.microsoft.com/cli/azure/install-azure-cli) version 2.80 or later
...
</Note>    > If a resource group with the same name already exists, `azd provision` uses the existing group. To avoid conflicts, choose a unique environment name or delete the existing resource group first.
...
</Info>    > If you aren't using an MCP server, comment out or remove the following lines in the `agent.yaml` file:
...
<Note>
...
</Note>    > You need **Contributor** access on your Azure subscription for resource provisioning.
    ...
    | Log Analytics Workspace | Manage all log data in one place | No direct cost. See [Log Analytics cost](https://learn.microsoft.com/azure/azure-monitor/logs/log-analytics-workspace-overview) |
...
</Tip>    > Run `azd down` when you finish this quickstart to delete resources and stop incurring charges.
...
</Tip>  > If the playground doesn't load or the agent doesn't respond, verify the agent status is `Started` using the CLI command below.
...
Use the [Microsoft Foundry for Visual Studio Code extension](https://marketplace.visualstudio.com/items?itemName=TeamsDevApp.vscode-ai-foundry) to deploy your agent code to Foundry from the IDE which just a few clicks. See the [VS Code extension documentation](/developer-experience/vs-code-agents-workflow-pro-code) for more information.
...
- [Connect MCP tools](/agents/tools/model-context-protocol) to extend agent functionality
- [Use function calling](/agents/tools/function-calling) to integrate custom logic
- [Add file search](/agents/tools/file-search) to search your documents
- [Enable code interpreter](/agents/tools/code-interpreter) to run Python code

You can see a full list of available tools in the [tool catalog](/agents/tools/tool-catalog) article.
