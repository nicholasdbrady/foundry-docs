...
sidebarTitle: "Build a workflow in Microsoft Foundry"
---
...
- A project in Microsoft Foundry. For more information, see [Create projects](/setup/create-projects).
- Access to create and run workflows in your Foundry project. For more information, see [Azure role-based access control (RBAC) in Foundry](/security/rbac-foundry).
...
- [Work with Declarative (Low-code) Agent workflows in Visual Studio Code](/developer-experience/vs-code-agents-workflow-low-code)
- [Work with Hosted (Pro-code) Agent workflows in Visual Studio Code](/developer-experience/vs-code-agents-workflow-pro-code)
...
</Info>   > Foundry doesn't save workflows automatically. Select **Save** after every change to preserve your work.
...
1. Configure the agent to send output as a JSON schema:
   ...
   <Frame>
  <img src="/images/select-parameters.png" alt="Screenshot that shows the window for configuring a JSON schema format for output." />
</Frame>
...
1. Copy the desired JSON schema and paste it in the **Add response format** window. The following screenshot shows a math example. Select **Save**.
   ...
   <Frame>
  <img src="/images/response-format.png" alt="Screenshot that shows the addition of a response format in JSON." />
</Frame>

  <Info>

</Info>  > Don't include secrets (passwords, keys, tokens) in JSON schemas, prompts, or saved workflow variables.
...
1. Select **Create new variable**. Choose a variable name, and then select **Done**.
   ...
   <Frame>
  <img src="/images/save-output.png" alt="Screenshot that shows options for creating a new variable in a Microsoft Foundry workflow." />
</Frame>
...
1. On the pane that appears, in the **Ask a question** box, enter **What is your name?** or another message. In the **Save user response as** box, enter a variable name; for example, `Var01`. Then select **Done**.
   ...
   <Frame>
  <img src="/images/ask-a-question-node.png" alt="Screenshot that shows the configuration of a question for sending a message." />
</Frame>
...
1. Add a **Send message** action. On the pane that appears, in the **Message to send** area, enter `{Upper(Local.Var01)}`. Then select **Done**.
   ...
   <Frame>
  <img src="/images/variable-message.png" alt="Screenshot that shows the variable instantiation for the action of sending a message." />
</Frame>
...
1. On the preview pane, send a message to the agent to invoke the workflow.
   ...
   <Frame>
  <img src="/images/type-question.png" alt="Screenshot that shows the preview of a question for the action of sending a message." />
</Frame>
...
1. Type `System.` in the **Condition** box to build a condition statement for each if/else branch.
   ...
   <Frame>
  <img src="/images/if-else-condition.png" alt="A screenshot showing the system variables in the if-else condition text box." />
</Frame>
...
| Issue | Solution |
|-------|----------|
| **Workflows** option not visible or can't create/edit workflows | Confirm you have the **Contributor** role or higher on your project. See [Azure role-based access control (RBAC) in Foundry](/security/rbac-foundry). |
...
- [Tool best practices for Foundry agents](/agents/tools/tool-best-practice)
- [Work with Declarative (Low-code) Agent workflows in Visual Studio Code](/developer-experience/vs-code-agents-workflow-low-code)
- [Work with Hosted (Pro-code) Agent workflows in Visual Studio Code](/developer-experience/vs-code-agents-workflow-pro-code)
