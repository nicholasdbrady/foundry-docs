...
<Tabs>
  ...
  <Tab title="Hosted Agents">
    ...
    This sample uses [`FoundryChatClient`](/agents/responses-api) from the Microsoft Agent Framework and calls `get_code_interpreter_tool()` to give the agent a sandboxed Python execution environment. Install the package with `pip install agent-framework-foundry aiohttp`, set the `FOUNDRY_PROJECT_ENDPOINT` and `FOUNDRY_MODEL` environment variables, and sign in with `az login`.
...
For examples of conversation and file cleanup patterns, see [Web search tool](/tools-and-knowledge/web-search) and [File search tool for agents](/agents/file-search).
