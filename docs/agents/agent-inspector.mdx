...
- A local agent to inspect. To start one, see [Run a hosted agent locally with the Azure Developer CLI](/agents/run-hosted-agent-locally).
...
The inspector only targets a local agent on `http://localhost:8088` by default. It doesn't authenticate to or proxy a deployed Foundry agent. For the deployed agent, use [`azd ai agent invoke`](/agents/invoke-hosted-agent) and [`azd ai agent monitor`](/agents/monitor-hosted-agent-logs).
...
- [Run a hosted agent locally with the Azure Developer CLI](/agents/run-hosted-agent-locally) to start the local agent.
- [Invoke a hosted agent with the Azure Developer CLI](/agents/invoke-hosted-agent) for terminal-based local and deployed invocation.
- [Isolate hosted agent sessions per user](/agents/isolate-sessions-per-user) to test isolation against a deployed agent.
