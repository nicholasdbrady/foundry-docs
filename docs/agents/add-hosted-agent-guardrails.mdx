...
* A hosted agent, or a container image ready to deploy as one. See [Deploy a hosted agent](/agents/deploy-hosted-agent).
* A guardrail (RAI policy) on the Foundry resource, and its full Azure Resource Manager (ARM) resource ID. To create one in the Foundry portal, see [Configure guardrails and controls](../../guardrails/how-to-create-guardrails). For a network egress guardrail, you can also [create the policy with `azd provision`](#add-egress-rules-by-using-the-azure-developer-cli). The ARM resource ID has this form:
...
* For the Python SDK method: the [Azure AI Projects client library](https://learn.microsoft.com/python/api/overview/azure/ai-projects-readme) for Python, version 2.2.0 or later:
...
In `azure.yaml` the field is camelCased as `raiPolicyName`. The deprecated standalone `agent.yaml` uses the snake_case `rai_policy_name`. Both map to `rai_config.rai_policy_name` on the agent version. Don't declare the guardrail in `agent.manifest.yaml` - `azd` reads that file only during `azd ai agent init` and ignores it at deploy time.
  ...
  <Tab title="Python SDK">
    ...
    Reference: [HostedAgentDefinition](https://learn.microsoft.com/python/api/azure-ai-projects/azure.ai.projects.models.hostedagentdefinition), [ContainerConfiguration](https://learn.microsoft.com/python/api/azure-ai-projects/azure.ai.projects.models.containerconfiguration), and [RaiConfig](https://learn.microsoft.com/python/api/azure-ai-projects/azure.ai.projects.models.raiconfig).
  ...
  <Tab title="JavaScript/TypeScript SDK">
...
You can add a maximum of **480 egress rules** per policy. This limit applies to all egress rule actions: **Allow**, **Deny**, **Transform**, and **Rewrite**. To request an increase to this limit, [create an Azure support request](https://learn.microsoft.com/azure/azure-portal/supportability/how-to-create-azure-support-request).
...
1. Add the following Bicep to the resource-group-scoped infrastructure for the resource group that contains your Foundry resource:
    ...
    Reference: [Microsoft.CognitiveServices accounts/raiPolicies](https://learn.microsoft.com/azure/templates/microsoft.cognitiveservices/2026-05-15-preview/accounts/raipolicies).
...
<Frame>
<img src="/images/network-egress-control.png" alt="Screenshot of the Network control in a guardrail showing the Egress rules row and the Outbound requests default action." />
</Frame>
...
<Frame>
<img src="/images/egress-rule-list.png" alt="Screenshot of the Create egress rules dialog with Audit and Enforce modes, a host match field, and an action list." />
</Frame>

1. Select **Create**, and then assign the guardrail to your hosted agent.

For details about creating and assigning guardrails in the portal, see [Configure guardrails and controls](../../guardrails/how-to-create-guardrails).
...
<Frame>
  <img src="/images/network-egress-decision-trace.png" alt="Screenshot of the Trajectories tab in the Foundry portal playground, showing a Network egress decision span in the trace timeline." />
</Frame>
...
- [Guardrails and controls overview](../../guardrails/guardrails-overview) — what guardrails are, the risks they detect, and where they intervene.
- [Configure guardrails and controls](../../guardrails/how-to-create-guardrails) — create the RAI policy you reference here.
- [Networking options for Foundry Agent Service](../concepts/networking-options) — how egress controls fit with virtual network and private networking options.
- [Deploy a hosted agent](/agents/deploy-hosted-agent) — the full deployment workflow for hosted agents.
