...
- An initialized hosted agent project with agent code and tests. To create a project, see [Initialize an agent project](/agents/init-agent-project).
...
- For deployed integration tests and structured evaluations, a deployed hosted agent. To deploy one, see [Deploy a hosted agent](/agents/deploy-hosted-agent).
...
- [Run a hosted agent locally with the Azure Developer CLI](/agents/run-hosted-agent-locally) for local development setup.
- [Debug a hosted agent](/agents/debug-hosted-agent) to diagnose issues when tests fail.
- [Monitor hosted agent logs with the Azure Developer CLI](/agents/monitor-hosted-agent-logs) to inspect runtime behavior.
