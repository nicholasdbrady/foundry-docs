...
- An initialized hosted agent project that works locally with `azd ai agent run` and `azd ai agent invoke --local`. For setup, see [Initialize an agent project](/agents/init-agent-project).
- A project that you've successfully deployed at least once with `azd up`. For deployment steps, see [Deploy a hosted agent](/agents/deploy-hosted-agent).
- The [azd Foundry extensions installed](/developer-tools-and-integrations/install-cli-foundry-extensions) locally and in your pipeline runner.
...
For more on running `azd ai` commands without an azd project on disk, see [Set the azd project context](/developer-tools-and-integrations/cli-project-context).
...
- [Deploy a hosted agent](/agents/deploy-hosted-agent) to understand what happens during deployment.
- [Azure YAML reference](/agents/azure-yaml-reference) to review deployment configuration details.
- [Configure a DevOps pipeline with azd](https://learn.microsoft.com/azure/developer/azure-developer-cli/configure-devops-pipeline) for the full azd pipeline reference.
