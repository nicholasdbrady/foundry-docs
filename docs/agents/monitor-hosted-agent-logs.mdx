...
- For session-specific logs, a session ID from an `azd ai agent invoke` response. To invoke an agent, see [Invoke a hosted agent with the Azure Developer CLI](/agents/invoke-hosted-agent).
...
For a comprehensive debugging workflow, see [Debug a hosted agent](/agents/debug-hosted-agent).

## Related content

- [Debug a hosted agent](/agents/debug-hosted-agent) for step-by-step diagnostic workflows.
- [Test a hosted agent](/agents/test-hosted-agent) for validation strategies before production.
- [Isolate hosted agent sessions per user](/agents/isolate-sessions-per-user) for logs in isolated sessions.
