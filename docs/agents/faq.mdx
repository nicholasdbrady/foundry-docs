...
- [Quotas and limits](./concepts/limits-quotas-regions.md)"
---
...
**Standard setup** stores agent data (threads, files, and vector stores) in your Azure resources that you declare through capability settings.

To compare setup options and choose the right one, see [Set up your environment](environment-setup.md#choose-your-setup) and [Configure agent capability settings](../how-to/configure-capability-settings.md).
...
- [Quickstart: Create a new agent](../quickstarts/get-started-code.md)
- New Foundry experience: [Quickstart: Get started with agents in code](../quickstarts/get-started-code.md)

### What are voice-first agents?

Voice-first agents are a type of prompt agent for real-time spoken conversations. Use them when users speak to the agent and hear spoken responses instead of primarily typing and reading text.

Voice-first agents use the same core Agent Service concepts as other prompt agents, including projects, saved agent versions, tools, identity, tracing, and monitoring. They can also have voice-specific model, audio, channel, and preview availability requirements.

For more information, see [How voice-based agents work](overview.md#how-voice-based-agents-work).
...
For coverage details across Foundry capabilities, see [Customer-managed key encryption in Microsoft Foundry](../concepts/customer-managed-keys.md).
...
No. All [quotas](./concepts/limits-quotas-regions.md) apply to using models with Foundry Agent Service.

## Tools and integrations

### Where can I learn about tools like Code Interpreter and File Search?

Start with [Toolbox overview](./concepts/toolbox-overview.md), then see the tool-specific guidance:

### How do I bring my own storage, Azure Cosmos DB, or Azure AI Search for agent data?

Declare those resources in the capability settings on your Foundry account or project.

- How-to: [Configure agent capability settings](../how-to/configure-capability-settings.md)
...
To understand usage and monitoring options, see [Metrics](../observability/how-to/how-to-monitor-agents-dashboard.md) and [Monitor the service](../observability/how-to/how-to-monitor-agents-dashboard.md).

### What should I do for business continuity and disaster recovery (BCDR)?

Agent Service BCDR guidance depends on your setup and the resources you provision. For the current guidance, see [Foundry Agent Service disaster recovery](../how-to/agent-service-disaster-recovery.md).
