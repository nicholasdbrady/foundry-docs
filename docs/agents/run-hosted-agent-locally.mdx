...
If you use a coding agent like GitHub Copilot, the [Microsoft Foundry Skill](/get-started/use-microsoft-foundry-skill) can help configure local startup, ports, secrets, and test commands before you deploy.
...
- Pass `--start-command` to override the command in `azure.yaml`:
   ...
   This command overrides the `startupCommand` defined in your `azure.yaml` service configuration. The `startupCommand` is the default command used both for local development (`azd ai agent run`) and for container startup when deployed. For more information, see [azure.yaml service configuration](/agents/azure-yaml-reference).
