...
sidebarTitle: "Use SharePoint content with agent API"
---
...
- Developers and end users have at least `Azure AI User` RBAC role assigned on the Foundry project. For more information about Azure role-based access control, see [Azure role-based access control in Foundry](/security/rbac-foundry).
...
</Note>     > Your `site_url` needs to follow the format above. If you copy the entire value from the address bar of your SharePoint, it doesn't work.
...
For more information about delegated access and identity passthrough in Foundry, see [Agent identity concepts in Microsoft Foundry](/agents/development/agent-identity).
