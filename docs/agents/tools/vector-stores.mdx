...
Vector store objects give the [file search](/agents/tools/file-search) tool the ability to search your files. When you add a file to a vector store, the service parses, chunks, embeds, and indexes it so the tool can run both keyword and semantic search.

Vector stores can be attached to both agents and conversations. Currently, you can attach at most one vector store to an agent and at most one vector store to a conversation. For a conceptual overview of conversations, see [Agent runtime components](/agents/development/runtime-components).

In the current agents developer experience, response generation uses **responses** and **conversations**. Some SDKs and older samples use the term *run*. If you see both terms, treat *run* as response generation. For migration guidance, see [How to migrate to the new agent service](/setup/migrate).

For a list of limits for vector search (such as maximum allowable file sizes), see the [quotas and limits](/agents/development/limits-quotas-regions) article.

## Prerequisites

- A [Microsoft Foundry project](/setup/create-projects).
- An agent or conversation that uses the [file search](/agents/tools/file-search) tool.
...
- Feature availability can vary by region. For current coverage, see [Microsoft Foundry feature availability across cloud regions](/operate/region-support).
...
For file size and token limits, see [quotas and limits](/agents/development/limits-quotas-regions).
...
For current default retrieval settings (chunk size and overlap, embedding model, and the maximum number of chunks added to context), see [How it works](/agents/tools/file-search#how-file-search-works).
...
To set up your environment, see [Agent environment setup](/setup/environment-setup). For more detail, see [Dependency on agent setup](/agents/tools/file-search#file-search-behavior-by-agent-setup-type).
...
To check readiness, use the SDK polling helpers (for example, *create-and-poll* and *upload-and-poll*) or poll the vector store object until its status is **completed**. For code examples, see [File search tool for agents](/agents/tools/file-search).
...
1. Upload your files and create a vector store. For a step-by-step example, see [Upload files and add them to a vector store](/agents/tools/file-search#upload-files-and-add-them-to-a-vector-store).
...
1. Attach the vector store to the agent or conversation that you use for file search. Keep the attachment limits in mind. See [Vector stores](/agents/tools/file-search#vector-stores).
1. Create a response that uses file search and verify that the tool is retrieving from the expected sources. See [Create response with file search](/agents/tools/file-search#create-response-with-file-search) and [Verify results](/agents/tools/file-search#verify-file-search-results).
1. Manage lifecycle: remove files you no longer need, and plan for expiration policies (especially for vector stores created by conversation helpers). See [Vector stores](/agents/tools/file-search#vector-stores) and [Conversation vector stores have default expiration policies](/agents/tools/file-search#conversation-vector-stores-have-default-expiration-policies).
...
When a vector store expires, response generation for that conversation fails. To fix the issue, recreate a new vector store with the same files and reattach it to the conversation. For more detail, see [Conversation vector stores have default expiration policies](/agents/tools/file-search#conversation-vector-stores-have-default-expiration-policies).

## Supported file types and key limits

For the supported file types list and encoding requirements, see [Supported file types](/agents/tools/file-search#supported-file-types).

Key limits to keep in mind:

- You can attach at most one vector store to an agent and at most one vector store to a conversation.
- File size and token limits vary by feature. See [Quotas and limits](/agents/development/limits-quotas-regions).
...
- **Uploads or ingestion fail**: Check file size and token limits in [Quotas and limits](/agents/development/limits-quotas-regions).

## Next steps

- Learn more about the [file search tool](/agents/tools/file-search)
- Review [tool best practices](/agents/tools/tool-best-practice) for guidance on reliability and security
- Learn about [agent runtime components](/agents/development/runtime-components)
