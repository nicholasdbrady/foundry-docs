...
sidebarTitle: "Use Grounding with Bing Search tools with the agents API"
---
...
<Note>
Start with the [web search tool (preview)](/agents/tools/web-search). Learn more about the differences between web search and Grounding with Bing Search (or Grounding with Bing Custom Search) in the [web grounding overview](/agents/tools/web-overview).
</Note>
### Usage support
...
- Azure RBAC roles:
  ...
  - **Azure AI Project Manager** role to create project connections in Foundry. For more information, see [Role-based access control for Microsoft Foundry](/security/rbac-foundry).
...
</Info>   > - You need the **Contributor** or **Owner** role at the subscription or resource group level to create Bing resources and get resource keys.
...
<Note>
See [best practices](/agents/tools/tool-best-practice) for information on optimizing tool usage.
</Note>
## Code examples

<Note>
- You need the latest prerelease package. See the [quickstart](/get-started/get-started-code) for details.
...
- [Tool use best practices](/agents/tools/tool-best-practice) - Learn optimization strategies for agent tools
- [Web search tool (preview)](/agents/tools/web-search) - Use web search without configuring Bing tool parameters
...
- [Connect OpenAPI tools to agents](/agents/tools/openapi) - Integrate custom APIs with your agents
- [Discover tools in the Foundry Tools (preview)](/agents/tools/tool-catalog) - Explore all available agent tools in Foundry Agent Service
