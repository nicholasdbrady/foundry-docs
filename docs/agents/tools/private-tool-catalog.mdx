...
</Note>    > The API Center name is the name that developers use to find the catalog in Foundry Tools. Use a descriptive name.
...
</Info>    > Treat any credentials as secrets. Don't paste secrets into prompts or source control. For guidance on authentication approaches in Agent Service (including shared and per-user authentication), see [MCP server authentication](/agents/tools/mcp-authentication).
