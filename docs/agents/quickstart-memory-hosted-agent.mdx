...
In this quickstart, you give a [hosted agent](/agents/hosted-agents) persistent, semantic memory backed by a Foundry [memory store](/agents/runtime-components#attach-memory-to-an-agent-preview). Without memory, every conversation starts from scratch. With a memory store, the agent retains stable facts about a user, such as a name or a dietary preference, and recalls them in later sessions.
...
If you use a coding agent like GitHub Copilot, the [Microsoft Foundry Skill](/get-started/use-microsoft-foundry-skill) can help provision the memory store, wire it to your hosted agent, and test recall behavior.
...
| `azd provision` fails with a permissions error | Confirm your identity has the project roles listed in the [Prerequisites](/agents/quickstart-hosted-agent#prerequisites). |
...
<Card title="Evaluate your hosted agent" icon="arrow-right" href="../../observability/quickstarts/quickstart-evaluate-hosted-agent.md" />
## Related content

- [What are hosted agents?](/agents/hosted-agents)
- [Attach memory to an agent](/agents/runtime-components#attach-memory-to-an-agent-preview)
