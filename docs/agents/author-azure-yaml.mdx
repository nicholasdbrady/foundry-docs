...
For a field-by-field description of every service and property, see [azure.yaml reference for hosted agents](/agents/azure-yaml-reference).
...
* Azure Developer CLI (`azd`) version 1.27.1 or later. For installation instructions, see [Install azd](https://learn.microsoft.com/azure/developer/azure-developer-cli/install-azd).
* The [Azure Developer CLI Foundry extensions](/developer-tools-and-integrations/install-cli-foundry-extensions) installed.
* The `azure.ai.agents` extension version `1.0.0-beta.8` or later and the `azure.ai.projects` extension version `1.0.0-beta.4` or later. For installation and version verification, see [Install the Azure Developer CLI Foundry extensions](/developer-tools-and-integrations/install-cli-foundry-extensions).
* An authenticated Azure session (`azd auth login`).
* A hosted agent project. To scaffold one, see [Initialize a hosted agent project](/agents/init-agent-project).
...
The `src/my-agent` directory holds your agent code and, for container builds, a `Dockerfile`. Running `azd ai agent init` scaffolds both. See [Initialize a hosted agent project](/agents/init-agent-project).
...
hosted agent](/agents/use-toolbox-hosted-agent).
...
To deploy a prebuilt image instead of building from source, set the `image` field on the agent service to the image URL. For the `codeConfiguration` fields that `code` mode uses, see [Deploy a hosted agent from source code](/get-started/deploy-hosted-agent-code).
...
| --- | --- | --- |
| Private networking | You need an account private endpoint and either customer-managed or Microsoft-managed agent egress. | [Private-network configuration](/agents/azure-yaml-reference#private-network-configuration) |
| Source-code deployment | You want Agent Service to build a ZIP package from your Python or .NET source. | [Source-code deployment](/agents/azure-yaml-reference#source-code-deployment) |
| Prebuilt image deployment | Your CI system builds, scans, and publishes the container image. | [Container builds and prebuilt images](/agents/azure-yaml-reference#container-builds-and-prebuilt-images) |
| Responsible AI policies | You need a Responsible AI policy on the hosted-agent version. | [Responsible AI policies](/agents/azure-yaml-reference#responsible-ai-policies) |
| Memory stores | Your application uses durable memory with chat and embedding model deployments. | [Memory stores](/agents/azure-yaml-reference#memory-stores) |
| Agent endpoint and discovery card | You expose agent protocols for discovery, endpoint authorization, or A2A. | [Configure an agent endpoint](/agents/azure-yaml-reference#configure-an-agent-endpoint) |
| Activity and WebSocket protocols | You need Microsoft 365 and Teams Activity support or WebSocket invocations. | [Additional runtime protocols and Activity endpoints](/agents/azure-yaml-reference#additional-runtime-protocols-and-activity-endpoints) |
| Agent identity and platform environment | Your code uses the platform identity, injected environment variables, or deployed endpoint URLs. | [Platform environment, identity, and endpoints](/agents/azure-yaml-reference#platform-environment-identity-and-endpoints) |
...
* [azure.yaml reference for hosted agents](/agents/azure-yaml-reference)
...
* [Hosted agent runtime contract](/agents/hosted-agent-contract)
