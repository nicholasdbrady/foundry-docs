...
If you use a coding agent like GitHub Copilot, the [Microsoft Foundry Skill](/get-started/use-microsoft-foundry-skill) can help you choose a starting template, scaffold the project, and interpret the generated `azd` configuration.
...
Your agent code must meet the [hosted agent runtime contract](/agents/hosted-agent-contract):

* Listen on port 8088.
* Serve a health probe at `GET /readiness`.
* Handle one of the supported protocols (`responses` or `invocations`).

If your code doesn't already speak one of these protocols, [add a protocol adapter](/agents/add-protocol-adapter), a lightweight SDK wrapper that translates between the Foundry protocol and your agent's logic.
...
When you initialize against an existing project with `--project-id`, the tooling skips the automatic role assignments that it performs when it creates a new project. Make sure the required roles are already assigned. For the full matrix, see [Hosted agent permissions reference](/agents/hosted-agent-permissions).
