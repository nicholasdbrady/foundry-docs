...
In **Phone numbers**, select **Add a number** and follow [Integrate telephony channels with a voice agent](/agents/voice-agent-telephony-channels).

To place outbound calls, use the [outbound call-job API](/agents/voice-agent-telephony-channels#place-outbound-calls-by-using-the-api). Outbound jobs use a provider connection and don't require an inbound number binding.
...
- [Integrate a telephony channel](/agents/voice-agent-telephony-channels)
- [Voice agent tracing, monitoring, and evaluation](/agents/voice-agent-observability).
- [Publish an Agent Application](https://learn.microsoft.com/azure/foundry/agents/how-to/agent-applications)
