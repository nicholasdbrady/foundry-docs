...
This step is the first step in the [optimization workflow](/agents/agent-optimizer-overview#the-optimization-workflow). The baseline configuration you create defines the inputs the optimizer improves: instructions, tools, skills, and the model. Your agent works the same whether or not optimization is active.
...
The rest of this article gives a complete example and explains how configuration resolution works. After an optimization run finishes, you apply the winning candidate and deploy—see [Deploy the winner](/agents/optimize-agent-targets#deploy-the-winner).

## Prerequisites

- A [Foundry project](../../how-to/create-projects) with a deployed hosted agent
- Familiarity with [hosted agents](/agents/hosted-agents)
...
During evaluation, the optimizer invokes your agent against every task in your dataset, so any external tool calls run for real. For guidance on avoiding unintended side effects, see [How the agent optimizer works](/agents/agent-optimizer-overview#how-the-agent-optimizer-works).

</Note>

1. **After applying a winner**: You run `azd ai agent optimize apply --candidate <id>` to write the optimized config files into `.agent_configs/<candidate_id>/` in your project. Then `azd deploy` deploys the agent with the improved configuration. For the full apply and deploy steps, see [Deploy the winner](/agents/optimize-agent-targets#deploy-the-winner).
