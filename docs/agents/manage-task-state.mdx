...
A [long-running hosted agent](/agents/long-running-agent-resilience) recovers from crashes only if its progress is durable. This article shows how to keep progress markers and checkpoints in the Foundry state store, and how to back a framework checkpointer so framework-level recovery survives restarts.
...
A store is bound to one caller-chosen name. Encode the workflow, session, thread, or run scope in that name. For the full store surface, see [Durable state store for hosted agents](/agents/agent-state-store).
...
- [Durable state store for hosted agents](/agents/agent-state-store)
- [Resilience for long-running hosted agents](/agents/long-running-agent-resilience#preserve-agent-progress)
