...
- [Isolate hosted agent sessions per user](/agents/isolate-sessions-per-user) for the default, per-caller isolation model.
...
- [Quotas and limits for Foundry Agent Service](/agents/limits-quotas-regions) for regional concurrent-session limits.
- [Migrate hosted agents](/agents/migrate-hosted-agent-preview) to move a container to protocol 2.0.0.
- [Hosted agent runtime contract](/agents/hosted-agent-contract) for the platform headers and environment variables a container receives.
