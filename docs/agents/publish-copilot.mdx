...
<Frame>
<img src="/images/agent-store.png" alt="A screenshot of the Agent store in Microsoft Copilot." />
</Frame>
...
To download the same package by using the REST API, see [Download and inspect the app package](/agents/publish-copilot-virtual-network#download-and-inspect-the-app-package).
...
Use the following table to resolve errors that occur while you publish from the portal. For more extensive troubleshooting, including how to find a published agent in the store, resolve errors when you chat with it, or troubleshoot a sign-in card redirect, see [Runtime issues](/agents/publish-copilot-virtual-network#runtime-issues).
...
- [Publish agents to Microsoft Copilot and Microsoft Teams by using the REST API](/agents/publish-copilot-virtual-network)
