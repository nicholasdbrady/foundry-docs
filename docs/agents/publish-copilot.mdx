...
If you're migrating from a previous publishing model, see [Migrate from Agent Applications to the new agent model](/agents/migrate-hosted-agent-preview).
...
For more information on selecting an active agent version and configuring other agent settings, see [Publish and share agents](/agents/configure-agent).
...
| Publishing fails with identity error | The agent doesn't have a unique identity (`agent.identity` is null) | See the [migration guide](/agents/migrate-agent-applications) for steps to resolve this. |
