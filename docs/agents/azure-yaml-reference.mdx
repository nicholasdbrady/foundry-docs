...
To learn how to compose and author this file step by step, see [Author azure.yaml for hosted agents](/agents/author-azure-yaml).
...
Developer CLI Foundry extensions](/developer-tools-and-integrations/install-cli-foundry-extensions).

Public-preview voice services require Azure Developer CLI version 1.32.0 or later and an agent extension that exposes the voice CLI options. The general minimum versions below don't establish voice feature support. Check the [voice agent prerequisites](/get-started/prompt-voice-agent#prerequisites).
...
For model and audio examples, see [Configure a voice agent](/agents/configure-voice-agent).
...
For the complete two-service sample, see [Deploy a hosted voice agent with azd](/agents/deploy-hosted-voice-agent).
...
to hosted agents](/agents/add-hosted-agent-guardrails).
...
tool or memory store APIs. For details, see [Use memory with agents](/toolboxes/memory-usage).
...
For the full protocol specification, see [Hosted agent runtime contract](/agents/hosted-agent-contract).
...
endpoint. For runtime protocol behavior, see [What are hosted agents?](/agents/hosted-agents).
...
resources. For details, see [Hosted agent permissions reference](/agents/hosted-agent-permissions).
...
dependency guidance, see [Deploy a hosted agent from source code](/get-started/deploy-hosted-agent-code).
...
see [Deploy a hosted agent with a private Azure Container Registry](/agents/deploy-hosted-agent-private-azure-container-registry).
...
* [Hosted agent infrastructure with the Azure Developer CLI](/agents/cli-infrastructure)
