...
- An initialized hosted agent project with an `azure.yaml` file. To create or update a project, see [Author azure.yaml for hosted agents](/agents/author-azure-yaml).
...
For secrets that shouldn't live in a local `.env` file, store them in a Foundry project connection and reference them with a `${{connections.<name>.credentials.<field>}}` placeholder. The platform resolves the placeholder at runtime. See [Run a hosted agent locally with the Azure Developer CLI](/agents/run-hosted-agent-locally) for the local run workflow.

## Related content

- [Author azure.yaml for hosted agents](/agents/author-azure-yaml) to configure hosted agent services.
- [Azure YAML reference](/agents/azure-yaml-reference) for azd environments and `azd env set`.
- [Deploy a hosted agent](/agents/deploy-hosted-agent) to redeploy after configuration changes.
