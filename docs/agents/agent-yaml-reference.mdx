...
Agent manifests (`agent.manifest.yaml`) and standalone agent definitions (`agent.yaml`) are deprecated. As of the Foundry `azd` extensions (`azure.ai.agents` 1.0.0-beta.1), all hosted agent configuration lives in a single `azure.yaml`. See [Author azure.yaml for hosted agents](/agents/author-azure-yaml) and [azure.yaml reference for hosted agents](/agents/azure-yaml-reference).
...
Both files describe *what* an agent is: its model, tools, protocols, environment, and resources. This is distinct from `azure.yaml`, which describes *how* to provision and deploy Azure resources. For the `azure.yaml` reference, see [azure.yaml reference for hosted agents](/agents/azure-yaml-reference).
