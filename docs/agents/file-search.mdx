...
The following code sample shows how to create an agent with the file search tool enabled. You need to upload files and create a vector store before running this code. Select **Prompt Agents** to use the Azure AI Projects SDK to create a server-side prompt agent, or **Hosted Agents** to use the Agent Framework [`FoundryChatClient`](/agents/responses-api) to build an ephemeral, in-process agent.
...
<Tabs>
  ...
  <Tab title="Hosted Agents">
    ...
    This sample uses [`FoundryChatClient`](/agents/responses-api) from the Microsoft Agent Framework. It uploads a file, creates a vector store, and calls `get_file_search_tool()` to give the agent access to the indexed content. Install the package with `pip install agent-framework-foundry aiohttp`, set the `FOUNDRY_PROJECT_ENDPOINT` and `FOUNDRY_MODEL` environment variables, and sign in with `az login`.
...
| -------- | ---------------- |
| Search existing Azure AI Search indexes | [Azure AI Search tool](/agents/ai-search) |
...
- [Azure AI Search tool](/agents/ai-search) - Search existing Azure AI Search indexes from your agents
