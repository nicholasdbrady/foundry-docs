...
- A voice-based agent. See [Quickstart: Create a voice-based prompt agent](/get-started/prompt-voice-agent).
- Familiarity with the agent definition fields in [Configure a voice agent](/agents/configure-voice-agent).
...
See [Configure a voice agent](/agents/configure-voice-agent) for the full set of options.
...
For details on both steps, see [Voice agent tracing, monitoring, and evaluation](/agents/voice-agent-observability).
