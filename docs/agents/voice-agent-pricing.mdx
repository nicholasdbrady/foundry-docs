...
For general cost management guidance, see [Plan and manage costs for Microsoft Foundry](/models/manage-costs).
...
For token usage on stored conversations, read the conversation's `usage` field after the session completes. See [Voice agent tracing, monitoring, and evaluation](/agents/voice-agent-observability).
