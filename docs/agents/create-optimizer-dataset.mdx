...
Creating these assets is the second step in the [optimization workflow](/agents/agent-optimizer-overview#the-optimization-workflow), after you [make your agent optimizer-ready](/agents/make-agent-optimizer-ready). The optimizer uses them to score your baseline and rank candidates.
...
For the interactive wizard, non-interactive flags, and details about the generated artifacts, see [Initialize evaluation assets](/evaluation/azure-developer-cli-evaluation#initialize-evaluation-assets).
...
To customize the generated assets, see [Customize evaluators](#customize-evaluators-advanced) and [Create a custom dataset](#create-a-custom-dataset-advanced). To change run options, edit `eval.yaml`; see [Configure the optimization run](/agents/optimize-agent-targets#configure-the-optimization-run).
...
Both sources use the same task schema described in the next section. For the `eval.yaml` wiring, see [Configure the optimization run](/agents/optimize-agent-targets#configure-the-optimization-run).
