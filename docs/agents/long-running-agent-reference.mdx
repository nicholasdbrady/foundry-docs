...
This reference lists the AgentServer SDK surface for building [long-running, resilient hosted agents](/agents/long-running-agent-resilience): the resilient task primitives, the resilient Responses options, and the streaming registry. For the concepts behind these APIs, see [Resilience for long-running hosted agents](/agents/long-running-agent-resilience). For task-oriented walkthroughs, see the [how-to guides](/agents/recover-long-running-work).
...
- [Stream long-running agent output with reconnect](/agents/stream-with-reconnect)
