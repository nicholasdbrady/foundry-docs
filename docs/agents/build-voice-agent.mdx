...
For background on hosted agents and the available protocols, see [What are hosted agents?](/agents/hosted-agents). For general container packaging and deployment steps, see [Deploy a hosted agent](/agents/deploy-hosted-agent).
...
For request/response style invocations, continue to use the HTTP [`/invocations`](/agents/deploy-hosted-agent#protocol-libraries) or `/responses` routes. A single container can expose all three protocols at the same time.
...
For the full deployment flow (build, push, RBAC, polling for status), see [Deploy a hosted agent](/agents/deploy-hosted-agent).
...
- **Tools.** Access Foundry-managed tools (Web Search, File Search, Code Interpreter, Azure AI Search, custom MCP servers, A2A) through the Toolbox MCP endpoint in your Foundry project. See [Curate intent-based toolbox in Foundry](/toolboxes/toolbox).

## Observability

The `/invocations_ws` upgrade and per-connection lifecycle should be instrumented with distributed trace spans, following the [OpenTelemetry GenAI conventions](/observability/trace-agent-concept) used by the HTTP `/invocations` protocol.
