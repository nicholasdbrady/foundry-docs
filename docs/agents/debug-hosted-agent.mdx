...
- For deployment and log issues, a deployed hosted agent. To deploy one, see [Deploy a hosted agent](/agents/deploy-hosted-agent).
...
For a structured health report, run `azd ai agent doctor`. For more information, see [Diagnose a project with agent doctor](/agents/agent-doctor).
...
<Tabs>
  ...
  <Tab title=".NET">
    ...
    Reference: [ReadyToRun deployment](https://learn.microsoft.com/dotnet/core/deploying/ready-to-run)
...
Reference: [Hosted agent container requirements](/agents/deploy-hosted-agent#container-requirements)
...
[Monitor hosted agent logs](/agents/monitor-hosted-agent-logs).
...
[Improve Azure OpenAI performance](/models/latency#improve-performance).
...
For the full resolution order and when each source wins, see [Understand azd project context](/developer-tools-and-integrations/cli-project-context).
...
- [Monitor hosted agent logs with the Azure Developer CLI](/agents/monitor-hosted-agent-logs) for deeper log inspection options.
- [Test a hosted agent](/agents/test-hosted-agent) to prevent issues with structured testing.
- [Diagnose a project with agent doctor](/agents/agent-doctor) for a structured project health report.
