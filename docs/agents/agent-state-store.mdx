...
Your agent explicitly writes and updates state-store items. Unlike [sessions and conversations](/agents/hosted-agents#sessions-conversations-and-the-state-store), the platform doesn't populate the store automatically. The store supports per-user isolation, tags, optimistic concurrency, and configurable expiration.
...
A hosted agent's compute is ephemeral. When the container restarts or the platform evicts it after an idle period, your agent loses anything it writes to local disk outside a [session](/agents/hosted-agents#sessions-conversations-and-the-state-store). The state store holds the state that must outlive the container.
...
Long-running agents can keep progress markers and bulk checkpoints in separate items in the same state store. For a recovery pattern, see [Manage state for long-running agents](/agents/manage-task-state).
...
- [Hosted agents in Foundry Agent Service](/agents/hosted-agents#sessions-conversations-and-the-state-store)
- [Hosted agent runtime contract](/agents/hosted-agent-contract#platform-request-headers-container-protocol-200)
