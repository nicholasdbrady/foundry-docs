...
- For remote invocation, a deployed hosted agent. To deploy one, see [Deploy a hosted agent](/agents/deploy-hosted-agent).
- For local invocation, a running local agent. To start one, see [Run a hosted agent locally with the Azure Developer CLI](/agents/run-hosted-agent-locally).
...
Use the playground to test a voice agent, as described in the [voice agent quickstart](/get-started/prompt-voice-agent#test-the-voice-agent).
...
For the full pattern, including how sessions, files, and monitor commands use the same flags, see [Pass isolation keys to a hosted agent](/agents/pass-isolation-keys).
...
- [Isolate hosted agent sessions per user](/agents/isolate-sessions-per-user) for per-user isolation in multi-tenant agents.
- [Run a hosted agent locally with the Azure Developer CLI](/agents/run-hosted-agent-locally) to start your agent for local development.
- [Monitor hosted agent logs with the Azure Developer CLI](/agents/monitor-hosted-agent-logs) to inspect deployed agent behavior.
