...
*Steering* lets you redirect a [long-running hosted agent](/agents/long-running-agent-resilience) while it's still working. A new input queues behind the active turn and the running handler cooperatively winds down. This approach avoids rejecting the new turn or racing two turns at once.
...
- [Deploy a steerable agent](/agents/deploy-steerable-agent)
