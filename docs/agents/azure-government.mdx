...
The following table lists tool availability for Foundry Agent Service in Azure Government. For details on each tool, see the [Foundry tool catalog](/get-started/tool-catalog).
...
Azure Government supports publishing agents. Each published agent gets a stable managed endpoint and a Microsoft Entra identity. You can register the agent with the Entra Agent Registry for discovery within your tenant. Publishing to Microsoft Teams and Microsoft 365 Copilot isn't supported. For more information, see [Publish an agent](/agents/publish-copilot).
...
- [Foundry tool catalog](/get-started/tool-catalog) — Detailed information on all tools
