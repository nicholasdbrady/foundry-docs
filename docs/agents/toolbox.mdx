...
| [MCP tool](/tools-and-knowledge/model-context-protocol) | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [Web Search tool](/tools-and-knowledge/web-search) | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [Azure AI Search tool](/agents/ai-search) | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [Code Interpreter tool](/agents/code-interpreter) | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [File Search tool](/agents/file-search) | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [OpenAPI tool](/tools-and-knowledge/openapi) | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | No |
| [Agent-to-Agent (A2A) tool](/tools-and-knowledge/agent-to-agent) | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | No |
| [Fabric IQ tool](/tools-and-knowledge/fabric-iq) | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| Guardrail (RAI policy) | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [Skill references](/tools-and-knowledge/skills) | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | No |
| [Tool Search tool](/tools-and-knowledge/tool-search) | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [Work IQ tool](/tools-and-knowledge/work-iq) | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ |
| [Browser Automation tool](/tools-and-knowledge/browser-automation) | ✔️ | ✔️ | ✔️ | ✔️ | ✔️ | No |

## Prerequisites

- An active [Microsoft Foundry project](../../../how-to/create-projects).
...
3. Author a toolbox YAML that references one or more existing connections by name. The YAML never embeds credentials:
   ...
   At least one of `connections`, `skills`, or `tools` must be non-empty. Skill references must point to skills that already exist in the same Foundry project; see [Use skills in Foundry](/tools-and-knowledge/skills) to create them with `azd ai skill create`. For end-to-end Tool Search setup details, see [Use tool search](/tools-and-knowledge/tool-search).
...
### [Model Context Protocol (MCP)](/tools-and-knowledge/model-context-protocol)
...
- See the [management section](/tools-and-knowledge/web-search#administrator-control-for-the-web-search-tool) for information about how Azure admins can manage access to use of web search.
...
The returned file `id` is the value you supply as `<FILE_ID>` in the tool configuration. Files are mounted in the sandbox at `/mnt/data/{file-id}-{original-filename}`. See [Code Interpreter](/agents/code-interpreter) for additional upload examples and language-specific samples.
...
Use the file name returned from Step 1 to download the file via the [File API download endpoint](https://learn.microsoft.com/rest/api/microsoft-foundry/azureopenai/files).
...
The resulting vector store ID is the value you supply as `<VECTOR_STORE_ID>`. See [File Search](/agents/file-search) for full examples in each language.
...
### [OpenAPI](/tools-and-knowledge/openapi)
...
### [Agent-to-Agent (A2A)](/tools-and-knowledge/agent-to-agent)
...
### [Fabric IQ](/tools-and-knowledge/fabric-iq)
...
For `server_url` patterns by Fabric item type, see [Find your Fabric IQ server details](/tools-and-knowledge/fabric-iq#find-your-fabric-iq-server-details).
...
### [Tool Search](/tools-and-knowledge/tool-search)
...
In Foundry Toolkit for Visual Studio Code, select **Tool search** on the **Build a Custom Toolbox** tab when you create or edit a toolbox. For more information, see [Enable tool search in a toolbox](/tools-and-knowledge/tool-search#enable-tool-search).

### [Work IQ](/tools-and-knowledge/work-iq)
...
### [Browser Automation](/tools-and-knowledge/browser-automation)
...
Attach [skills](/tools-and-knowledge/skills) to a toolbox version to make them available to agents through the toolbox MCP endpoint. Each skill reference specifies the skill name and an optional version. Omit `version` to use the skill's `default_version`; pin a `version` string to use an immutable snapshot.
...
For full setup instructions, usage examples, and limitations, see [Reminder tool for self-scheduling agents](/agents/reminder-tool).

## Virtual network support

When your Foundry project uses [network isolation (private link)](../../../how-to/configure-private-link), not all toolbox tool types are supported. The following table shows the support status for each tool type and how traffic flows in a network-isolated environment.

| Tool type | VNet support | Traffic flow |
| --------- | ------------ | ------------ |
| [MCP](/tools-and-knowledge/model-context-protocol) | ✅ Supported | Through your VNet subnet |
| [Azure AI Search](/agents/ai-search) | ✅ Supported | Through private endpoint |
| [Code Interpreter](/agents/code-interpreter) | ✅ Supported | Microsoft backbone network |
| [Web Search](/tools-and-knowledge/web-search) | ✅ Supported | Public endpoint |
| [OpenAPI](/tools-and-knowledge/openapi) | ✅ Supported | Depends on target API network configuration |
| [File Search](/agents/file-search) | ❌ Not supported | Not yet available |
| [Agent-to-Agent (A2A)](/tools-and-knowledge/agent-to-agent) | ✅ Supported | Through private endpoint |
| [Fabric IQ](/tools-and-knowledge/fabric-iq) | ❌ Not supported | Not yet available |
| [Work IQ](/tools-and-knowledge/work-iq) | ❌ Not supported | Not yet available |
| [Browser Automation](/tools-and-knowledge/browser-automation) | ❌ Not supported | Not yet available |

For full network isolation setup instructions, including VNet injection for the agent client, DNS configuration, and private endpoint requirements, see [Configure network isolation for Microsoft Foundry](../../../how-to/configure-private-link).
...
Before deploying a toolbox, verify that your target region supports the tool types you plan to use. For the full compatibility tables, see [Tool support by region and model](/agents/limits-quotas-regions#tool-support-by-region-and-model).

## Related content

- [Connect agents to Model Context Protocol servers](/tools-and-knowledge/model-context-protocol)
- [Add MCP server authentication](/tools-and-knowledge/mcp-authentication)
- [Web search tool](/tools-and-knowledge/web-search)
- [Azure AI Search tool](/agents/ai-search)
- [Guardrails overview](../../../guardrails/guardrails-overview)
- [Manage skills](/tools-and-knowledge/skills)
- [Deploy a Hosted agent](/agents/deploy-hosted-agent)
- [Add a connection to your project](../../../how-to/connections-add)
