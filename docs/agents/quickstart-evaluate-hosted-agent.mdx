...
* **Python SDK**: [Python 3.9 or later](https://www.python.org/downloads/), and the Azure CLI signed in with `az login` so that `DefaultAzureCredential` can authenticate. For installation, see [Install the Azure CLI](https://learn.microsoft.com/cli/azure/install-azure-cli).
