...
* A deployed, invokable hosted agent from [Deploy your first hosted agent](/agents/quickstart-hosted-agent). For the Azure Developer CLI path, you also need the `azd` project directory you created in that quickstart.
...
* **Python SDK**: [Python 3.10 or later](https://www.python.org/downloads/), and the Azure CLI signed in with `az login` so that `DefaultAzureCredential` can authenticate. For installation, see [Install the Azure CLI](https://learn.microsoft.com/cli/azure/install-azure-cli).
...
<Frame>
<img src="/images/agent-playground.png" alt="Screenshot of the agent Playground tab in the Foundry portal, showing the Setup panel with agent info on the left and a chat window for sending test prompts on the right." />
</Frame>
...
<Frame>
<img src="/images/agent-evaluation-create.png" alt="Screenshot of the Create new evaluation wizard in the Foundry portal with the Criteria step expanded, showing the Intent Resolution and Task Adherence agent evaluators selected and the dataset field mapping on the right." />
</Frame>
...
<Frame>
<img src="/images/agent-evaluation-submit.png" alt="Screenshot of the Review step of the Create new evaluation wizard in the Foundry portal, showing the evaluation name field and a Summary panel with the agent target, scope, dataset, and evaluators, plus the Submit button." />
</Frame>
...
To remove the hosted agent and the Azure resources you created, follow the cleanup steps in [Deploy your first hosted agent](/agents/quickstart-hosted-agent#clean-up-resources).
...
<Card title="Optimize a hosted agent" icon="arrow-right" href="../../agents/quickstarts/quickstart-optimize-hosted-agent.md" />
Continue improving your evaluation workflow:

- [Set up continuous and scheduled evaluations](/agents/how-to-monitor-agents-dashboard) to track your agent's quality in production.

## Related content

* [Evaluate your AI agents](/evaluation/evaluate-agent)
* [Run batch evaluations from the SDK](/evaluation/cloud-evaluation)
* [Generate a synthetic evaluation dataset](/evaluation/evaluation-dataset-synthetic) to create test queries and evaluators automatically.
