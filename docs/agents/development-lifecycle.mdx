...
- Familiarity with the [Agents playground](/developer-tools-and-integrations/concept-playgrounds)
- For code development: Familiarity with the [development environment setup](/developer-tools-and-integrations/install-cli-sdk)
...
1. **Debug with tracing**: Use tracing to confirm tool calls, latency, and end-to-end behavior. For details, see [Agent tracing overview](/observability/trace-agent-concept).
1. **Evaluate quality and safety**: Run repeatable evaluations to catch regressions before publishing. For conceptual guidance, see [Agent evaluators](/evaluation/agent-evaluators).
1. **Optimize hosted agents (preview)**: Automatically improve your hosted agent's instructions and discover skills using the [agent optimizer](/agents/agent-optimizer-overview).
1. **Publish and integrate**: Publish a stable endpoint and integrate it into your application. For steps, see [Agent applications in Microsoft Foundry](/agents/agent-applications).
1. **Monitor and iterate**: Monitor performance and quality in production, then update and republish as needed. For guidance, see [Monitor agents](/agents/how-to-monitor-agents-dashboard).

## Agent types in Microsoft Foundry

There are three types of agents:

- **Prompt-based**: A prompt-based agent is a declaratively defined single agent that combines a Foundry model, instructions, tools, and natural language prompts to drive behavior. Extend it by attaching tools for knowledge and memory. Edit, version, test, evaluate, monitor, and publish prompt-based agents from the [Agents playground](/developer-tools-and-integrations/concept-playgrounds) in the Foundry portal.

- **Voice-based prompt agents**: A voice-based prompt agent is a prompt-based agent optimized for real-time spoken interactions. Configure the model, instructions, audio settings, optional greeting, and tools, then connect to it over a WebSocket with Voice Live. Foundry manages the runtime, tool execution, and lifecycle while you focus on the spoken experience. For details, see [Quickstart: Create a voice-based prompt agent](/get-started/prompt-voice-agent).
...
Create prompt-based and voice-based prompt agents in the Foundry portal or your own development environment by using the CLI, SDK, or REST API. For more information, see the [quickstart](/get-started/get-started-code).
...
To save an agent with a tool attached, you must successfully configure the tool. Reuse configured tools across agents. For information about how to add tools for agents using a Toolbox and what's available, see [Toolbox Overview](/get-started/toolbox-overview).
...
For more information, see [Agent tracing overview](/observability/trace-agent-concept).
...
- For the key evaluation dimensions for agents, see [Agent evaluators](/evaluation/agent-evaluators).
- For a code-first workflow you can automate, see [Evaluate your AI agents](/evaluation/evaluate-agent).
...
For guidance, see [Trace agents](/observability/trace-agent-concept).
...
For details, see [Agent identity concepts in Microsoft Foundry](/agents/agent-identity).
...
- Before publishing, confirm that the agent identity and tool connections in the published agent application have only the access they need. For details, see [Agent identity concepts in Microsoft Foundry](/agents/agent-identity).

### Publish your agent

After you create an agent version that you're happy with, [publish it as an agent application](/agents/agent-applications). You get a stable endpoint that you can open and test in the browser, share with others, or embed in your existing applications. You and your collaborators can validate performance and identify what needs refinement. Make any necessary updates and republish a new version at any time.
...
- [Agent runtime components](/agents/runtime-components)
...
- [Monitor agents](/agents/how-to-monitor-agents-dashboard)
