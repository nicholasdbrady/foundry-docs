...
- A Foundry project with a deployed hosted agent. To create one, see [Deploy a hosted agent](/agents/deploy-hosted-agent).
...
<Tabs>
  <Tab title="Application Insights">
    ...
    **`APPLICATIONINSIGHTS_CONNECTION_STRING`**: The connection string for the linked Application Insights resource. Foundry sets this value automatically. It's reserved and can't be overridden in `agent.yaml`.
    ...
    Telemetry appears in the linked Application Insights resource under **Investigate** > **Transaction search** or **Investigate** > **Performance**. For analysis guidance, see [Enable tracing in your project](/observability/trace-agent-concept).

  </Tab>
  <Tab title="OTLP exporter">
...
1. Reference the connection field from `agent.yaml` with a placeholder:
   ...
   Foundry resolves the placeholder when the session sandbox starts and injects the value as a plain environment variable. For more information, see [Reference project connections in environment variables](/agents/deploy-hosted-agent#reference-project-connections-in-environment-variables).
