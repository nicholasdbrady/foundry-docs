...
If you use a coding agent like GitHub Copilot, the [Microsoft Foundry Skill](/get-started/use-microsoft-foundry-skill) can help tailor the pipeline template, fill in project-specific values, and validate deployment after changes.
...
| GitHub OIDC configuration | Configure a Microsoft Entra application or federated credential that allows your GitHub workflow to sign in to Azure with OIDC. For more information, see [Use the Azure login action with OpenID Connect](https://learn.microsoft.com/azure/developer/github/connect-from-azure#use-the-azure-login-action-with-openid-connect). |
...
* [Use OpenID Connect with Azure Login for GitHub Actions](https://learn.microsoft.com/azure/developer/github/connect-from-azure)
