...
The entire process runs in the cloud. Start it with `azd ai agent optimize` (requires the [azd CLI extension](/agents/quickstart-optimize-hosted-agent#prerequisites)). The run takes 5 to 20 minutes depending on dataset size.
...
For the best results, generate a dataset tailored to your agent with `azd ai agent eval generate` before running optimization. The optimizer auto-detects the generated `eval.yaml`. For details, see [Create an evaluation dataset](/agents/create-optimizer-dataset).
...
Your agent always works with or without optimization. You don't need feature flags or conditional logic. Call `load_config()` and use the values it returns. For the full resolution order and implementation details, see [Make your agent optimizer-ready](/agents/make-agent-optimizer-ready#configuration-resolution-order).
...
- [Create an evaluation dataset](/agents/create-optimizer-dataset)
