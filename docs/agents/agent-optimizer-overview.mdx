...
For the end-to-end portal experience, see [Quickstart: Optimize a prompt agent](/agents/quickstart-optimize-prompt-agent).
...
For the end-to-end hosted-agent experience, see [Quickstart: Optimize a hosted agent](/agents/quickstart-optimize-hosted-agent).
...
1. **Run the optimizer.** Choose the available optimization targets and models, and then generate and evaluate candidates. For prompt agents, review the [cost estimate](/agents/agent-optimizer-costs#pre-run-cost-estimate-for-prompt-agents) in the portal before you submit the run.
1. **Review the results.** Compare candidate scores against your baseline, and then pick the best candidate. When available, review the [post-run measured token usage](/agents/agent-optimizer-costs#post-run-measured-token-usage). See [Understand optimization results](#understand-optimization-results).
...
Hosted agents require optimizer-ready code integration. Foundry Toolkit automatically scaffolds this integration when you start an optimization. For the Azure Developer CLI workflow, add the optimization package and a baseline configuration before you start. See [Make your agent optimizer-ready](/agents/make-agent-optimizer-ready). Prompt agents don't require this code integration.
...
For hosted agents, after you make your agent [optimizer-ready](/agents/make-agent-optimizer-ready), no further code changes are needed between runs. `load_config()` returns your baseline normally and supplies optimized configuration during a run without feature flags or conditional logic.
...
{/* 
  <img src="/images/agent-optimizer-architecture.svg" alt="Diagram showing how the agent optimizer evaluates an agent baseline, generates candidates, and ranks them." />
*/}
...
For hosted-agent baseline inputs, see [Make your agent optimizer-ready](/agents/make-agent-optimizer-ready). To run and configure hosted-agent targets, see [Optimize agent instructions, skills, tools, and models](/agents/optimize-agent-targets).
...
- Hosted-agent optimization is available in all regions where [hosted agents are available](/agents/hosted-agents#region-availability), except Norway East.
- Hosted-agent optimization requires the [Responses protocol](/agents/hosted-agents#protocols-responses-invocations-and-invocations-websocket).

## Related content

- [Quickstart: Create a prompt agent](/get-started/prompt-agent)
- [Quickstart: Optimize a prompt agent](/agents/quickstart-optimize-prompt-agent)
