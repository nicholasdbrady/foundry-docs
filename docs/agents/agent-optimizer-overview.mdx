...
1. **Prepare your agent.** Add the optimization package and a baseline configuration so the optimizer can read and improve your agent. See [Make your agent optimizer-ready](/agents/make-agent-optimizer-ready).
1. **Create an evaluation dataset.** Define the tasks and criteria the optimizer scores against. See [Create an evaluation dataset and evaluators](/agents/create-optimizer-dataset).
1. **Run the optimizer.** Start an optimization run and choose which targets and models to explore. See [Optimize agent instructions, skills, tools, and models](/agents/optimize-agent-targets).
...
1. **Apply and deploy.** Promote the winning candidate to your agent's configuration and redeploy. See [Deploy the winner](/agents/optimize-agent-targets#deploy-the-winner).

To try the full workflow with a sample agent, start with the [Quickstart: Optimize a hosted agent](/agents/quickstart-optimize-hosted-agent).
...
The entire process runs in the cloud and takes 5 to 20 minutes, depending on dataset size. After you make your agent [optimizer-ready](/agents/make-agent-optimizer-ready), no further code changes are needed between runs: `load_config()` returns your baseline normally and supplies optimized configuration automatically during and after a run - no feature flags or conditional logic.
...
{/* 
  <img src="/images/agent-optimizer-architecture.svg" alt="Diagram showing how the agent optimizer interacts with your hosted agent. The agent loads configuration at startup, and the agent optimizer evaluates, generates candidates, and ranks them." />
*/}
...
To set up these baseline inputs, see [Make your agent optimizer-ready](/agents/make-agent-optimizer-ready). To run and configure each target, see [Optimize agent instructions, skills, tools, and models](/agents/optimize-agent-targets).
...
You specify these models in `eval.yaml` or with CLI flags, and `optimization_model` is required. For configuration steps, see [Choose the eval and optimization models](/agents/optimize-agent-targets#choose-the-eval-and-optimization-models).
...
- The agent optimizer is available in all regions where [hosted agents are available](/agents/hosted-agents#region-availability), except Norway East.
- The agent optimizer is supported for hosted agents that use the [Responses protocol](/agents/hosted-agents#protocols-responses-invocations-and-invocations-websocket).
