...
Voice-based agents support real-time spoken conversations. Availability depends on the region, selected model, audio features, channel, and enabled preview features. For model configuration, see [Configure a voice agent](/agents/configure-voice-agent). For publishing options, see [Publish and connect voice agent channels](/agents/voice-agent-channels-publish).
...
The following table shows regional support for the [Responses API](/models/responses), agents, and voice-based agents (preview). It also shows support for private Class A IP address ranges.
...
Some Azure OpenAI models aren't available in every region. For details, see [Region availability for Foundry Models sold by Azure](/models/models-sold-directly-by-azure-region-availability).
...
For more information, see [Use a virtual network with Foundry Agent Service](../how-to/virtual-networks).
...
Foundry Agent Service is also available in Azure Government (US Gov Virginia and US Gov Arizona) with a subset of agent types and tools. For the full list of supported features, see [Foundry Agent Service feature availability in Azure Government](/agents/azure-government).
...
- Review [Provisioned throughput](/models/provisioned-throughput) and [Spillover traffic management](/models/spillover-traffic-management).
...
- Review [Azure OpenAI quotas and limits](/models/quotas-limits) for your deployment's tokens-per-minute and requests-per-minute caps.
...
[Capability settings](../../how-to/configure-capability-settings) tell Agent Service where to store and process file uploads, vector stores, and conversation history. To store agent data in your own resources, see [Standard agent setup](standard-agent-setup).
...
The Agent Service limits in this table are fixed and apply uniformly across all subscription types. Rate limiting for model calls is applied at the model deployment level; see [Azure OpenAI quotas and limits](/models/quotas-limits) for model-specific rate limits.
...
For file search scenarios, see [Vector stores for file search](/toolboxes/vector-stores) for guidance on managing vector store growth.
...
- **Monitor usage trends.** Track agent activity by using [Foundry Agent Service metrics](/agents/how-to-monitor-agents-dashboard) to identify growth before you hit limits.
...
To view or request more model quota, see [Manage and increase quotas for resources with Microsoft Foundry (Foundry projects)](/models/quota).

## Request a limit increase

The limits in this article are default values for Foundry Agent Service. If your workload requires higher limits:

- **Model quotas.** You can request increases for model deployment quotas. See [Manage and increase quotas for resources with Microsoft Foundry](/models/quota).
- **Hosted agent session quotas.** To request a higher concurrent session limit, [create an Azure support request](https://learn.microsoft.com/azure/azure-portal/supportability/how-to-create-azure-support-request). Specify the subscription, region, and expected number of concurrent sessions.
...
- [Threads, runs, and messages in Foundry Agent Service](/agents/runtime-components)
- [Configure agent capability settings](../../how-to/configure-capability-settings)
- [Standard agent setup](standard-agent-setup)
- [Tool support by region and model](#tool-support-by-region-and-model)
- [Vector stores for file search](/toolboxes/vector-stores)
- [Monitor Foundry Agent Service](/agents/how-to-monitor-agents-dashboard)
- [Azure OpenAI quotas and limits](/models/quotas-limits)
- [Manage and increase quotas for resources with Microsoft Foundry](/models/quota)
