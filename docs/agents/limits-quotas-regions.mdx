...
Foundry agent service is only available for Foundry projects created in regions that support the [Azure OpenAI Responses API](/models/responses#supported-regions). Your Foundry project must be in one of these regions to use Agent Service. Some Azure OpenAI models may not be available in the same regions. See [Region availability for Foundry Models sold by Azure](/models/models-sold-directly-by-azure-region-availability) for details. 

<Info>

Not all tools are available in every region. For example, file search isn't available in Italy North and Brazil South. For the full tool-by-region matrix, see [Tool support by region and model](/agents/tool-best-practice#tool-support-by-region-and-model).
...
- Check the [tool support by region and model](/agents/tool-best-practice#tool-support-by-region-and-model) table to confirm availability before you deploy.
...
For file search scenarios, see [Vector stores for file search](/tools-and-knowledge/vector-stores) for guidance on managing vector store growth.
...
- [Tool support by region and model](/agents/tool-best-practice#tool-support-by-region-and-model)
- [Vector stores for file search](/tools-and-knowledge/vector-stores)
