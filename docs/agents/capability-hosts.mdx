...
To declare the Azure resources that Foundry Agent Service uses for agent state, vector data, and files, see [Configure agent capability settings](../../how-to/configure-capability-settings). This article is retained for reference.
...
Voice-based agents in Foundry can store transcripts, event timelines, and audio when you [enable conversation storage](/agents/configure-voice-agent#persist-conversations). Stored conversations and Microsoft-managed audio are retained for 60 days.

Voice tracing is separate. Trace retention follows the settings of the Application Insights resource connected to your project. For more information, see [Trace data and storage](/observability/trace-data#where-data-is-stored).

### Bring-your-own resources
When you create capability hosts at both the account and project levels, your Azure resources store and process agent data. This is **standard agent setup**. For securing your agent service, see [Set up private networking for Foundry Agent Service](../how-to/virtual-networks).
...
To learn more about standard agent setup, see [Built-in enterprise readiness with standard agent setup](../concepts/standard-agent-setup).
...
Reference: [Foundry account management REST API](https://learn.microsoft.com/rest/api/microsoftfoundry/accountmanagement/operation-groups)
...
Reference: [Project Capability Hosts - Create or update](https://learn.microsoft.com/rest/api/microsoftfoundry/accountmanagement/project-capability-hosts/create-or-update)
...
- [Standard agent setup](../concepts/standard-agent-setup)
