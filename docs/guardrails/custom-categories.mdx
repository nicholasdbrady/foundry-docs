...
sidebarTitle: "Custom categories in Azure AI Content Safety (preview)"
---
...
<Tip>
Another way to deal with emerging content incidents is to use [Blocklists](https://learn.microsoft.com/azure/ai-services/content-safety/how-to/use-blocklist), but that option only allows exact text matching and no image matching. The Custom categories (rapid) API offers the following advanced capabilities:
...
<Tabs>
  <Tab title="Custom categories (standard) API">
  ...
  <Tab title="Custom categories (rapid) API">
...
The Custom categories APIs support all languages that Content Safety text moderation supports. See [Language support](https://learn.microsoft.com/azure/ai-services/content-safety/language-support).
...
To use these APIs, you must create your Azure AI Content Safety resource in one of the supported regions. For more information, see [Region availability](../overview#region-availability).

## Next step

Follow a how-to guide to use the Azure AI Content Safety APIs to create custom categories.

* [Use custom category (standard) API](../how-to/custom-categories)
* [Use the custom categories (rapid) API](../how-to/custom-categories-rapid)
