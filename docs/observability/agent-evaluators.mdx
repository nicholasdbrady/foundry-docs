...
For AI-assisted evaluators, you can use Azure OpenAI or OpenAI [reasoning models](/foundry-models/reasoning) and non-reasoning models for the LLM judge. For complex evaluation that requires refined reasoning, we recommend `gpt-5-mini` for its balance of performance, cost, and efficiency.
