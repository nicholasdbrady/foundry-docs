...
**Resolution:** Verify that Application Insights is connected to your project. For setup instructions, see [Configure Application Insights for agent tracing](/agent-service/trace-agent-setup). If Application Insights is already configured, wait a few minutes and refresh the page.

### Reviewer can't access the preview web app

**Cause:** The reviewer doesn't have the required role on the Foundry project.

**Resolution:** Assign the **Azure AI User** role to the reviewer on the Foundry project. For instructions, see [Role-based access control in Microsoft Foundry](/security/rbac-foundry).
