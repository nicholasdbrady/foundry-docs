...
- Once you have your Azure subscription, create a Foundry services resource. For more information, see [Create a Foundry Services resource](/setup/multi-service-resource).
* A model deployment. For more information, see [Add and configure models to Foundry services](/models/capabilities/create-model-deployments).
...
<Tabs>
  <Tab title="Azure OpenAI API">
    ...
    If you haven't yet created a content filter, you can do so in [Foundry](https://ai.azure.com/?cid=learnDocs). See [Content filtering](https://learn.microsoft.com/azure/ai-foundry/openai/how-to/content-filters#create-a-content-filter-in-azure-ai-foundry).
...
It can take around 5 minutes for a new term to be added to the blocklist. Test the blocklist after 5 minutes.
    ...
    Now you can test out your deployment that has the blocklist. For instructions on calling the Azure OpenAI endpoints, visit the [Quickstart](https://learn.microsoft.com/azure/ai-foundry/openai/quickstart). 
...
This article refers to the [Microsoft Foundry (new)](../what-is-foundry.md#microsoft-foundry-portals) portal.
    ...
    Go to [Microsoft Foundry](https://ai.azure.com/?cid=learnDocs) and select **Build** in the upper right menu. On the left pane, select **Guardrails**, then go to the **Blocklists** tab. Use the **Create blocklist** button to create your own blocklist. You can add terms manually or upload a .csv file of terms.
...
- Read more about [content filtering categories and severity levels](https://learn.microsoft.com/azure/ai-foundry/openai/concepts/content-filter) with Azure OpenAI in Foundry models. 

- Learn more about red teaming from our: [Introduction to red teaming large language models (LLMs)](https://learn.microsoft.com/azure/ai-foundry/openai/concepts/red-teaming) article.
