...
<Note>
The following upgrade guidance only applies to Standard deployment types. For guidance on updating or migrating provisioned deployment types, review the [model management documentation](/models/catalog/working-with-models).
</Note>
...
* Read [what's new](../../../foundry-classic/openai/whats-new.md) and [models](/models/catalog/models-sold-directly-by-azure) to understand the changes and new features.
* Read the documentation on [Deploy Foundry Models](/models/capabilities/deploy-foundry-models) and [version upgrades](/models/catalog/working-with-models) to understand how to work with model versions.
...
Azure notifies you of upcoming retirements through email to subscription owners and contributors, Azure Service Health alerts, and the upcoming retirement tables in [Model deprecation and retirement for Microsoft Foundry Models](/operate/model-lifecycle-retirement).
