...
sidebarTitle: "How to use vision-enabled chat models"
---
...
- An Azure OpenAI resource with a vision-enabled model deployed (GPT-4o, GPT-4.5, GPT-5, or o-series). See [Create and deploy an Azure OpenAI Service resource](https://learn.microsoft.com/azure/ai-services/openai/how-to/create-resource).
- For Python: The `openai` Python package version 1.0 or later. Install with `pip install openai`.

## Call the Chat Completion APIs

The following command shows the most basic way to use a vision-enabled chat model with code. If this is your first time using these models programmatically, we recommend starting with our [Chat with images quickstart](/models/capabilities/gpt-v-quickstart). 
...
</Warning>    > Remember to set a `"max_tokens"` or `max_completion_tokens` value, or the return output will be cut off. For o-series reasoning models, use `max_completion_tokens` instead of `max_tokens`.
...
* [Vision-enabled chats quickstart](/models/capabilities/gpt-v-quickstart)
