...
If you're new to provisioned throughput, start with [What is provisioned throughput for Foundry Models?](/models/provisioned-throughput). When you're ready to create your deployment, see [Quickstart: Create a provisioned throughput deployment](/models/provisioned-quickstart).

## Prerequisites

- Familiarity with the concepts in [What is provisioned throughput for Foundry Models?](/models/provisioned-throughput).
...
For older models (before GPT-4o), the [request/call shape distribution](/models/provisioned-throughput#ptu-sizing) affects capacity consumption: a small number of large calls can consume significantly more capacity than many small calls with the same average token count. For GPT-4o and later models, TPM per PTU is set for input and output tokens separately, so this tiering effect doesn't apply.
...
For all deployments, cached tokens are deducted 100% from the utilization calculation, meaning repeated prompt tokens don't consume PTU capacity. See [Prompt caching](/models/prompt-caching) for more information.
...
Long-context requests that exceed 128K prompt tokens aren't supported for `gpt-5.4`, `gpt-4.1`, `gpt-4.1-mini`, and `gpt-4.1-nano`. The system routes these requests to [spillover deployments](/models/spillover-traffic-management), if available. Otherwise, the requests return an error.
...
With GPT-6 Astra, PTUs support [prompt cache breakpoints](/models/prompt-caching#configure-prompt-cache-breakpoints), enabling higher cache hit rates and a more deterministic caching experience. Cache reads and writes significantly increase cache hit rates, especially for agentic workloads. As cache reuse grows, GPT-6 Astra uses normalized tokens to align PTU throughput accounting with Global Standard pay-as-you-go pricing.
...
Before using Azure Monitor to monitor a deployed image model, review [Limitations and known issues for image model metrics](/models/provisioned-get-started#limitations-and-known-issues-for-image-model-metrics).
...
- [What is provisioned throughput for Foundry Models?](/models/provisioned-throughput)
- [Quickstart: Create a provisioned throughput deployment](/models/provisioned-quickstart)
- [Provisioned throughput billing and cost management](/models/provisioned-throughput-billing)
- [Operate provisioned deployments in production](/models/provisioned-get-started)
