...
For all deployments, cached tokens are deducted 100% from the utilization calculation, so repeated prompt tokens don't consume PTU capacity. See [Prompt caching](/models/prompt-caching) for more information.
...
GPT-6 Astra and GPT-6 Sol support [prompt cache breakpoints](/models/prompt-caching#configure-prompt-cache-breakpoints) for explicit control over prompt caching. Cache reads and writes are included in normalized-token accounting.
