...
For GPT-6 Astra, tool calling requires the Responses API. The model doesn't support the `none` reasoning effort level, so the Chat Completions workaround described in the next section doesn't apply. If you use tools with Chat Completions, follow the [Responses API migration guide](https://learn.microsoft.com/azure/developer/ai/how-to/azure-openai-to-responses).
