...
|---|---|---|---|
| Standard pay-per-token | [Foundry Models sold by Azure](/models/models-sold-directly-by-azure) | Per input and output token | Lowest-friction path to get started; bursty traffic on hosted models with no capacity planning. |
|[Provisioned throughput](/models/provisioned-throughput) | Foundry Models sold by Azure | Reserved throughput units | Predictable, sustained load on select Foundry Models sold by Azure with consistent latency. |
...
Both authentication options work on both endpoint routes. For end-to-end client code samples (OpenAI SDK with Entra ID or API key), see [Send a test request](/models/deploy-models-managed#send-a-test-request).
...
{/* For the current list of regions and accelerator families per region, see the Foundry [general availability matrix](general-availability). */}
...
For details on viewing usage, attributing cost to a project, and requesting quota, see [Plan and manage costs for Microsoft Foundry](/models/manage-costs) and [Manage and increase quotas](/models/quota).
...
- [Role-based access control for Microsoft Foundry](rbac-foundry)
- [Plan and manage costs for Microsoft Foundry](/models/manage-costs)
- [Manage and increase quotas](/models/quota)
- [Authentication and authorization in Foundry](authentication-authorization-foundry)
