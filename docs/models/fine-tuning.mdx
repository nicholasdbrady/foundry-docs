...
- Read the [guide on when to use Foundry fine-tuning](../openai/concepts/fine-tuning-considerations).
...
The training and validation data that you use *must* be formatted as a JSON Lines (JSONL) document. It must also be formatted in the conversational format that the [Chat Completions](../openai/how-to/chatgpt) API uses.
...
If you don't have an existing dataset prepared, you can use the [data generation](../fine-tuning/data-generation) capabilities to create a new one.
...
1. Sign in to [Foundry](https://ai.azure.com/) and select your project. If you don't have a project already, first [create a project](../how-to/create-projects).
...
The rest of this article covers steps for the SFT method. For instructions specific to other customization methods, see the [guide for DPO](../openai/how-to/fine-tuning-direct-preference-optimization) and the [guide for RFT](../openai/how-to/reinforcement-fine-tuning).
...
For more information, see the [fine-tuning deployment guide](../openai/how-to/fine-tuning-deploy).
...
- Read the [guide on when to use Azure OpenAI fine-tuning](/models/fine-tuning-considerations).
- You need an Azure subscription. [Create one for free](https://azure.microsoft.com/pricing/purchase-options/azure-account?cid=msft_learn).
...
The training and validation data that you use *must* be formatted as a JSON Lines (JSONL) document. It must also be formatted in the conversational format that the [Chat Completions](/models/chatgpt) API uses.
...
Importing from Azure Blob Storage requires the storage account to have **public network access enabled**. If your organization's policies don't allow public access on storage accounts, use the [local file upload](https://learn.microsoft.com/rest/api/azureopenai/files/upload) method instead.
...
We recommend using the Global Standard tier for the training type, because it offers [cost savings](https://azure.microsoft.com/pricing/details/cognitive-services/openai-service/) and uses global capacity for faster queuing times. However, it does copy data and weights outside the current resource region. If [data residency](https://azure.microsoft.com/explore/global-infrastructure/data-residency/) is a requirement, use a [model](/models/models-sold-directly-by-azure) that supports Standard-tier training.
...
To learn about supported hyperparameters for the other customization methods, see the [guide for direct preference optimization](/models/fine-tuning-direct-preference-optimization) and the [guide for reinforcement fine-tuning](/models/reinforcement-fine-tuning).
...
If you're deploying for further validation, consider deploying for [testing](https://learn.microsoft.com/en-us/azure/foundry-classic/openai/how-to/fine-tune-test) by using a Developer deployment.
...
To learn about cross-region deployment and how to use the deployed model, see [Use your deployed fine-tuned model](/models/fine-tuning-deploy#use-your-deployed-fine-tuned-model).

If you're ready to deploy for production or you have particular data-residency needs, follow the [deployment guide](/models/fine-tuning-deploy).
...
You can delete your customized model by using [Foundry](/models/fine-tuning).
...
- Read the [guide on when to use Foundry fine-tuning](/models/fine-tuning-considerations).
- You need an Azure subscription. [Create one for free](https://azure.microsoft.com/pricing/purchase-options/azure-account?cid=msft_learn).
- You need a Foundry project resource. To create one, sign in to the [Foundry portal](https://ai.azure.com).
...
If you want a step-by-step walkthrough of fine-tuning `gpt-4o-mini-2024-07-18`, see the [Microsoft Foundry fine-tuning tutorial](https://learn.microsoft.com/en-us/azure/foundry-classic/openai/tutorials/fine-tune).
...
If you're fine-tuning a model that supports [global training](/models/models-sold-directly-by-azure), you can specify the training type by using the `extra_body` named argument and using `api-version=2025-04-01-preview`:
...
You can optionally delete training and validation files that you uploaded for training, and result files generated during training, from your Azure OpenAI subscription. You can use [Foundry](/models/fine-tuning) to delete the files.
...
- [Model catalog and regional availability](/models/models-sold-directly-by-azure)
- [Quotas and limits](/models/quotas-limits)
- [View and interpret evaluation results](/evaluation/evaluate-results)
