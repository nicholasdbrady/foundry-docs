...
| **401** | Unauthorized | Invalid API key, expired or misconfigured Microsoft Entra token, wrong token scope or audience, missing RBAC role assignment | Verify your API key or token. For Microsoft Entra ID authentication, use the `https://ai.azure.com/.default` scope for Azure OpenAI endpoints. See [Configure keyless authentication with Microsoft Entra ID](../../foundry-models/how-to/configure-entra-id#troubleshooting). |
| **403** | Forbidden | See [HTTP 403 causes](#http-403-forbidden) below | Depends on the specific cause |
| **404** | Not Found | Incorrect endpoint URL, deployment name doesn't exist, unsupported API version | Verify your endpoint URL, deployment name, and API version in the [REST API reference](../reference). |
| **429** | Too Many Requests | Rate limit exceeded, system capacity throttling, or a temporary rate limit adjustment | Follow the `retry-after-ms` header when present. Review [quotas and limits](/models/quotas-limits) and [429 throttling guidance](/models/quota#understanding-429-throttling-errors-and-what-to-do). |
...
To set up a Foundry private endpoint, see [Configure private link](../../how-to/configure-private-link). For virtual network, firewall, and IP rules, see [Configure Azure AI services virtual networks](https://learn.microsoft.com/azure/ai-services/cognitive-services-virtual-networks).
...
For more information, see [Abuse monitoring](../concepts/abuse-monitoring).

## Related content

- [Azure OpenAI REST API reference](../reference)
- [Quotas and limits](/models/quotas-limits)
- [429 throttling guidance](/models/quota#understanding-429-throttling-errors-and-what-to-do)
- [Content filtering](../concepts/content-filter-severity-levels)
- [Abuse monitoring](../concepts/abuse-monitoring)
