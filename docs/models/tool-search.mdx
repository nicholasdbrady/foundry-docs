...
Tool search in the Responses API is different from [tool search in a Foundry Agent Service toolbox](/toolboxes/tool-search). Responses API tool search defers tool definitions that you declare in a request. Toolbox tool search discovers tools configured in a versioned Foundry toolbox.
