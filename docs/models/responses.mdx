...
Use the Azure OpenAI Responses API to generate stateful, multi-turn responses. It brings together capabilities from chat completions and the Assistants API in one unified experience. Use [multi-agent orchestration](/models/responses-multi-agent) to delegate independent work to parallel subagents, or use [tool search](/models/tool-search) to load tool definitions only when the model needs them. The Responses API also supports the `computer-use-preview` model that powers [Computer use](https://learn.microsoft.com/en-us/azure/foundry-classic/openai/how-to/computer-use).

For complete request and response parameter descriptions, see the [Responses API parameter reference](https://learn.microsoft.com/rest/api/microsoft-foundry/azureopenai/responses).
...
Not every model is available in every supported region. Check the [models page](/models/models-sold-directly-by-azure) for model region availability.
...
Use structured outputs to make a response follow a JSON Schema. In a Responses API request, define the schema in `text.format`. Chat Completions uses `response_format` instead. For SDK and REST examples, supported schema constraints, and limitations, see [Structured outputs](/models/structured-outputs).
...
- A file ID created with the [Files API](https://learn.microsoft.com/rest/api/microsoft-foundry/azureopenai/files)
...
The image generation tool doesn't currently support streaming mode. To stream partial images, call the [image generation API](/models/dall-e) directly outside of the Responses API.
...
For examples of how to use reasoning models with the responses API see the [reasoning models guide](/models/reasoning#reasoning-summary).
...
- [API version lifecycle](../api-version-lifecycle)
- [Responses API request and response parameter reference](https://learn.microsoft.com/rest/api/microsoft-foundry/azureopenai/responses)
