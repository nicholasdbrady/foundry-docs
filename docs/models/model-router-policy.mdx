...
- A Foundry resource in a region that supports model router. To learn more, see [Use model router for Microsoft Foundry](/models/model-router).
...
Model router uses the same built-in Foundry policy that governs other model deployments: **Cognitive Services Deployments should only use approved Registry Models**. To assign or update the policy, follow the steps in [Built-in policy for model deployment](model-deployment-policy). The publisher names and asset IDs that you allow apply to model router selections automatically. No separate policy definition is required.
...
For the full deployment walkthrough that doesn't include policy steps, see [Use model router for Microsoft Foundry](/models/model-router).
...
1. Submit the deployment by using the REST API examples in [Use model router for Microsoft Foundry](/models/model-router#configure-custom-settings-with-the-rest-api).
...
- [Use model router for Microsoft Foundry](/models/model-router)
- [Model router for Microsoft Foundry concepts](/models/model-router)
- [Built-in policy for model deployment](model-deployment-policy)
- [Create custom policy definitions](custom-policy-definition)
