...
Use the [Realtime API via WebSockets](/models/realtime-audio-websockets) if you need to:
...
You can access the GPT real-time models for global deployments in the [East US 2 and Sweden Central regions](/models/models-sold-directly-by-azure-region-availability).
...
If you're a current customer using Preview endpoints, [migrate to the GA Protocol](/models/realtime-audio-preview-api-migration-guide). 
...
For migration details, see [Migration from Preview to GA version of Realtime API](/models/realtime-audio-preview-api-migration-guide#update-webrtc-endpoints).
...
For the complete list of Realtime API events, see the [API reference](../realtime-audio-reference).
...
**Reference:** [RTCPeerConnection](https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection), [Realtime API events](../realtime-audio-reference)
