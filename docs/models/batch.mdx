...
Set expiration for input files to increase the per-resource limit and manage stored files. Batch input file limits don't apply to output files, such as `result.jsonl` and `error.jsonl`. To avoid Files API input file limits, use [Batch with Azure Blob Storage](https://learn.microsoft.com/en-us/azure/foundry-classic/openai/how-to/batch-blob-storage).
