...
The GPT real-time models are available for global deployments in [East US 2 and Sweden Central regions](/models/models-sold-directly-by-azure-region-availability).
...
- [Use the Realtime API via WebRTC](/models/realtime-audio-webrtc) for browser-based applications
- [Use the Realtime API via WebSocket](/models/realtime-audio-websockets) for server-to-server scenarios
- [Azure OpenAI webhooks](/models/webhooks) for webhook configuration details
- [Realtime API reference](../realtime-audio-reference) for complete API documentation
