...
- An Azure OpenAI resource in a region that supports the [Responses API](/models/responses).
- A GPT-5.6 model deployment. Check [model availability](/models/models-sold-directly-by-azure-region-availability) before you create the deployment.
...
Reference: [Azure OpenAI v1 API authentication](../api-version-lifecycle) | [Use the Azure OpenAI Responses API](/models/responses)
...
Reference: [Azure OpenAI v1 API authentication](../api-version-lifecycle)
...
Reference: [Use the Azure OpenAI Responses API](/models/responses)
...
With HTTP, a response completes after every active agent finishes or pauses for a client-executed function call. Run all pending function calls, preserve the output items, and submit their outputs in the next request so the paused agents can continue. For the base tool execution pattern, see [Function calling](/models/function-calling).
...
- [Use the Azure OpenAI Responses API](/models/responses)
- [Use function calling](/models/function-calling)
- [Use the Responses API in WebSocket mode](/models/websockets)
