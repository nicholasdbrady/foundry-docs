...
For server-to-server integrations, see [Use GPT-Live for real-time voice](/models/gpt-live) instead.
...
Save `session.id` if your backend attaches a [sideband WebSocket](/models/gpt-live#observe-a-session-with-a-sideband-websocket) to observe or steer the session.
...
- All other session events—`session.update`, the `session.instructions.append` / `session.thinking.append` / `session.commentary.append` context events, delegation events, transcripts, usage, and errors—use the same schema as WebSocket. See the [GPT-Live event API reference](../gpt-live-reference).

## Related content

- [Use GPT-Live for real-time voice](/models/gpt-live)
