...
sidebarTitle: "Azure OpenAI in Foundry Models Webhooks"
---
...
- Azure CLI installed and signed in. For example: `az --version` and `az login`. See [https://learn.microsoft.com/cli/azure/install-azure-cli](https://learn.microsoft.com/cli/azure/install-azure-cli).
...
Deploy your webhook server using [`az webapp up`](https://learn.microsoft.com/cli/azure/webapp). The command must be run from the folder where the `app.py` code for your application is located.
...
- Create an [App Service plan](https://learn.microsoft.com/azure/app-service/overview-hosting-plans).
- Create a web app.
- Deploy your code.
...
<Info>
The signing secret is only shown once during creation. Secrets can be stored securely using [Azure Key Vault](https://learn.microsoft.com/azure/key-vault/general/overview).
</Info>
