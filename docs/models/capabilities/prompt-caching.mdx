...
When a match is found between the token computations in a prompt and the current content of the prompt cache, it's referred to as a cache hit. Cache hits will show up as [`cached_tokens`](https://learn.microsoft.com/azure/ai-foundry/openai/reference-preview#cached_tokens) under [`prompt_tokens_details`](https://learn.microsoft.com/azure/ai-foundry/openai/reference-preview#properties-for-prompt_tokens_details) in the chat completions response.
...
Feature support of o1-series models varies by model. For more information, see our dedicated [reasoning models guide](/models/capabilities/reasoning).
