...
sidebarTitle: "How to use predicted outputs with Azure OpenAI in Microsoft Foundry Models"
---
...
<Note>
Unlike [prompt caching](/models/capabilities/prompt-caching) which only works when a set minimum number of initial tokens at the beginning of a request are identical, predicted outputs isn't constrained by token location. Even if your response text contains new output that will be returned prior to the predicted output, `accepted_prediction_tokens` can still occur.
</Note>
