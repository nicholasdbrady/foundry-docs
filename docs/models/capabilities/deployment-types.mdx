...
sidebarTitle: "Understanding deployment types in Microsoft Foundry Models"
---
...
<Note>
Not all models support all deployment types. Check [Foundry Models sold directly by Azure](/models/catalog/models-sold-directly-by-azure) for model availability by deployment type and region.
</Note>
<Note>
...
Learn more in the "Model region availability by deployment type" section of [Foundry Models sold directly by Azure](/models/catalog/models-sold-directly-by-azure#foundry-models-sold-directly-by-azure).
...
Customers with high consistent volume might experience greater latency variability. The threshold is set per model. To learn more, see the [Quotas page](/setup/quotas-limits). For applications that require lower latency variance at large workload usage, consider provisioned throughput.

Global Standard supports priority processing (preview) for faster response times on a pay-as-you-go basis. To learn more, see [Priority processing for Foundry models (preview)](/models/capabilities/priority-processing).
...
Customers with high consistent volume might experience greater latency variability. The threshold is set per model. To learn more, see the [quotas and limits page](/setup/quotas-limits). For workloads that require low latency variance at large volume, consider provisioned deployment types.

Data Zone Standard supports priority processing (preview) for faster response times on a pay-as-you-go basis. To learn more, see [Priority processing for Foundry models (preview)](/models/capabilities/priority-processing).
...
Data Zone Batch deployments provide the same functionality as [Global Batch](/models/capabilities/batch), including 50% cost savings and 24-hour turnaround. Traffic is routed only to datacenters within the Microsoft-defined data zone (US or EU).
...
Regional Provisioned deployments allow you to specify the amount of throughput you require in a deployment. The service then allocates the necessary model processing capacity and ensures it's ready for you. Throughput is defined in terms of provisioned throughput units (PTUs), which is a normalized way of representing the throughput for your deployment. Each model-version pair requires different amounts of PTUs to deploy, and provides different amounts of throughput per PTU. Minimum PTU requirements vary by model. For current minimums and available capacity, see [Provisioned throughput concepts](/models/capabilities/provisioned-throughput).
...
|-------|-------|------------|
| Deployment type unavailable | Model doesn't support the selected type | Check [model availability by deployment type](/models/catalog/models-sold-directly-by-azure) |
...
For quota limits by deployment type, see [Foundry Models quotas and limits](/setup/quotas-limits).
...
- [Foundry Models sold directly by Azure](/models/catalog/models-sold-directly-by-azure)
- [Model region availability by deployment type](/models/catalog/models-sold-directly-by-azure#foundry-models-sold-directly-by-azure)
...
- [Global Batch processing](/models/capabilities/batch)
