...
1. Add the `Azure OpenAI Requests` metric.
    ...
    ![A screenshot showing the metrics for a basic spillover example in the Azure portal.](/images/spillover-metrics-menu.png)
...
1. Select **Apply splitting** and apply the `ModelDeploymentName` split and `StatusCode` splits to the `Azure OpenAI Requests` metric. This shows a chart with the `200` (success) and `429` (too many requests) response codes generated for your resource.   
    ...
    ![A screenshot showing the menu for adding splits in the Azure portal.](/images/add-splitting.png)
    
    Be sure to add the model deployments you want to view when applying the `ModelDeploymentName` split.

    ![A screenshot showing the available model filters.](/images/model-filter.png)
    ...
    ![A screenshot showing the metrics for visualizing spillover.](/images/spillover-chart-simplified.png)
    ...
    > ![A screenshot showing the response codes from a provisioned deployment.](/images/spillover-chart-errors.png)
...
![A screenshot showing the spillover split in Azure portal.](/images/spillover-chart.png)
