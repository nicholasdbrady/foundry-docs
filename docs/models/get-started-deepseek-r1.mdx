...
To create a Foundry project that supports deployment for DeepSeek-V4-Pro, follow these steps. You can also create the resources by using [Azure CLI](/get-started/get-started-code) or [infrastructure as code, with Bicep](/get-started/get-started-code).
...
Reasoning might generate longer responses and consume a larger number of tokens. See the [rate limits](/models/quotas-limits) that apply to DeepSeek models. Consider having a retry strategy to handle rate limits. You can also [request increases to the default limits](/models/quotas-limits#request-increases-to-the-default-limits).
...
- **Monitor token usage.** DeepSeek-V4-Pro reasoning content (within `<think>` tags) counts toward your token limit. See [quotas and limits](/models/quotas-limits) for the current rate limits.
