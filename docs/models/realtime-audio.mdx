...
- [Realtime API via WebRTC](/models/realtime-audio-webrtc)
- [Realtime API via SIP](/models/realtime-audio-sip)
- [Realtime API via WebSockets](/models/realtime-audio-websockets)
...
For region-by-region model support, see [Region availability for Foundry Models sold by Azure](/models/models-sold-directly-by-azure-region-availability).
...
- An API key or Microsoft Entra ID credentials for authentication. For production applications, we recommend using [Microsoft Entra ID](https://learn.microsoft.com/en-us/azure/foundry-classic/openai/how-to/managed-identity) for enhanced security.
...
- For steps to deploy and use a GPT realtime model over WebSockets, see [the WebSockets quickstart](/models/realtime-audio-websockets#voice-agent-quickstart).
- Try the [WebRTC via HTML and JavaScript example](/models/realtime-audio-webrtc#step-3-optional-create-a-websocket-observercontroller) to get started with the Realtime API via WebRTC.
...
- For server-to-server setup, see [Use the GPT Realtime API via WebSockets](/models/realtime-audio-websockets).
- For browser-native, low-latency media setup, see [Use the GPT Realtime API via WebRTC](/models/realtime-audio-webrtc).
...
The Realtime API has specific rate limits for audio tokens and concurrent sessions. Before deploying to production, review [Azure OpenAI quotas and limits](/models/quotas-limits) for your deployment type.
...
| 429 Too Many Requests | Rate limit exceeded | Implement exponential backoff retry logic. Check your [quota and limits](/models/quotas-limits). |
...
For more information about quotas, see [Azure OpenAI quotas and limits](/models/quotas-limits).
