...
| **Priority processing** | Pay per token (priority tier rate) | [Defined latency target per model](/models/priority-processing#latency-target) | Latency-sensitive production workloads needing consistent low latency without a long-term commitment |
| **Provisioned** | Per [PTU](#provisioned-throughput-units) per hour (or using [Azure reservations](#azure-reservations)) | [Defined latency target per model](/models/provisioned-throughput-sizing#deployment-parameters-and-throughput-values-by-model) | Mission-critical, high-scale production workloads requiring guaranteed throughput and consistent latency |
...
- **Throughput varies by model**: The tokens per minute (TPM) that a given number of PTUs delivers depends on the model. A heavier model requires more PTUs to serve the same TPM as a lighter one. For per-model PTU-to-TPM ratios, see [Per-model throughput parameters](/models/provisioned-throughput-sizing#deployment-parameters-and-throughput-values-by-model).
- **Minimum deployment sizes apply**: Each model has a minimum PTU count required to create a deployment. Minimums vary by model and are listed in [Deployment parameters and throughput values by model](/models/provisioned-throughput-sizing#deployment-parameters-and-throughput-values-by-model).
...
For step-by-step guidance on creating provisioned deployments and handling capacity constraints, see [Get started with provisioned deployments](/models/provisioned-get-started).
...
For the complete sizing methodology, including formulas, worked examples, and the capacity calculator reference, see [Determine PTU sizing for a workload](/models/provisioned-throughput-sizing).
...
For a full comparison of all Foundry deployment types, including standard, batch, and provisioned, see [Deployment types for Microsoft Foundry Models](/models/deployment-types).

## Supported models

For a full list of Foundry Models that support provisioned throughput, including which deployment types each model supports and regional availability, see [Region availability for Foundry Models sold directly by Azure](/models/models-sold-directly-by-azure-region-availability).
...
Spillover can be configured for all requests on a deployment or controlled on a per-request basis using the `x-ms-spillover-deployment` request header. For configuration steps, see [Manage traffic with spillover for provisioned deployments](/models/spillover-traffic-management).
...
For complete guidance on hourly billing and scaling provisioned deployments, see [Hourly billing](/models/provisioned-throughput-billing#hourly-billing).
...
For complete guidance on sizing, purchasing, and managing reservations, see [Azure Reservations for provisioned throughput](/models/provisioned-throughput-billing#azure-reservations-for-provisioned-throughput).
...
- [PTU costs and billing](/models/provisioned-throughput-billing)
- [Manage traffic with spillover for provisioned deployments](/models/spillover-traffic-management)
...
- [Foundry Models quotas and limits](/models/quotas-limits)
