...
If you're new to provisioned throughput, start with [What is provisioned throughput for Foundry Models?](/models/provisioned-throughput). To estimate the number of PTUs your workload needs, see [Determine PTU sizing for a workload](/models/provisioned-throughput-sizing). When you're ready to create your first deployment, see [Get started with provisioned deployments](/models/provisioned-get-started).
...
- **Model-independent**: Your [PTU quota](/models/provisioned-throughput#quota-and-capacity) is shared across all [supported models](/models/models-sold-directly-by-azure-region-availability) in a region and [deployment type](/models/provisioned-throughput#provisioned-throughput-deployment-types). The same PTU pool can be used to deploy any supported model. You don't buy PTUs for a specific model. PTU quota for each provisioned deployment type appears in the Foundry portal's **Quota** page.
...
- [Determine PTU sizing for a workload](/models/provisioned-throughput-sizing)
- [Get started with provisioned deployments](/models/provisioned-get-started)
- [Foundry Provisioned Throughput reservation documentation](https://aka.ms/oai/docs/ptum-reservations)
- [Performance and latency](/models/latency)
