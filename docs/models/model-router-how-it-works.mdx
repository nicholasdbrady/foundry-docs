...
This article explains the capabilities, routing modes, and best practices that power model router. For supported models and version information, see the [model router overview](/models/model-router). For deployment and usage steps, see [Use model router](/models/model-router).

## Prerequisites

- Familiarity with LLMs and the [Chat Completions API](https://learn.microsoft.com/azure/ai-foundry/openai/how-to/chatgpt)
- Understanding of the [model router overview](/models/model-router)
...
Managed routing doesn't remove the need for evaluation. Compare model router with a meaningful workload baseline, and reevaluate after you change a routing mode or model subset. For evaluation guidance, see [Evaluate model router for your workload](/models/evaluate-model-router).
...
Keeping related requests on the same model can improve behavioral continuity and the opportunity for prompt-cache reuse. Session affinity doesn't inspect provider cache signals, compare the benefit of staying with switching, or guarantee cache hits. For configuration steps, see [Keep Chat Completions requests on the same model](/models/model-router#keep-chat-completions-requests-on-the-same-model-preview).
...
Model router exposes three routing modes that control the cost-quality tradeoff. For the mode descriptions and configuration steps, see the [model router overview](/models/model-router#routing-mode) and the [how-to guide](/models/model-router#optional-change-the-routing-mode).
...
- [Model router overview](/models/model-router)
