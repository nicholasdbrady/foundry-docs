...
- **Standard deployment in Foundry resources** — For Foundry Models, including [Foundry Models sold by Azure](/models/models-sold-directly-by-azure) (also known as Azure Direct Models, or ADM) and [select Models from partners and community](/models/models-from-partners). This option is the preferred and most capable deployment path.
...
|---|---|---|
| **Models** | [ADM models](/models/models-sold-directly-by-azure) (Azure OpenAI + partner models billed through Azure) and select [Models from partners and community](/models/models-from-partners)| Other models in the model catalog from partners and custom models. For example, models from Hugging Face, NVIDIA NIMs, industry models, and Databricks. |
| **Billing** | Token usage or [provisioned throughput units (PTU)](/models/provisioned-throughput) | Hourly per accelerator SKU |
...
All Foundry Models, including [Foundry Models sold by Azure](/models/models-sold-directly-by-azure) and [select Models from partners and community](/models/models-from-partners) use standard deployment. Foundry Models sold by Azure include all Azure OpenAI models and selected models from top providers that are billed through your Azure subscription, covered by Azure service-level agreements, and supported by Microsoft. Select Models from partners and community that use standard deployment include Anthropic models, and specific models from partners like Mistral, Cohere, and Meta.

### Capabilities

Standard deployment supports:

- **Multiple deployment types** — Global Standard, Data Zone Standard, Regional Standard, Provisioned, Batch, and more. Each type controls where data is processed and how you pay. For details, see [Deployment types for Microsoft Foundry Models](/models/deployment-types).
...
- **Provisioned throughput** — Reserve capacity with PTUs for predictable, low-latency performance. For details, see [Provisioned throughput](/models/provisioned-throughput).
...
To get started with deployment, see [Deploy Microsoft Foundry Models in the Foundry portal](/models/deploy-foundry-models) or [Deploy models using Azure CLI and Bicep](/models/create-model-deployments).
...
- [Deploy open-source models with managed compute](/models/deploy-models-managed)
