...
sidebarTitle: "MAI-Transcribe-2 - Speech Service"
---
...
- An audio file in one of these formats: WAV, MP3, or FLAC. For the maximum file size and audio duration, see the `audio` parameter in the [Transcriptions - Transcribe](https://learn.microsoft.com/rest/api/speechtotext/transcriptions/transcribe) REST reference. If you enable speaker diarization, see the note on recording length in the next section.
...
- Use one of the regions where MAI-Transcribe is available [Speech service regions](regions).
