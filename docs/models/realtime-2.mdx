...
Both models support Global Standard and Data Zone Standard deployments. For region-by-region availability, see [Region availability for Foundry Models sold by Azure](/models/models-sold-directly-by-azure-region-availability).
...
The GA endpoint doesn't use a date-based API version or an `api-version` query parameter. For complete examples, see [Use the GPT Realtime API via WebSockets](/models/realtime-audio-websockets) and [Use the GPT Realtime API via WebRTC](/models/realtime-audio-webrtc).
...
Realtime models have separate audio-token and concurrent-session quotas. Review [Azure OpenAI quotas and limits](/models/quotas-limits) before production deployment.
