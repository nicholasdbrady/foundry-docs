...
- A [Foundry project](create-projects). Note your resource endpoint (for example, `https://{resource}.services.ai.azure.com`).
- A [deployed model](/models/deploy-foundry-models) for inference calls.
- Authentication credentials: either a [Microsoft Entra ID token](../concepts/rbac-foundry) or an API key from the Foundry portal.
...
Don't use the Foundry Model Inference API (`https://<resource>.services.ai.azure.com/models` path) for new integrations. The Azure AI Inference beta SDK retired on August 26, 2026. Use the OpenAI v1-compatible route instead. For details, see the [migration guide](../how-to/model-inference-to-openai-migration).
...
- [Swagger for Foundry REST API](https://learn.microsoft.com/rest/api/microsoft-foundry/)
- [Swagger for OpenAI v1 compatibility](https://learn.microsoft.com/rest/api/microsoft-foundry/)
...
For the full SDK reference, see the [SDK overview](../how-to/develop/sdk-overview).

## Troubleshoot common integration issues

| Error | Cause | Resolution |
|-------|-------|------------|
| `401 Unauthorized` | Invalid or expired token/key | Regenerate your API key or refresh your Entra token. See [authentication options](../concepts/rbac-foundry). |
...
| `429 Too Many Requests` | Rate limit exceeded | Implement retry with exponential backoff, or [increase quota](/models/quota). |
...
- [Get started with Microsoft Foundry SDKs and endpoints](develop/sdk-overview)
