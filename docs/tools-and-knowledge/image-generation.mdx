...
1. Confirm your region and model support for image generation. See [Best practices for using tools in Microsoft Foundry Agent Service](/tools-and-knowledge/tool-best-practice).
...
This sample creates an agent with the image generation tool, generates an image, and saves it to a file. Select **Prompt Agents** to use the Azure AI Projects SDK to create a server-side prompt agent, or **Hosted Agents** to use the Agent Framework [`FoundryChatClient`](../../quickstarts/responses-api) to build an ephemeral, in-process agent.
...
| Tool not available | Regional or model limitation | Confirm the image generation tool is available in your region and with your orchestrator model. See [Best practices for using tools](/tools-and-knowledge/tool-best-practice). |
