...
In this article, you learn how to connect a knowledge base in Foundry IQ to an agent in Foundry Agent Service. The connection uses the [Model Context Protocol (MCP)](/tools-and-knowledge/model-context-protocol) to facilitate tool calls. When invoked by the agent, the knowledge base orchestrates the following operations:
...
- A [Microsoft Foundry project](../../how-to/create-projects) with an [LLM deployment](/models/create-model-deployments), such as `gpt-4.1-mini`.
...
Optionally, if your knowledge base includes a remote SharePoint knowledge source, you must also include the `x-ms-query-source-authorization` header in the MCP tool connection. For more information, see [Enforce permissions at query time (preview)](https://learn.microsoft.com/azure/search/agentic-retrieval-how-to-retrieve#enforce-permissions-at-query-time).
