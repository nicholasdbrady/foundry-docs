...
- Your Foundry resource must support MCP tools in Foundry Agent Service. MCP tools require the Agent Service Enterprise tier. For the list of regions and models that support MCP tools, see [Tool support by region and model](/agents/tool-best-practice#tool-support-by-region-and-model).
