...
- **Private endpoints**: Connect to MCP servers that aren't exposed to the public internet. Private MCP requires [Standard Agent Setup with private networking](../virtual-networks) and a dedicated MCP subnet within your virtual network.
...
To learn about supported authentication options, including key-based, Microsoft Entra identities, and OAuth identity passthrough, see [MCP server authentication](/tools-and-knowledge/mcp-authentication).
...
The following example shows how to use the GitHub MCP server as a tool for an agent. Select **Prompt Agents** to use the Azure AI Projects SDK to create a server-side prompt agent, or **Hosted Agents** to use the Agent Framework [`FoundryChatClient`](/agents/responses-api) to build an ephemeral, in-process agent.
...
<Tabs>
  ...
  <Tab title="Hosted Agents">
    ...
    This sample uses [`FoundryChatClient`](/agents/responses-api) from the Microsoft Agent Framework and calls `get_mcp_tool()` to register a hosted MCP server with per-tool approval control. Install the package with `pip install agent-framework-foundry aiohttp`, set the `FOUNDRY_PROJECT_ENDPOINT` and `FOUNDRY_MODEL` environment variables, and sign in with `az login`.
...
<Frame>
<img src="/images/background-mode-enable.png" alt="Screenshot of the model parameters panel in the Foundry portal with a supported model selected and the Background mode toggle turned on." />
</Frame>

1. Under **Tools**, add a tool whose MCP server supports MCP tasks, such as a Fabric data agent added through the Fabric IQ tool. For steps, see [Connect agents to Microsoft Fabric with Fabric IQ](/tools-and-knowledge/fabric-iq#run-a-fabric-data-agent-in-background-mode).
...
<Frame>
<img src="/images/background-mode-running.png" alt="Screenshot of the Foundry portal chat showing a background run in progress, with a progress indicator, after the user sends a message." />
</Frame>
...
- [MCP server authentication](/tools-and-knowledge/mcp-authentication)
- [Build and register a Model Context Protocol (MCP) server](/tools-and-knowledge/build-your-own-mcp-server)
