...
In most cases, use the [Realtime API via WebRTC](/foundry-models/realtime-audio-webrtc) for real-time audio streaming in client-side applications such as a web application or mobile app. WebRTC is designed for low-latency, real-time audio streaming and is the best choice for most scenarios.
...
The GPT real-time models are available for global deployments in the [East US 2 and Sweden Central regions](/foundry-models/models-sold-directly-by-azure#global-standard-model-availability).
...
For more information about supported models, see the [models and versions documentation](/foundry-models/models-sold-directly-by-azure#audio-models).
...
- A client-side caller establishes a connection to `/realtime`, which starts a new [`session`](/api-sdk/realtime-audio-reference#realtimerequestsession).
...
- Each caller-initiated [`response.create`](/api-sdk/realtime-audio-reference#realtimeclienteventresponsecreate) can override some of the output [`response`](/api-sdk/realtime-audio-reference#realtimeresponse) behavior, if desired.
...
Now that you've done the above steps, you can follow the instructions in the [Realtime API quickstart](/foundry-models/realtime-audio#quickstart) to get started with the Realtime API via WebSockets.

## Related content

* Try the [real-time audio quickstart](/foundry-models/realtime-audio#quickstart)
