...
The GPT real-time models are available for global deployments in the [East US 2 and Sweden Central regions](/foundry-models/models-sold-directly-by-azure-region-availability).
...
- A client-side caller establishes a connection to `/realtime`, which starts a new [`session`](/api-sdk/realtime-audio-reference).
...
- Each caller-initiated [`response.create`](/api-sdk/realtime-audio-reference) can override some of the output [`response`](/api-sdk/realtime-audio-reference) behavior, if desired.
