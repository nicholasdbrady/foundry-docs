...
- **Role-based access control (RBAC):** One or both of the following roles at the subscription or resource group scope:
  ...
  - [**Foundry User**](/security/rbac-foundry#built-in-roles) – View Foundry resource data and usage context.
...
- **Region and model availability check:** Confirm required model and feature availability in your target regions before deployment. For details, see [Feature availability across cloud regions](/foundry-models/region-support).
...
If you need to grant these roles to team members, see [Assign access to Cost Management data](https://learn.microsoft.com/azure/cost-management-billing/costs/assign-access-acm-data) and [Foundry RBAC roles](/security/rbac-foundry).
...
- **View Foundry resources and related usage context:** [Foundry User](/security/rbac-foundry#built-in-roles).
...
1. Required models and services are available in your target regions. See [Feature availability across cloud regions](/foundry-models/region-support).
...
Fine-tuned deployments incur hosting charges while deployed, even during low usage periods. Remove or scale down deployments that you don't need. For details on deployment lifecycle and cleanup policies, see the [fine-tuning documentation](/foundry-models/fine-tuning).
...
This custom role example doesn't grant access to Foundry resources by itself. Assign an additional role such as [Foundry User](/security/rbac-foundry#built-in-roles) if users also need Foundry resource visibility.
...
Estimates do not reflect discounts or contracted pricing that may appear on your final bill. Estimates cover standard deployment costs only, not [provisioned throughput](/foundry-models/provisioned-throughput).
...
**Reference:** [Agent concepts](/agent-service/development-lifecycle)
...
**Reference:** [Monitor agent metrics](/agent-service/how-to-monitor-agents-dashboard)
...
- [Foundry status dashboard](/operate/foundry-status-dashboard-documentation)
