...
<Note>
If you're building a new application, refer to the [Realtime API quickstart](/foundry-models/realtime-audio#quickstart) instead. This guide is only for migrating existing Preview applications to GA.
</Note>
...
See detailed information on the endpoint format in [this article](/foundry-models/realtime-audio-websockets#connection-and-authentication). See example of GA endpoint format usage in [Quick start on GPT Realtime API for speech and audio](/foundry-models/realtime-audio#quickstart).
...
Ephemeral keys for WebRTC requests are now created with a `POST /openai/v1/realtime/client_secrets` request on your Azure OpenAI endpoint (see details in [Step-by-step guide](/foundry-models/realtime-audio-webrtc#step-1-set-up-service-to-procure-ephemeral-token)).

The URL for initializing the WebRTC connection in the browser is now `/openai/v1/realtime/calls` (see details in [Step-by-step guide](/foundry-models/realtime-audio-webrtc#step-2-set-up-your-browser-application)).
...
- [WebSocket implementation guide](/foundry-models/realtime-audio-websockets) - Detailed WebSocket configuration
- [WebRTC implementation guide](/foundry-models/realtime-audio-webrtc) - Browser-based audio implementation
- [SIP integration](/foundry-models/realtime-audio-sip) - Enable phone call integration
- [Realtime API quickstart](/foundry-models/realtime-audio#quickstart) - Additional implementation examples
