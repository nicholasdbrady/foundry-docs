...
Before configuring Claude Code, deploy the available [Claude models](/foundry-models/models-from-partners#anthropic) that Claude Code needs. Claude models in Foundry are available for [global standard deployment](/foundry-models/deployment-types#global-standard). Claude Code uses different models for different tasks:

<Info>
Claude models in Microsoft Foundry are currently in preview. Model availability might change. Check the [Foundry Models from partners](/foundry-models/models-from-partners#anthropic) page for the latest list of available models.
</Info>
...
1. Follow the instructions in [Deploy Microsoft Foundry Models in the Foundry portal](/foundry-models/deploy-foundry-models) to deploy a Claude model, such as Opus 4.6.
...
To use Model Router with Claude Code, first deploy the supported Claude models, then deploy Model Router and enable them through [model subset configuration](/foundry-models/model-router#select-your-model-subset).
...
- [Deploy and use Claude models in Microsoft Foundry](/foundry-models/use-foundry-models-claude)
