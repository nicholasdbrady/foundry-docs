...
- Importing training data from Azure Blob Storage requires the storage account to have public network access enabled. If your enterprise environment restricts public access on storage accounts, use the [local file upload](https://learn.microsoft.com/rest/api/azureopenai/files/upload) method or the SDK to upload training data directly.
...
- Learn more about [Foundry fine-tuning](/foundry-models/fine-tuning)
- Explore our [fine-tuning tutorial](https://learn.microsoft.com/en-us/azure/foundry-classic/openai/tutorials/fine-tune)
