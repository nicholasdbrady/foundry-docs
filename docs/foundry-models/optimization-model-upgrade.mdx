...
1. Select a model name to view its details in the model catalog, or select **Deploy** to go to the model deployment page. To learn how to deploy a model, see [Add and configure models to Foundry Models](/foundry-models/create-model-deployments).
...
| Issue | Resolution |
|-------|------------|
| Ask AI doesn't suggest any model upgrades | Verify that your deployed model has a newer version available in the [model catalog](/foundry-models/create-model-deployments). Ask AI only recommends upgrades when newer versions exist. |
