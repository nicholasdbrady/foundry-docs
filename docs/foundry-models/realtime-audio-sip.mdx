...
The GPT real-time models are available for global deployments in [East US 2 and Sweden Central regions](/foundry-models/models-sold-directly-by-azure#global-standard-model-availability).
...
- [Use the Realtime API via WebRTC](/foundry-models/realtime-audio-webrtc) for browser-based applications
- [Use the Realtime API via WebSocket](/foundry-models/realtime-audio-websockets) for server-to-server scenarios
- [Azure OpenAI webhooks](/foundry-models/webhooks) for webhook configuration details
