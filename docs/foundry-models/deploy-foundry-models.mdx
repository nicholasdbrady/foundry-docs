...
This article uses a Foundry Model from partners and community `Llama-3.2-90B-Vision-Instruct` for illustration. Models from partners and community require that you subscribe to Azure Marketplace before deployment. On the other hand, Foundry Models sold by Azure, such as Azure OpenAI in Foundry Models, don't have this requirement. For more information about Foundry Models, including the regions where they're available for deployment, see [Foundry Models sold by Azure](/foundry-models/models-sold-directly-by-azure) and [Foundry Models from partners and community](/foundry-models/models-from-partners).

## Prerequisites

To complete this article, you need:

- An Azure subscription with a valid payment method. If you don't have an Azure subscription, create a [paid Azure account](https://azure.microsoft.com/pricing/purchase-options/azure-account?cid=msft_learn) to begin. If you're using GitHub Models, you can [upgrade to Foundry Models](/foundry-models/quickstart-github-models) and create an Azure subscription in the process.
...
- [Foundry Models from partners and community](/foundry-models/models-from-partners) require access to **Azure Marketplace** to create subscriptions. Ensure you have the [permissions required to subscribe to model offerings](../how-to/configure-marketplace). [Foundry Models sold by Azure](/foundry-models/models-sold-directly-by-azure) don't have this requirement.
...
1. For [Foundry Models from partners and community](/foundry-models/models-from-partners), you need to subscribe to Azure Marketplace. This requirement applies to `Llama-3.2-90B-Vision-Instruct`, for example. Read the terms of use and select **Agree and Proceed** to accept the terms.

<Note>

For [Foundry Models sold by Azure](/foundry-models/models-sold-directly-by-azure), such as the Azure OpenAI model `gpt-4o-mini`, you don't subscribe to Azure Marketplace.
...
Each model supports different deployment types, providing different data residency or throughput guarantees. See [deployment types](/foundry-models/deployment-types) for more details. In this example, the model supports the Global Standard deployment type.

</Tip>

1. When the deployment completes, you land on the [Foundry Playgrounds](/foundry-models/concept-playgrounds) where you can interactively test the model. Your project and resource must be in one of the supported regions of deployment for the model. Verify that the deployment status shows **Succeeded** in your deployment list.
...
- To use the **Responses API with Foundry Models sold by Azure**, such as Microsoft AI, DeepSeek, and Grok models, see [How to generate text responses with Microsoft Foundry Models](/foundry-models/generate-responses).

- To use the **Responses API with OpenAI models**, see [Getting started with the responses API](/foundry-models/responses).
...
For Foundry Models, the default quota varies by model and region. Certain models might only be available in some regions. For more information on availability and quota limits, see [Azure OpenAI in Microsoft Foundry Models quotas and limits](/foundry-models/quotas-limits) and [Microsoft Foundry Models quotas and limits](/foundry-models/quotas-limits).
...
For more information about quota, see [Microsoft Foundry Models quotas and limits](/foundry-models/quotas-limits) and [Manage Azure OpenAI quota](https://learn.microsoft.com/en-us/azure/foundry-classic/openai/how-to/quota).
...
| Region not supported | Check [regional availability](/foundry-models/quotas-limits) and deploy in a supported region. |
...
- [How to generate text responses with Microsoft Foundry Models](/foundry-models/generate-responses)
...
- [Azure OpenAI supported programming languages](/foundry-models/supported-languages)
