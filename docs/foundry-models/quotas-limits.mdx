...
| Maximum prompt tokens per request | Varies per model. For more information, see [Azure OpenAI models](/foundry-models/models-sold-directly-by-azure).|
...
<Note>
Currently, both the Foundry portal and the capacity API return quota/capacity information for models that are [retired](/foundry-models/model-retirements) and no longer available.
</Note>
