...
You can currently check you quota tier with the [control plane API](https://learn.microsoft.com/rest/api/aifoundry/accountmanagement/quota-tiers/get):
...
You might receive **429 (Too Many Requests)** responses even when token usage metrics appear below your quota. For an explanation of why this happens, see [Why you might see 429s even when token usage metrics are below quota](/foundry-models/quota#why-you-might-see-429s-even-when-token-usage-metrics-are-below-quota).
...
For detailed best practices, retry-with-backoff code samples, and a 429 troubleshooting guide, see [Manage Azure OpenAI in Microsoft Foundry Models quota](/foundry-models/quota#rate-limit-best-practices).
