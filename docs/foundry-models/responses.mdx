...
Before you run the examples in this article, confirm that your resource region supports the Responses API. The v1 API is required to access the latest features — for details, see the [API version lifecycle](/api-sdk/api-version-lifecycle). The Responses API is currently available in the following regions:
...
Not every model is available in every supported region. Check the [models page](/foundry-models/models-sold-directly-by-azure) for model region availability. For the full set of request and response parameters, see the [Responses API reference documentation](/api-sdk/reference-preview-latest).
...
- A file ID created with the [Files API](/api-sdk/reference-preview-latest)
...
The image generation tool doesn't currently support streaming mode. To stream partial images, call the [image generation API](/foundry-models/dall-e) directly outside of the Responses API.
...
For examples of how to use reasoning models with the responses API see the [reasoning models guide](/foundry-models/reasoning#reasoning-summary).
