...
1. Confirm your region and model support for image generation. See [Best practices for using tools in Microsoft Foundry Agent Service](/agent-service/tool-best-practice).

## Code examples

Before you start, install the latest SDK package. The .NET SDK is currently in preview. For package installation instructions, see the [quickstart](/get-started/get-started-code).
...
This sample creates an agent with the image generation tool, generates an image, and saves it to a file. Select **Prompt Agents** to use the Azure AI Projects SDK to create a server-side prompt agent, or **Hosted Agents** to use the Agent Framework [`FoundryChatClient`](/agent-service/responses-api) to build an ephemeral, in-process agent.
...
For a thorough look at how you can tweak your text prompts to generate different kinds of images, see [Image prompt engineering techniques](/foundry-models/gpt-4-v-prompt-engineering).
...
| Tool not available | Regional or model limitation | Confirm the image generation tool is available in your region and with your orchestrator model. See [Best practices for using tools](/agent-service/tool-best-practice). |
...
- [Image generation in Azure OpenAI](/foundry-models/dall-e)
- [Responses API in Azure OpenAI](/foundry-models/responses)
