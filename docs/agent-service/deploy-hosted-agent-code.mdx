...
This article shows you how to deploy a [Hosted agent](/agent-service/hosted-agents) in Foundry Agent Service from Python or .NET source code, without building or pushing a container image. You upload a `.zip` of your code (and optionally your dependencies), and Agent Service either runs it as-is or builds your dependencies for you in the cloud.

If you're deploying for the first time or want the fastest path, use the [Quickstart: Deploy your first hosted agent](/agent-service/quickstart-hosted-agent) instead. The quickstart uses the **Azure Developer CLI (azd)** or the **Foundry Toolkit for VS Code**, which package your source, upload it, poll for `active`, and configure role-based access control automatically. Choose **Code** (or **Source Code (ZIP upload)**) when the quickstart asks for a deployment method.
...
If you need full control of the runtime image or you already have a working Dockerfile, use the container-based path: [Deploy a hosted agent](/agent-service/deploy-hosted-agent).
...
- A [Microsoft Foundry project](/setup/create-projects) in a supported region.
...
For a guided walkthrough using `azd` or the Foundry Toolkit, see the [Quickstart](/agent-service/quickstart-hosted-agent). The rest of this article walks the same lifecycle using the REST API.
...
`code_configuration` and `container_configuration` are mutually exclusive in the agent definition: include `code_configuration` for source-code deploy (this article) or `container_configuration` for image-based deploy. See [Deploy a hosted agent (container)](/agent-service/deploy-hosted-agent) for the image variant.
...
| `409 AgentNotCodeBased` on download | Agent is image-based | Use the [container-based deploy doc](/agent-service/deploy-hosted-agent). |
...
If you scaffolded the project from the [Quickstart](/agent-service/quickstart-hosted-agent) with `azd`, run `azd down` from the project root to remove the entire provisioned environment instead.
...
- [Deploy a hosted agent in a virtual network](/setup/virtual-networks)

## Related content

- [Deploy a hosted agent (container)](/agent-service/deploy-hosted-agent)
