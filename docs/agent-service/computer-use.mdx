...
The following code sample demonstrates how to create an agent version with the computer use tool, send an initial request with a screenshot, and perform multiple iterations to complete a task. Select **Prompt Agents** to use the Azure AI Projects SDK to create a server-side prompt agent, or **Hosted Agents** to use the Agent Framework [`FoundryChatClient`](/agent-service/responses-api) to build an ephemeral, in-process agent.
...
<Tabs>
  ...
  <Tab title="Hosted Agents">
    ...
    This sample uses [`FoundryChatClient`](/agent-service/responses-api) from the Microsoft Agent Framework and calls `get_computer_use_tool()` to attach the computer use preview tool. Install the package with `pip install agent-framework-foundry aiohttp`, set the `FOUNDRY_PROJECT_ENDPOINT` (point `FOUNDRY_MODEL` at a `computer-use-preview` deployment), and sign in with `az login`. The screenshot-capture loop is application-specific; see the upstream sample helper file referenced below.
...
The following table lists some of the differences between the computer use tool and [browser automation](/agent-service/browser-automation) tool.
...
To use the computer use tool, you need a [computer use model](/foundry-models/models-sold-directly-by-azure#computer-use-preview) deployment. The computer use model is available in the following regions:
...
- [Tool best practices for agents](/agent-service/tool-best-practice)
- [Browser automation tool](/agent-service/browser-automation)
- [Set up your agent environment](/setup/environment-setup)
- [Get started with Foundry agents](/get-started/get-started-code)
- [Computer use risks and limitations](/responsible-ai/transparency-note#risk-and-limitations-of-computer-use-preview)
