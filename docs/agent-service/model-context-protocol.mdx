...
- The latest SDK package for your language. The .NET and Java SDKs are currently in preview. See the [quickstart](/get-started/get-started-code) for installation details.
...
To learn about supported authentication options (key-based, Microsoft Entra identities, and OAuth identity passthrough), see [MCP server authentication](/agent-service/mcp-authentication).
...
Use the following code sample to create an agent and call the function. The .NET and Java SDKs are currently in preview. See the [quickstart](/get-started/get-started-code) for details.
...
<Tip>
For details on the MCP tool schema and approval items, see [OpenAI.MCPTool](/api-sdk/foundry-project-rest-preview#openaimcptool) and the MCP approval item types in the REST reference.
</Tip>
...
- [MCP tool REST reference](/api-sdk/foundry-project-rest-preview#openaimcptool)
