...
- **Foundry manages the backing secret name.** When you create the connection, Foundry stores the value in Key Vault under a name it chooses — you can't reference a pre-existing Key Vault secret by name. To attach your own Key Vault as the backing store, see [Set up a Key Vault connection](/security/set-up-key-vault-connection).
