...
- Latest SDK package installed for your language. The .NET and Java SDKs are currently in preview. See the [quickstart](/get-started/get-started-code) for installation steps.
...
For examples of conversation and file cleanup patterns, see [Web search tool (preview)](/agent-service/web-search) and [File search tool for agents](/agent-service/file-search).
