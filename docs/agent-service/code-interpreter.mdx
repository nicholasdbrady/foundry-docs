...
- Basic or standard agent environment. See [agent environment setup](/setup/environment-setup) for details.
...
You can customize Code Interpreter behavior at runtime, such as specifying which files to include or adjusting tool parameters per request, by using [structured inputs](/agent-service/structured-inputs).
...
<Tabs>
  ...
  <Tab title="Hosted Agents">
    ...
    This sample uses [`FoundryChatClient`](/agent-service/responses-api) from the Microsoft Agent Framework and calls `get_code_interpreter_tool()` to give the agent a sandboxed Python execution environment. Install the package with `pip install agent-framework-foundry aiohttp`, set the `FOUNDRY_PROJECT_ENDPOINT` and `FOUNDRY_MODEL` environment variables, and sign in with `az login`.
...
For the current list of supported regions and models for Code Interpreter, see [Best practices for using tools in Microsoft Foundry Agent Service](/agent-service/tool-best-practice).
...
| Python package not available. | Code Interpreter has a fixed set of packages. | Code Interpreter includes common data science packages. For custom packages, use [Custom code interpreter](/agent-service/custom-code-interpreter). |
...
For examples of conversation and file cleanup patterns, see [Web search tool](/agent-service/web-search) and [File search tool for agents](/agent-service/file-search).
...
If you need more control over the sandbox runtime or you need a different isolation model, see [Custom code interpreter tool for agents](/agent-service/custom-code-interpreter).
