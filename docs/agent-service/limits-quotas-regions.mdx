...
<Note>
Foundry Agent Service is generally available (GA). Some sub-features, such as [hosted agents](/agent-service/hosted-agents), are in public preview and might have different constraints.
</Note>
...
Foundry Agent Service is available in the same regions as the [Azure OpenAI Responses API](/foundry-models/responses#region-availability).

<Info>
Not all tools are available in every region. For example, file search isn't available in Italy North and Brazil South. For the full tool-by-region matrix, see [Tool support by region and model](/agent-service/tool-best-practice#tool-support-by-region-and-model).
</Info>

## Azure OpenAI model support

Foundry Agent Service is compatible with current Azure OpenAI models. For a complete list of supported models and their availability by region, see [Foundry Models sold directly by Azure](/foundry-models/models-sold-directly-by-azure).
...
- Try a different region that supports the model and version. See the [model and region support table](/agent-service/limits-quotas-regions).
...
- Check the [tool support by region and model](/agent-service/tool-best-practice#tool-support-by-region-and-model) table to confirm availability before you deploy.
...
- Review [Provisioned throughput](/foundry-models/provisioned-throughput) and [Spillover traffic management](/foundry-models/spillover-traffic-management).
...
- Review [Azure OpenAI quotas and limits](/foundry-models/quotas-limits) for your deployment's tokens-per-minute and requests-per-minute caps.
...
If you're using threads and messages, see [Threads, runs, and messages in Foundry Agent Service](/agent-service/runtime-components). If you're using file search, see [Vector stores for file search](/agent-service/vector-stores).
...
The Agent Service limits in this table are fixed and apply uniformly across all subscription types. Agent Service doesn't impose separate rate limits on API calls. Rate limiting is applied at the model deployment level. See [Azure OpenAI quotas and limits](/foundry-models/quotas-limits) for model-specific rate limits.
...
For file search scenarios, see [Vector stores for file search](/agent-service/vector-stores) for guidance on managing vector store growth.
...
- **Monitor usage trends.** Track agent activity by using [Foundry Agent Service metrics](/agent-service/how-to-monitor-agents-dashboard) to identify growth before you hit limits.
...
To view or request more model quota, see [Manage and increase quotas for resources with Microsoft Foundry (Foundry projects)](/foundry-models/quota).

## Request a limit increase

The limits in this article are default values for Foundry Agent Service. If your workload requires higher limits:

- **Model quotas.** You can request increases for model deployment quotas. See [Manage and increase quotas for resources with Microsoft Foundry](/foundry-models/quota).
...
- [Threads, runs, and messages in Foundry Agent Service](/agent-service/runtime-components)
- [Tool support by region and model](/agent-service/tool-best-practice#tool-support-by-region-and-model)
- [Vector stores for file search](/agent-service/vector-stores)
- [Monitor Foundry Agent Service](/agent-service/how-to-monitor-agents-dashboard)
- [Azure OpenAI quotas and limits](/foundry-models/quotas-limits)
- [Manage and increase quotas for resources with Microsoft Foundry](/foundry-models/quota)
