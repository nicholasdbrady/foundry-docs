...
In this article, you learn how to connect a knowledge base in Foundry IQ to an agent in Foundry Agent Service. The connection uses the [Model Context Protocol (MCP)](/agent-service/model-context-protocol) to facilitate tool calls. When invoked by the agent, the knowledge base orchestrates the following operations:
...
- A [Microsoft Foundry project](/setup/create-projects) with an [LLM deployment](/foundry-models/create-model-deployments), such as `gpt-4.1-mini`.
