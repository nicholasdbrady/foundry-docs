...
For background on hosted agents and the available protocols, see [What are hosted agents?](/agent-service/hosted-agents). For general container packaging and deployment steps, see [Deploy a hosted agent](/agent-service/deploy-hosted-agent).
...
For request/response style invocations, continue to use the HTTP [`/invocations`](/agent-service/deploy-hosted-agent#protocol-libraries) or `/responses` routes. A single container can expose all three protocols at the same time.
...
|-----------|-------------|
| [Microsoft Voice Live](https://learn.microsoft.com/azure/ai-services/speech-service/voice-live) | Real-time voice interaction with built-in STT, LLM, and TTS. Supports cascaded pipelines and speech-to-speech models such as GPT-4o Realtime. |
...
For the full deployment flow (build, push, RBAC, polling for status), see [Deploy a hosted agent](/agent-service/deploy-hosted-agent).
...
- **Tools.** Access Foundry-managed tools (Web Search, File Search, Code Interpreter, Azure AI Search, custom MCP servers, A2A) through the Toolbox MCP endpoint in your Foundry project. See [Curate intent-based toolbox in Foundry](/agent-service/toolbox).

## Observability

The `/invocations_ws` upgrade and per-connection lifecycle should be instrumented with distributed trace spans, following the [OpenTelemetry GenAI conventions](/agent-service/trace-agent-concept) used by the HTTP `/invocations` protocol.
...
- [Manage hosted agent sessions](/agent-service/manage-hosted-sessions)
- [Configure virtual networks](/setup/virtual-networks)
