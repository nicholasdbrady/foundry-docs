...
- A Foundry project. For more information, see [Create a Foundry project](/setup/create-projects).
- Tracing connected to an Azure Monitor Application Insights resource. To set it up, see [Set up tracing in Microsoft Foundry](/agent-service/trace-agent-setup).
...
1. Assign an appropriate role to your user or group.
    ...
    If you use log-based queries, start by granting the [Log Analytics Reader role](https://learn.microsoft.com/azure/azure-monitor/logs/manage-access).
...
1. Confirm tracing is connected for your project. If needed, follow [Set up tracing in Microsoft Foundry](/agent-service/trace-agent-setup).
1. Run your agent at least once.
1. In the Foundry portal, go to **Observability** > **Traces**.
1. Confirm a new trace appears with spans for your agent's operations.
...
- Learn core concepts and architecture in the [Agent tracing overview](/agent-service/trace-agent-concept).
- If you haven't enabled tracing yet, see [Set up tracing in Microsoft Foundry](/agent-service/trace-agent-setup).
- Visualize agent health and performance metrics with the [Agent Monitoring Dashboard](/agent-service/how-to-monitor-agents-dashboard).
