...
Use a custom code interpreter when the built-in [Code Interpreter tool for agents](/agent-service/code-interpreter) doesn't meet your requirements—for example, when you need specific Python packages, custom container images, or dedicated compute resources.

For more information about MCP and how agents connect to MCP tools, see [Connect to Model Context Protocol servers (preview)](/agent-service/model-context-protocol).
...
For the latest SDK and API support for agents tools, see [Best practices for using tools in Microsoft Foundry Agent Service](/agent-service/tool-best-practice).
...
1. In Microsoft Foundry, verify your agent calls the tool using tracing. For more information, see [Best practices for using tools in Microsoft Foundry Agent Service](/agent-service/tool-best-practice).
...
- [Code Interpreter tool for agents](/agent-service/code-interpreter)
