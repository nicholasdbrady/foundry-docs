...
This sample works for both prompt agents and hosted agents that use the responses protocol. For hosted agents that use the invocations protocol, the `input_messages` format is different — provide a freeform JSON object instead of the structured template. For details and code samples, see [Hosted agent invocations protocol](/evaluation/cloud-evaluation#hosted-agent-invocations-protocol) in the cloud evaluation guide.

</Tip>

<Tip>

To evaluate agent interactions that already occurred using traces from Application Insights, see [Trace evaluation](/evaluation/cloud-evaluation#trace-evaluation-preview) in the cloud evaluation guide.
...
- [Trace evaluation in the cloud](/evaluation/cloud-evaluation#trace-evaluation-preview)
