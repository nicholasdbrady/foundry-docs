...
[Evaluation datasets in Microsoft Foundry](/evaluation/evaluation-datasets) and
...
- **Simulation seed (multi-turn)** produces scenario descriptions that feed the [Simulate conversations](/evaluation/cloud-evaluation-simulate-conversations) flow for multi-turn evaluation.
...
Synthetic generation and trace-based generation are complementary: synthetic datasets cover edge cases and prelaunch scenarios, while trace-based datasets reflect real production behavior. Using both gives the strongest evaluation signal. See [Convert agent traces into evaluation datasets](/observability/traces-to-dataset).
...
- An Azure OpenAI model deployment that supports the Responses API. Both the `simple_qna` and `simulation_seed` recipes use this model to synthesize output rows. For the supported-model list, see [Azure OpenAI Responses API model support](https://learn.microsoft.com/azure/foundry/openai/how-to/responses).
- A supported region. For the list, see [Supported regions for data generation](/evaluation/evaluation-regions-limits-virtual-network#supported-regions-for-data-generation).
...
<Frame>
  <img src="/images/data-generation-synthetic-evaluation.png" alt="Screenshot of the Generate synthetic data dialog showing Dataset usage set to Evaluation, Task type set to Simple Q&A, Generator model, source inputs, Maximum number of samples, and Output file name." />
</Frame>
...
<Tabs>
  ...
  <Tab title="JavaScript/TypeScript">
    ...
    Reference: [datasets.createGenerationJob](https://learn.microsoft.com/javascript/api/@azure/ai-projects/aiprojectclient)
...
Simulation seed jobs produce a dataset of scenario descriptions that feed the [Simulate conversations](/evaluation/cloud-evaluation-simulate-conversations) flow. Generated rows can contain `id`, `category`, `test_case_description`, and `desired_num_turns`. Only `test_case_description` is required.
...
<Tabs>
  ...
  <Tab title="JavaScript/TypeScript">
    ...
    Reference: [datasets.listGenerationJobs](https://learn.microsoft.com/javascript/api/@azure/ai-projects/aiprojectclient)

  </Tab>
</Tabs>

For more context, see [Manage data generation jobs](/observability/traces-to-dataset#manage-data-generation-jobs).
...
- **Combine synthetic and trace-based generation for the strongest coverage.** Synthetic data fills gaps before launch and for edge cases; production traces reflect how your agent actually behaves. Use both sources together rather than treating them as alternatives. See [Convert agent traces into evaluation datasets](/observability/traces-to-dataset).
...
- [Run cloud evaluations](/evaluation/cloud-evaluation)
- [Simulate conversations](/evaluation/cloud-evaluation-simulate-conversations)
- [Evaluate conversations in the cloud](/evaluation/cloud-evaluation-conversations)
- [Set up tracing for your agent](/observability/trace-agent-setup)
