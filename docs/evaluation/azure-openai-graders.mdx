...
Azure OpenAI graders use the OpenAI Evals API and are configured differently from [built-in evaluators](/evaluation/built-in-evaluators), which use the `azure_ai_evaluator` type. Use graders when you need custom LLM-based classification or scoring with full control over the prompt, or when you need deterministic string or similarity checks without an LLM judge.
