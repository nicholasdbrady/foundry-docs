...
Custom evaluators let you define domain-specific quality metrics that go beyond the [built-in evaluator catalog](/evaluation/built-in-evaluators). Use a custom evaluator when you need to measure criteria unique to your application, such as brand tone, domain-specific accuracy, or output format compliance.
...
After you create a custom evaluator, you can add it to the evaluator catalog in your Foundry project and use it in [batch evaluation runs](/evaluation/cloud-evaluation).
...
For more information on data source options, evaluator mappings, and advanced scenarios, see [Run evaluations from the SDK](/evaluation/cloud-evaluation).
...
For detailed steps on running evaluations from the portal, see [Run evaluations from the portal](/evaluation/evaluate-generative-ai-app#create-an-evaluation).
