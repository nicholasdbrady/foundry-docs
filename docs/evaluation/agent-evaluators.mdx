...
For AI-assisted evaluators, you can use Azure OpenAI or OpenAI [reasoning models](/models/reasoning) and non-reasoning models for the LLM judge. For complex evaluation that requires refined reasoning, we recommend `gpt-5-mini` for its balance of performance, cost, and efficiency.
...
For more complex agent interactions with tool calls, use message arrays for `query` and `response`. These arrays use the same OpenAI message structure as the preferred `messages` schema. See [Separate query and response format](/evaluation/evaluation-dataset-schema#separate-query-and-response-format). The system message is optional but useful for evaluators that assess agent behavior against instructions, including `task_adherence`, `task_completion`, `tool_call_accuracy`, `tool_selection`, `tool_input_accuracy`, `tool_output_utilization`, and `groundedness`:
...
See [Run evaluations from the SDK](/evaluation/cloud-evaluation) for details on running evaluations and configuring data sources.
...
[Messages with tool calls](/evaluation/evaluation-dataset-schema#messages-with-tool-calls)
and [Separate query and response format](/evaluation/evaluation-dataset-schema#separate-query-and-response-format).
...
- [How to run batch evaluation](/evaluation/cloud-evaluation)
