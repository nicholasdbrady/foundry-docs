...
Instead of authoring scenarios by hand, generate them by using the **Simulation seed (multi-turn)** task type. Before using an existing generated dataset with this API, normalize `id` to `test_case_id` and move `desired_num_turns` into `simulation_configuration`. See [Generate a simulation seed dataset](/evaluation/evaluation-dataset-synthetic#generate-a-simulation-seed-dataset-sdk).
...
|---|---|---|
| `model` | Yes | Simulator deployment in `{connectionName}/{modelDeploymentName}` format. The [model router](/models/model-router) isn't supported as the simulator model; it can only be an evaluation target. |
...
Poll the run until it reaches a terminal state as described in [Get cloud evaluation results](/evaluation/cloud-evaluation-results). When `enable_conversation_dataset_generation` is `true`, a completed run includes an `output_datasets` entry like this one:
...
- To evaluate stored conversations, see [Evaluate conversation datasets](/evaluation/cloud-evaluation-conversations).
- To evaluate production traces, see [Evaluate deployed model and agent conversations](/evaluation/cloud-evaluation-deployed-conversations).
