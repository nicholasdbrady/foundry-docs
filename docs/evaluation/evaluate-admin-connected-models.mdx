...
- Reference: [`TestingCriterionAzureAIEvaluator`](https://learn.microsoft.com/python/api/azure-ai-projects/azure.ai.projects.models.testingcriterionazureaievaluator) (Python)
...
Use this target with the [model target evaluation flow described in Run evaluations in the cloud](/evaluation/cloud-evaluation-targets#evaluate-a-model-target).

## Use other evaluation scenarios

Other model-based scenarios in [Run evaluations in the cloud by using the Microsoft Foundry SDK](/evaluation/cloud-evaluation) work similarly with admin-connected models. Wherever the scenario accepts a supported model deployment, replace the deployment name with `<connection-name>/<deployment-name>`. For example, conversation simulation accepts this reference in `item_generation_params.model`.
...
- [Run evaluations in the cloud by using the Microsoft Foundry SDK](/evaluation/cloud-evaluation)
- [Bring your own model to Foundry Agent Service](../../agents/how-to/ai-gateway)
