...
Though the AI Red Teaming Agent can be run [locally](/evaluation/run-scans-ai-red-teaming-agent) during prototyping and development to help identify safety risks, running them in the cloud allows for the following scenarios:
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    Install [cURL](https://curl.se/) and the [Azure CLI](https://learn.microsoft.com/cli/azure/install-azure-cli). In the cURL examples, replace the project endpoint placeholders and `<token>` with an access token for `https://ai.azure.com`.
...
1. **Create a connection**: Follow the instructions in [Add a new connection to your project](../connections-add) to connect your Azure OpenAI or AI Services resource to your Foundry project.
...
<Tabs>
  ...
  <Tab title="JavaScript/TypeScript">
    ...
    Reference: [evaluationTaxonomies.create](https://learn.microsoft.com/javascript/api/@azure/ai-projects/aiprojectclient)
...
- [Run AI Red Teaming Agent locally](/evaluation/run-scans-ai-red-teaming-agent)
- [AI Red Teaming Agent concepts](/evaluation/ai-red-teaming-agent)
