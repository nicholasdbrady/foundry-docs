...
| `model` | Yes | Model deployment to use for simulating the user. For example, `gpt-4.1`. The [model router](/models/model-router) isn't supported as the simulator model; it can only be used as the evaluation target. |
