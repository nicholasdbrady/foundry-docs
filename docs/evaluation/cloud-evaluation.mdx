...
Use cloud evaluations for most scenarios - especially when testing at scale, integrating evaluations into continuous integration and continuous delivery (CI/CD) pipelines, or performing predeployment testing. Running evaluations in the cloud eliminates the need to manage local compute infrastructure and supports large-scale, automated testing workflows. You can also [schedule evaluations](/agents/how-to-monitor-agents-dashboard) to run on a recurring basis, or set up [continuous evaluation](/agents/how-to-monitor-agents-dashboard#) to automatically evaluate sampled agent responses in production.

Cloud evaluation results are stored in your Foundry project. You can review results in the portal, retrieve them through the SDK, or route them to Application Insights if connected. Cloud evaluation supports all Microsoft-curated [built-in evaluators](/observability/observability#what-are-evaluators) and your own [custom evaluators](/evaluation/custom-evaluators). Evaluators are managed in the [evaluator catalog](/evaluation/evaluate-generative-ai-app) with the same project-scope, role-based access control.
...
| **[Red team evaluation](/evaluation/run-ai-red-teaming-cloud)** | You want automated adversarial testing - Foundry generates jailbreaks and harmful-content prompts and scores how the target responds. | `azure_ai_red_team` → `azure_ai_model` or `azure_ai_agent` |
...
For an overview of available evaluators and how to pick them, see [built-in evaluators](/observability/observability#what-are-evaluators) and [custom evaluators](/evaluation/custom-evaluators).
...
- Optionally, you can [use your own storage account](/evaluation/evaluation-regions-limits-virtual-network#bring-your-own-storage) to run evaluations.

<Note>

Some evaluation features have regional restrictions. See [supported regions](/evaluation/risk-safety-evaluators#foundry-project-configuration-and-region-support) for details.
...
Before you begin, complete [Get started](#get-started). This scenario also requires an [Application Insights resource connected to your Foundry project](/observability/trace-agent-setup).
...
- An [Application Insights resource](https://learn.microsoft.com/azure/azure-monitor/app/app-insights-overview) connected to your Foundry project. See [Set up tracing in Microsoft Foundry](/observability/trace-agent-setup).
- The project's managed identity must have the **Log Analytics Reader** role on both the Application Insights resource and its linked Log Analytics workspace. If the tables that store your traces are [protected](https://learn.microsoft.com/azure/azure-monitor/logs/protected-tables-configure) (their protection level is set to **Protected**), also assign the [Privileged Monitoring Data Reader](https://learn.microsoft.com/azure/azure-monitor/logs/manage-access) role at the same scopes so the service can read the protected trace tables.
...
**Evaluator compatibility**: Each evaluator supports specific evaluation levels. Check the evaluator's `supported_evaluation_levels` field in the [evaluator catalog](/evaluation/evaluate-generative-ai-app).
...
- Check the [supported tools](/evaluation/agent-evaluators#supported-tools) for agent evaluators.
...
- [See evaluation results in the Foundry portal](/evaluation/evaluate-results)
