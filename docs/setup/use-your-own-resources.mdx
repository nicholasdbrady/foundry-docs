...
Starting in May 2025, the Azure AI Agent Service uses an endpoint for [Foundry projects](/what-is-microsoft-foundry?/what-is-foundry#choosing-a-project) instead of the connection string that was used for hub-based projects before this time. Connection strings are no longer supported in current versions of the SDKs and REST API. We recommend creating a new foundry project.
...
To use the [Grounding with Bing Search tool](/agent-service/bing-tools) the following provider must be registered: `Microsoft.Bing`
...
* Learn about the different [tools](/agent-service/tool-catalog) agents can use.
