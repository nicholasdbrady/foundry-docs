...
sidebarTitle: "New Microsoft Foundry portal general availability overview"
---
...
- Confirm required role assignments for your teams and service identities. For role details, see [Role-based access control for Microsoft Foundry](/security/rbac-foundry).
- Define how your organization restricts preview feature access in production environments. For guidance, see [Disable preview features in Microsoft Foundry](/security/disable-preview-features).
...
- **Standalone Azure OpenAI resources**: Not supported in the new Foundry portal. Continue using Foundry (classic) portal, or upgrade to a Foundry project. For upgrade guidance, see [Upgrade Azure OpenAI to Microsoft Foundry](/setup/upgrade-azure-openai).
...
Confirm that your target regions support the models and features you need. For region details, see [Feature availability across cloud regions](/foundry-models/region-support).
...
| Discover | Model | GA |
| Discover | [Instant Models](/foundry-models/instant-models) | Preview |
| Discover | Tools | GA |
| Discover | [Solution Templates](/developer-experience/ai-template-get-started) | GA |
| Discover | Search | GA |
| Discover | Ask AI | Preview |
| Build | [Agents](/agent-service/overview) (core) | GA |
| Build | Agents — Voice Live | Preview |
| Build | Agents — traces in agent builder | Preview |
| Build | [Workflows](/agent-service/workflow) | Preview |
| Build | [Models](/foundry-models/foundry-models-overview) | GA |
| Build | [Tracing](/agent-service/trace-agent-concept) (including Trace Replay) | Partial GA (GA for prompt agents; Preview for hosted, workflow and external agents) |
| Build | Tracing VNet | Preview |
| Build | Optimization (cluster analysis) | Preview |
| Build | [Fine-tuning](/foundry-models/fine-tuning) | GA |
| Build | Tools | GA (check label on individual tools in the catalog to determine if they are GA or Preview) |
| Build | [Knowledge (Foundry IQ)](/agent-service/what-is-foundry-iq) | Partial GA (API-level GA; portal access remains Preview) |
| Build | Data (core) | GA |
| Build | Data — stored completions | Preview |
| Build | [Evaluations](/observability/evaluate-generative-ai-app) | GA (some evaluators and features are Preview; check individual evaluator labels) |
| Build | Memory | Preview |
| Build | [Guardrails](/guardrails/guardrails-overview) — Models | GA |
| Build | [Guardrails](/guardrails/guardrails-overview) — Agents | Preview |
| Build | Guardrails — Controls and intervention | Preview |
| Build | [Monitoring](/observability/observability) | Preview |
| Build | [Red teaming](/best-practices/red-teaming) | GA |
| Build | AI services speech playgrounds | GA |
| Operate | Overview | Preview |
| Operate | Assets | Preview |
| Operate | Compliance | Preview |
| Operate | [Quota](/foundry-models/quota) | GA |
| Operate | Admin | GA |
| Docs | All | GA |
...
For upgrade guidance, see [Upgrade Azure OpenAI to Microsoft Foundry](/setup/upgrade-azure-openai).
...
To get started, see [Microsoft Foundry SDKs](/api-sdk/sdk-overview) and [Microsoft Foundry API](https://learn.microsoft.com/rest/api/aifoundry/).
...
- Target-region model and feature availability are confirmed in [Feature availability across cloud regions](/foundry-models/region-support).
...
- Assuming API key authentication provides the same governance granularity as Entra ID with RBAC. See [Role-based access control for Microsoft Foundry](/security/rbac-foundry) for proper configuration.
- Skipping region availability validation for required models and services. See [Feature availability across cloud regions](/foundry-models/region-support).
- Migrating assistants or AOAI workflows without a documented fallback path in Foundry (classic) portal. See [Migrate to the new Foundry Agent Service](/agent-service/migrate).
- Assuming all GA features work behind a virtual network. Some features, including Traces and Workflow Agents, don't yet fully support network isolation. Hosted Agents require public access to Azure Container Registry. If your workload requires private networking, review the feature limitations table in [How to configure network isolation for Microsoft Foundry](/security/configure-private-link#foundry-feature-limitations).

## Next steps

- [What is Microsoft Foundry?](/what-is-microsoft-foundry/what-is-foundry)
- [Upgrade Azure OpenAI to Microsoft Foundry](/setup/upgrade-azure-openai)
...
- [Microsoft Foundry rollout across my organization](/setup/planning)
