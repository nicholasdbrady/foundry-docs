...
sidebarTitle: "Upgrade Azure OpenAI to Microsoft Foundry"
---
...
* Foundry model and feature availability [differs by region](/operate/region-support). For example, Agent service is [available](../agents/concepts/model-region-support.md) in select regions compared to Azure OpenAI service.
...
<Tabs>
  ...
  <Tab title="Azure portal">
    ...
    <Frame>
  <img src="/images/azure-portal-upgrade.png" alt="Screenshot shows how to upgrade in Azure portal." />
</Frame>
  ...
  <Tab title="Azure Bicep">
    ...
    * Configure managed identity.
    ...
    Run the template by using [Azure Bicep CLI](https://learn.microsoft.com/azure/azure-resource-manager/bicep/bicep-cli) or your [Visual Studio Code extension for Bicep](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-bicep) as a patch operation on your current resource.

    **References**: [`Microsoft.CognitiveServices/accounts`](https://learn.microsoft.com/azure/templates/microsoft.cognitiveservices/accounts), [Bicep CLI documentation](https://learn.microsoft.com/azure/azure-resource-manager/bicep/bicep-cli).
...
</Note>   > The portal doesn't show all Foundry tools by default in the left side navigation. The navigation is customizable. Select *... More* to locate them.
...
If you plan to roll out non-OpenAI features gradually, update your RBAC role assignments, Azure Policy definitions, and any custom roles before you perform the upgrade. For details on permissions, see [Role Based Access Control](/security/rbac-foundry).
...
|---|---|---|
|[Cognitive Services User (RBAC role)](https://learn.microsoft.com/azure/role-based-access-control/built-in-roles/ai-machine-learning#cognitive-services-user)|OpenAI features|All Foundry features |
|[Cognitive Services OpenAI User (RBAC role)](https://learn.microsoft.com/azure/role-based-access-control/built-in-roles/ai-machine-learning#cognitive-services-openai-user)|OpenAI features|OpenAI features|
|[Custom RBAC roles](https://learn.microsoft.com/azure/role-based-access-control/custom-roles)|Only features you defined|Only features you defined|
|Model access (no policy applied)|OpenAI models|Any Foundry model|
|[Model access (policy enforced)](/security/model-deployment-policy)|Only OpenAI models allowed by policy|Only models you allow through policy|
...
* If you're using Azure DNS, create an [Azure DNS Zone](https://learn.microsoft.com/azure/dns/dns-zones-records) for each of the above domains.

* If you're using a custom DNS implementation, implement a conditional forwarder for each of the above configurations. 

After you configure DNS, update or alternatively [delete and re-create a private link endpoint](/security/configure-private-link) on your resource. Your private link endpoint creates an IP address in your Azure Virtual Network to map to each endpoint.
...
<Tabs>
  <Tab title="Foundry portal">
    ...
    **Option 1: Use Foundry portal**
    ...
    <Frame>
  <img src="/images/rollback.png" alt="Screenshot shows the rollback option in the Foundry portal." />
</Frame>
  ...
  <Tab title="Azure portal">
    ...
    <Frame>
  <img src="/images/rollback-azure-portal.png" alt="Screenshot shows how to roll back in Azure portal." />
</Frame>
...
If you're not sure who upgraded your resource to Foundry, you can [view the activity log in the Azure portal](https://learn.microsoft.com/azure/azure-monitor/platform/activity-log-insights#view-the-activity-log) to find out when the upgrade operation took place and which user performed it:
