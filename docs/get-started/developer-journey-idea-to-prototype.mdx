...
- A SharePoint connection configured in your project.
  ...
  - The SharePoint site and Foundry project must be in the same Microsoft Entra tenant. An agent can use one SharePoint tool. For setup instructions, see [Use the SharePoint tool](/toolboxes/sharepoint).
...
The Python cloud evaluation requires the **Foundry User** role, a deployed GPT model that supports chat completions, and a region that supports the evaluators you select. Review [supported regions](/evaluation/risk-safety-evaluators#foundry-project-configuration-and-region-support) before you continue.
...
- Add more data sources ([Azure AI Search](/toolboxes/ai-search), [other sources](../how-to/connections-add)).
- Implement advanced evaluation methods ([AI-assisted evaluation](/evaluation/evaluate-agent)).
- Create [custom tools](/toolboxes/private-tool-catalog) for business-specific operations.
...
- [SharePoint tool documentation](/toolboxes/sharepoint)
- [MCP tool integration](/toolboxes/model-context-protocol).
- [Multi-agent patterns](/toolboxes/agent-to-agent).
