...
**Key migration dates:**
- **August 26, 2026** &mdash; `azure-ai-inference` package retires. [Migrate to the `openai` package](./model-inference-to-openai-migration).
- **August 26, 2026** &mdash; Assistants API sunsets.  Use the generally available [Microsoft Foundry Agents service](/agents/overview). Follow the [migration guide](/agents/migrate#migrate-classic-agents-to-new-agents) to update your workloads. [Learn more](/agents/migrate).
...
1. **Migrate agents to the Responses API.** Rewrite agents that use the Assistants API to use the [Responses API](/models/generate-responses) before the August 2026 sunset.
1. **Verify your Foundry resource region supports the Responses API.** The Responses API and Foundry Agent Service aren't available in every Azure region. If your Foundry resource is in an unsupported region, agents and other Responses API features don't work in the current portal. Check the [Responses API region availability](/models/responses#supported-regions) list before migrating.
...
The Responses API isn't available in all Azure regions. If your Foundry resource is in an unsupported region, you can't create or run agents in the current Foundry portal. Before migrating, verify your resource is in a [supported region](/models/responses#supported-regions). If it isn't, create a new Foundry resource in a supported region.
...
| Build agents | **Agents** in the left pane | **Build** > [**Agents**](/agents/quickstart-hosted-agent) |
| Browse the model catalog | **Model catalog** in the left pane | **Discover** > **Model catalog** |
| View evaluations | **Evaluation** in the left pane | **Build** > [**Evaluations**](/evaluation/evaluate-generative-ai-app) |
| Fine-tune a model | **Fine-tuning** in the left pane | **Build** > [**Fine-tuning**](/models/fine-tuning) |
...
| Agents unavailable in the current portal | Foundry resource is in a region that doesn't support the Responses API | Create a Foundry resource in a [supported region](/models/responses#supported-regions) |

## Related content

- [What is Microsoft Foundry?](/what-is-microsoft-foundry/what-is-foundry)
- [Get started with Foundry Agent Service](/agents/quickstart-hosted-agent)
- [Develop with the Responses API](/models/responses)
- [Deploy models from the model catalog](/models/deploy-foundry-models)
- [Use a screen reader with Microsoft Foundry](../tutorials/screen-reader)
