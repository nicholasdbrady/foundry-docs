...
- An Azure subscription with a valid payment method. If you don't have an Azure subscription, create a [paid Azure account](https://azure.microsoft.com/pricing/purchase-options/azure-account?cid=msft_learn) to begin. If you're using GitHub Models, you can [upgrade from GitHub Models to Microsoft Foundry Models](/foundry-models/quickstart-github-models) and create an Azure subscription in the process.
...
Reasoning might generate longer responses and consume a larger number of tokens. DeepSeek-R1 supports up to 5,000 requests per minute and 5,000,000 tokens per minute. See the [rate limits](/foundry-models/quotas-limits) that apply to DeepSeek-R1 models. Consider having a retry strategy to handle rate limits. You can also [request increases to the default limits](/foundry-models/quotas-limits#request-increases-to-the-default-limits).
...
- **Monitor token usage.** DeepSeek-R1 reasoning content (within `<think>` tags) counts toward your token limit. See [quotas and limits](/foundry-models/quotas-limits) for the current rate limits.
- **Request quota increases.** If you consistently hit rate limits, [request increases to the default limits](/foundry-models/quotas-limits#request-increases-to-the-default-limits).
...
- [Use chat reasoning models](/foundry-models/use-chat-reasoning)
