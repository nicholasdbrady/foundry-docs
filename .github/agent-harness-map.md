...
| Repo-local truth read | Pass | `.github/copilot-instructions.md`, `.github/agents/agentic-workflows.md`, `.github/workflows/*.md`, `.github/workflows/shared/**`, `.mcp.json`, `fastmcp.json`, `pyproject.toml`, `README.md`, `foundry_docs_mcp/_server_factory.py`, `scripts/run_testbench.py`, `scripts/run_docs_eval.py`, `scripts/run_abcd_eval.py`, `scripts/scrape_model_catalog.py`, `tests/search_testbench.json`, and `tests/docs_eval_scenarios.json`. |
| Current first-party docs retrieved | Pass | Mintlify MCP/AI-readiness docs, FastMCP client/server testing docs, and GitHub gh-aw v0.81.6 workflow docs were retrieved (refreshed by the #479/#480 mechanical fleet upgrade; originally v0.67.1 when this map was first written). |
...
| gh-aw compilation and safety | gh-aw workflows are Markdown plus YAML frontmatter and must compile to `.lock.yml`; write permissions are not allowed in workflow permissions and GitHub writes should use `safe-outputs`. Source: <https://github.com/github/gh-aw/blob/main/.github/aw/github-agentic-workflows.md> (current gh-aw docs; fleet compiler is v0.81.6 as of the #479 mechanical upgrade). | Any workflow source change must compile, and mutating outputs should remain in `safe-outputs`, not raw `gh` or write-scoped workflow permissions. |
| gh-aw deterministic boundaries | gh-aw custom `steps`/`jobs` run outside the Agent Workflow Firewall and should be deterministic preparation/cleanup, not agentic compute. `network` and `tools` frontmatter define allowed egress and tools; strict mode must stay enabled. Source: <https://github.com/github/gh-aw/blob/main/.github/aw/github-agentic-workflows.md>. | Move data collection/diff generation into deterministic scripts or custom steps; keep agent prompts bounded to summarization and decisions. |
...
| `.github/agents/agentic-workflows.md` | Copilot custom agent | gh-aw dispatcher and workflow-authoring adapter; references gh-aw prompts and constraints. Paired with `.github/skills/agentic-workflows/SKILL.md` and `.github/skills/agentic-workflow-designer/SKILL.md` (dispatcher/designer skills, current gh-aw template as of the #479 upgrade). |
...
| `.mcp.json` | Repo MCP client config | Local/client MCP connection hints. Configured servers include `microsoft-foundry-docs-vnext` at the hosted Mintlify `/mcp` endpoint and Microsoft Web IQ at `https://api.microsoft.ai/v3/mcp`, with its `x-apikey` supplied through secure user input. Unattended workflows must use the repository `WEBIQ_API_KEY` Actions secret through an explicit workflow-local MCP wrapper. |
...
| #477-#481 Mechanical gh-aw v0.81.6 fleet upgrade | Copilot implementation session running vendor `gh aw upgrade`/`gh aw init`/`gh aw compile` commands directly; no custom codemods authored in-repo | Sequential: preflight (#478) -> apply (#479) -> refresh local guidance (#480, this row) -> package evidence (#481) | Allow editing all `.lock.yml` files, `.github/aw/actions-lock.json`, the generated dispatcher agent/skill files, and `.github/workflows/agentics-maintenance.yml` — but only via the vendor CLI's own commands, not hand-authored YAML. Manual edits are restricted to non-generated files the CLI doesn't own (e.g. `copilot-setup-steps.yml`, `docs-eval-harness.yml`) and to minimal, warning-fix-only prompt edits (no behavior redesign). | Fleet compiles under one consistent gh-aw version with 0 errors; only pre-existing, out-of-scope advisories (e.g. slash-command centralization) may remain as warnings; no `docs/`/`docs-vnext/` content, test, or Python script changes. | Code-review verifier diffs against the correct merge-base and checks for permission/scope drift, exactly as done for PR #509. |

### Scope note: mechanical gh-aw upgrades vs. behavioral hardening

The gh-aw fleet-upgrade tranche (#477-#481) is intentionally scoped to **mechanical compiler/runtime currency only** — running the vendor's own `gh aw upgrade`/`gh aw init`/`gh aw compile` commands, updating pinned Action versions, and fixing only the compile warnings/errors those commands themselves surface. It must never be combined with, or used as cover for, behavioral changes to workflow prompts, validators, report schemas, AI Credit budgets, or experiments — those remain separately tracked, unstarted follow-on work (see `Workflow and automation inventory` above for per-workflow risk notes). In particular, **`docs-noob-tester.md` hardening (making it `strict: true`, adding an evidence validator, replacing wildcard bash) is explicitly deferred** and is not implemented by the gh-aw fleet-upgrade tranche — it is the first prioritized behavioral follow-up once the fleet is on a stable, uniform gh-aw baseline.
