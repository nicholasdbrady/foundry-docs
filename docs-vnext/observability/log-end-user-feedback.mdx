...
- A Foundry project with an [Application Insights resource](https://learn.microsoft.com/azure/azure-monitor/app/app-insights-overview) connected. See [Set up tracing](/agent-service/trace-agent-setup).
- OpenTelemetry instrumentation configured in your application. See [Set up tracing in Microsoft Foundry](/agent-service/trace-agent-setup) for setup instructions.
