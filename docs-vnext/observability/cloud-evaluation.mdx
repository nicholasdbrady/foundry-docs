...
Use cloud evaluations for most scenarios - especially when testing at scale, integrating evaluations into continuous integration and continuous delivery (CI/CD) pipelines, or performing predeployment testing. Running evaluations in the cloud eliminates the need to manage local compute infrastructure and supports large-scale, automated testing workflows. You can also [schedule evaluations](/agent-service/how-to-monitor-agents-dashboard) to run on a recurring basis, or set up [continuous evaluation](/agent-service/how-to-monitor-agents-dashboard#) to automatically evaluate sampled agent responses in production.
...
| **[Red team evaluation](/observability/run-ai-red-teaming-cloud)** | You want automated adversarial testing - Foundry generates jailbreaks and harmful-content prompts and scores how the target responds. | `azure_ai_red_team` → `azure_ai_model` or `azure_ai_agent` |
...
For an overview of available evaluators and how to pick them, see [built-in evaluators](/observability/observability#what-are-evaluators) and [custom evaluators](/observability/custom-evaluators).
...
Send queries to a Foundry agent at runtime and evaluate the responses by using the `azure_ai_target_completions` data source type with an `azure_ai_agent` target. This scenario works for both [prompt agents](/agent-service/overview) and [hosted agents](/agent-service/hosted-agents).
...
Before you begin, complete [Get started](#get-started). This scenario also requires an [Application Insights resource connected to your Foundry project](/agent-service/trace-agent-setup).
...
- An [Application Insights resource](https://learn.microsoft.com/azure/azure-monitor/app/app-insights-overview) connected to your Foundry project. See [Set up tracing in Microsoft Foundry](/agent-service/trace-agent-setup).
...
**Evaluator compatibility**: Each evaluator supports specific evaluation levels. Check the evaluator's `supported_evaluation_levels` field in the [evaluator catalog](/observability/evaluate-generative-ai-app).
...
- [Set up continuous evaluation](/agent-service/how-to-monitor-agents-dashboard#set-up-continuous-evaluation)
