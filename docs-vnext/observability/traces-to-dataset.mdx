...
- Set up tracing for a deployed agent that emits traces. Foundry agents emit traces automatically, and OpenTelemetry-instrumented third-party agents are also supported. For setup steps, see [Set up tracing for your agent](/agents/development/trace-agent-setup).
...
For the full evaluation flow, including selecting evaluators and reviewing results, see [Run cloud evaluations](/observability/cloud-evaluation).
...
- [Generate a synthetic evaluation dataset](/observability/evaluation-dataset-synthetic) — bootstrap an evaluation dataset without production traces.
- [Agent tracing in Microsoft Foundry](/agents/development/trace-agent-concept)
