...
---
This article provides guidance on migrating your applications from the Azure AI Inference SDK to the OpenAI SDK. The OpenAI SDK provides broader model support (Azure OpenAI, DeepSeek, Grok, and other Foundry Models), a unified API surface across providers, built-in support for both API key and Microsoft Entra ID authentication, and implicit API versioning that removes the need to manually specify `api-version` parameters.
...
<Note>
Azure AI Inference SDK for Go does not have equivalent wrapper APIs. The examples in this section show only the OpenAI SDK.
</Note>
...
The key differences are covered in the [comparison table](#key-differences). Practically:

- The `model` parameter is always required in the OpenAI SDK (set it to your deployment name). In the Azure AI Inference SDK, it is optional for single-model endpoints.
- Both SDKs return similar response structures: `choices[].message.content`, `choices[].finish_reason`, and token usage. The class name differs — `ChatCompletions` in the Inference SDK, `ChatCompletion` in the OpenAI SDK.
...
<Check>Update request method calls (`complete` → `chat.completions.create`)</Check>
...
Verify your API key is not expired. For Microsoft Entra ID, ensure your application has the required permissions and the credential scope is `https://cognitiveservices.azure.com/.default`.

### Endpoint errors

Verify the endpoint URL includes `/openai/v1/` and your resource name is correct. Confirm the model deployment exists and is active.

### Model not found errors

Ensure you use your deployment name (not the base model name), that it matches exactly (case-sensitive), and that it is active in your Foundry resource.
