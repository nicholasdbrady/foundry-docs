...
- [Define AI platform sharing boundaries](https://learn.microsoft.com/azure/cloud-adoption-framework/ai/platform/ai-platform-sharing-isolation-colocation#1-define-ai-platform-sharing-boundaries)
...
|---|---|---|
| Identity and access | Define admin, project manager, and project user personas. Map each persona to least-privilege roles and Microsoft Entra ID groups. | [Role-based access control in Foundry](/security/rbac-foundry) |
| Networking | Choose the network model per environment. Use managed virtual network for a more secure, straightforward setup. Use bring-your-own (BYO) virtual network for advanced network control and custom routing requirements. Validate private DNS and endpoint approval flow before production. | [Configure managed virtual network](/security/managed-virtual-network), [Configure private link for Foundry](/security/configure-private-link), and [Network-secured setup (BYO virtual network)](/setup/virtual-networks) |
| Data protection and keys | Decide whether Microsoft-managed keys meet policy requirements or whether customer-managed keys are required. | [Customer-managed keys in Foundry](/security/encryption-keys-portal) |
| Authentication model | Prefer Microsoft Entra ID and RBAC for people and services. Use API keys only where role granularity isn't required. | [Role-based access control in Foundry](/security/rbac-foundry) |
...
- [Models sold directly by Azure](/foundry-models/models-sold-directly-by-azure)
- [Models from partners](/foundry-models/models-from-partners)
- [Quota in Foundry](/foundry-models/quota)
- [Quotas and limits for Foundry models](/foundry-models/quotas-limits)
...
Use [Add connections in Foundry](/setup/connections-add) to standardize
connection setup.
...
- [Quickstart: Deploy a Microsoft Foundry resource by using a Bicep file](/setup/create-resource-template)
...
- [Model deployment policy in Foundry](/security/model-deployment-policy)
- [Manage costs in Foundry](/foundry-models/manage-costs)
- [Agent 365 integration](/agent-service/agent-365-integration)
...
[Role-based access control in Foundry](/security/rbac-foundry).
...
- [Monitoring across fleets in Foundry](/manage/monitoring-across-fleet)
- [Agent 365 integration](/agent-service/agent-365-integration)
...
<Frame>
  <img src="/images/sample-platform-deployment.svg" alt="Diagram showing Contoso use cases moving from a shared exploration Foundry environment into isolated or co-located dev/test environments, and then into isolated prod environments for a smaller number of workloads." />
</Frame>
