...
The process of transforming a user's raw search query into a retrieval-optimized query before issuing it to a search backend. The `FoundryProjectOpenAI.rewrite_query_for_search` method in `foundry_docs_mcp/foundry_client.py` uses the OpenAI Responses API to expand technical terms and synonyms while preserving intent, improving recall from the Azure AI Search hybrid index.
...
The OpenAI API endpoint (`responses.create`) for single-turn text generation. Used by `FoundryProjectOpenAI` in `foundry_docs_mcp/foundry_client.py` to perform query rewriting: the Responses API rewrites a user's natural-language search query into a concise, retrieval-optimized form before it is sent to Azure AI Search.

## S

### Search Feedback

Structured telemetry events emitted when search results are insufficient or incorrect. The `submit_search_feedback` MCP tool records the original user request, query attempted, result paths returned, expected result, and proposed resolution to `telemetry/feedback.jsonl`. This feedback is consumed by `scripts/build_testbench.py` to generate evaluation test cases.

### Search Testbench

A JSON file (`tests/search_testbench.json`) containing query-to-expected-path test cases for evaluating documentation search quality. Built from search feedback events by `scripts/build_testbench.py` and executed by `scripts/run_testbench.py` and `scripts/run_abcd_eval.py`.
