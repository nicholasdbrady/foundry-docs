...
If you already have an existing agent project, skip this step and see [Make your agent optimizer-ready](/agent-service/make-agent-optimizer-ready) to add optimization support.
...
For details, see [Create an evaluation dataset](/agent-service/create-optimizer-dataset).
...
For more details on agent targeting, see [Which agent gets optimized](/agent-service/optimize-agent-targets#which-agent-gets-optimized).
...
The *eval model* scores each response (any chat-completion model works). The *optimization model* generates improved candidates and must be from the [supported list](/agent-service/agent-optimizer-overview#models) (gpt-5 family or DeepSeek).
