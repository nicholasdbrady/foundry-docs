...
- The `azd ai agent` extension installed (`azd extension install azure.ai.agents`). If you don't have the extension installed, when you initialize the starter template or run `azd ai agent` the extension is installed automatically. To learn more about the `azd` AI agent extension see, [Microsoft Foundry agent extension](https://learn.microsoft.com/azure/developer/azure-developer-cli/extensions/azure-ai-foundry-extension)
...
- The `Foundry User` role on the Foundry resource (previously named `Azure AI User`). For more information, see [Role-based access control for Microsoft Foundry](/security/rbac-foundry).
...
- `builtin.<name>` — references a [built-in evaluator](/observability/built-in-evaluators) provided by Foundry.
- `<name>` — references a [custom evaluator](/observability/custom-evaluators) registered in the Foundry project. Use the evaluator's registered name without the version suffix.
...
- The agent preparation required by the optimizer is complete. For optimizer prerequisites and agent preparation requirements, see [Optimize agent prompts with Prompt Optimizer](/agent-service/prompt-optimizer).
...
- [Human evaluation for Microsoft Foundry agents](/observability/human-evaluation)
- [Evaluation cluster analysis](/observability/cluster-analysis)
- [Optimize agent prompts with Prompt Optimizer](/agent-service/prompt-optimizer)
- [Set up tracing for AI agents in Microsoft Foundry](/agent-service/trace-agent-setup)
