...
- A [Foundry project](/setup/create-projects) with a deployed hosted agent
- The `azure.ai.agents` CLI extension installed (see [Quickstart: Optimize a hosted agent](/agent-service/quickstart-optimize-hosted-agent))
- A model deployed for evaluation (for example, `gpt-4.1-mini`) and an optimization model from the [supported list](/agent-service/agent-optimizer-overview#models) (for example, `gpt-5.1`)
- Your agent is [optimizer-ready](/agent-service/make-agent-optimizer-ready) (calls `load_config()`)
...
For more details on how these models are used, see [Models](/agent-service/agent-optimizer-overview#models).
...
After optimization completes, review the results table. For detailed scoring guidance, see [Understand optimization results](/agent-service/agent-optimizer-overview#understand-optimization-results).
...
- [Agent optimizer overview](/agent-service/agent-optimizer-overview)
- [Create a custom evaluation dataset](/agent-service/create-optimizer-dataset)
