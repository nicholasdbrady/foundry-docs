...
For a managed knowledge base experience, see [Foundry IQ](/agent-service/foundry-iq-connect). For tool optimization, see [best practices](/agent-service/tool-best-practice).
...
Select **Prompt Agents** to use the Azure AI Projects SDK to create a server-side prompt agent, or **Hosted Agents** to use the Agent Framework [`FoundryChatClient`](/agent-service/responses-api) to build an ephemeral, in-process agent.
...
The quickest way to create a connection is through the Foundry portal. For all connection methods and supported connection types, see [Add a new connection to your project](/setup/connections-add).
...
<Tabs>
  <Tab title="Foundry portal">
    ...
    For more detailed steps, see [Add a new connection to your project](/setup/connections-add).
...
Don't put real keys in source control. Store secrets in a secure store (for example, Azure Key Vault) and inject them at deployment time.
    ...
    For the full API specification, see [Project Connections REST API reference](https://learn.microsoft.com/rest/api/aifoundry/accountmanagement/project-connections).
  ...
  <Tab title="Bicep">
    ...
    For more information about deploying connections with Bicep, see [Add a new connection to your project](/setup/connections-add).
...
| Tool can't access the index (401/403) | Missing RBAC roles (keyless auth) | Assign the **Search Index Data Contributor** and **Search Service Contributor** roles to the Foundry project's managed identity. See [Azure RBAC in Foundry](/security/rbac-foundry). |
...
- [Add a new connection to your project](/setup/connections-add)
- [Project Connections REST API reference](https://learn.microsoft.com/rest/api/aifoundry/accountmanagement/project-connections)
