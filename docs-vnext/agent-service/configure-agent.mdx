...
- [Publish an agent as a digital worker in Agent 365](/agent-service/agent-365)

<Note>

If you're migrating from the previous publishing model, see [Migrate from Agent Applications to the new agent model](/agent-service/migrate-agent-applications).
...
- [Foundry User role](/security/rbac-foundry) on the Foundry project scope to create, manage, and invoke agents
...
<Frame>
  <img src="/images/agent-object-model.png" alt="Diagram illustrating how Foundry projects organize agent versions and agents." />
</Frame>
...
To enable the A2A protocol on your agent, see [Enable incoming A2A on a Foundry agent](/agent-service/enable-agent-to-agent-endpoint).
...
| Publishing to M365/Teams fails | The agent doesn't have a unique identity (`agent.identity` is null) | See the [migration guide](/agent-service/migrate-agent-applications) for steps to resolve this. |
