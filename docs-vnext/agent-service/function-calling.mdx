...
- The SDK package for your language:
  ...
  For installation and authentication steps, see the [quickstart](/get-started/get-started-code).
...
Use the following code sample to create an agent, handle a function call, and return tool output back to the agent. Select **Prompt Agents** to use the Azure AI Projects SDK to create a server-side prompt agent, or **Hosted Agents** to use the Agent Framework [`FoundryChatClient`](/agent-service/responses-api) to build an ephemeral, in-process agent.
...
If you use tracing in Microsoft Foundry, confirm the tool invocation occurred. For guidance on validating tool invocation and controlling tool usage, see [Best practices for using tools in Microsoft Foundry Agent Service](/agent-service/tool-best-practice).
...
- [Connect OpenAPI tools to Microsoft Foundry agents](/agent-service/openapi)
