...
Foundry agent service is only available for Foundry projects created in regions that support the [Azure OpenAI Responses API](/foundry-models/responses#region-availability). Your Foundry project must be in one of these regions to use Agent Service. Some Azure OpenAI models may not be available in the same regions. See [Region availability for Foundry Models sold directly by Azure](/foundry-models/models-sold-directly-by-azure-region-availability) for details. 
