...
If you're deploying for the first time or want the fastest path, use the [Quickstart: Create and deploy a Hosted agent](/agent-service/quickstart-hosted-agent) instead. The quickstart uses the **Azure Developer CLI (azd)** or **VS Code extension**, which handle building, pushing, versioning, and RBAC configuration automatically.

<Tip>

Prefer a Docker-less inner loop? You can also [deploy a hosted agent directly from source code (preview)](/agent-service/deploy-hosted-agent-code) — upload a `.zip` of your Python or .NET code and the platform builds and hosts it for you.
...
* Agent code using a [supported framework](/agent-service/hosted-agents#language-support).
...
* While `azd` and VS Code extensions handle basic RBAC assignments automatically, complex scenarios may require additional manual configuration. For comprehensive details about all permissions and role assignments involved, see [Hosted agent permissions reference](/agent-service/hosted-agent-permissions).

</Note>

For more information, see [Authentication and authorization](/security/authentication-authorization-foundry).

<Info>

The Azure Container Registry that holds your Hosted agent's container image must currently be reachable over its public endpoint. Placing the registry behind a private network (private endpoint with public network access disabled) isn't currently supported for Hosted agents — the platform can't pull the image. For the full list of network constraints, see [Limitations](/setup/virtual-networks#limitations).
...
If you already have a Hosted agent that uses the **Responses** or **Invocations** protocol and you want to add real-time voice interaction without rewriting it as a WebSocket agent, see [Use Voice Live with hosted agents](../../../ai-services/speech-service/how-to-voice-live-hosted-agent-integration).
*/}
...
First, create a `CustomKeys` connection on the project that holds the secret. See [Add a new connection in Microsoft Foundry](/setup/connections-add). Then reference it from `agent.yaml`:
...
- **Foundry manages the backing secret name.** When you create the connection, Foundry stores the value in Key Vault under a name it chooses — you can't reference a preexisting Key Vault secret by name. To attach your own Key Vault as the backing store, see [Set up a Key Vault connection](/security/set-up-key-vault-connection).
...
The Azure Developer CLI (`azd`) and VS Code extension automate the full deployment lifecycle. For a step-by-step walkthrough, see the [Quickstart: Create and deploy a Hosted agent](/agent-service/quickstart-hosted-agent).
...
1. Assign the **Container Registry Repository Reader** role to this identity on your container registry. See [Azure Container Registry roles and permissions](https://learn.microsoft.com/azure/container-registry/container-registry-roles).
...
For detailed RBAC requirements and permission troubleshooting, see [Hosted agent permissions reference](/agent-service/hosted-agent-permissions).
...
- [Agent applications](/agent-service/agent-applications)

{/* - [Add voice to a Hosted agent with Voice Live](../../../ai-services/speech-service/how-to-voice-live-hosted-agent-integration) */}
