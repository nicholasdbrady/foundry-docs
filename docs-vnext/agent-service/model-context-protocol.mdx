...
- **Private endpoints**: Connect to MCP servers that aren't exposed to the public internet. Private MCP requires [Standard Agent Setup with private networking](/setup/virtual-networks) and a dedicated MCP subnet within your virtual network.
...
For details about tool support in network-isolated environments, see [Agent tools with network isolation](/security/configure-private-link#agent-tools-with-network-isolation).
...
To learn about supported authentication options, including key-based, Microsoft Entra identities, and OAuth identity passthrough, see [MCP server authentication](/agent-service/mcp-authentication).
...
For general guidance on tool usage, see [Best practices for using tools in Microsoft Foundry Agent Service](/agent-service/tool-best-practice).
...
The following example shows how to use the GitHub MCP server as a tool for an agent. Select **Prompt Agents** to use the Azure AI Projects SDK to create a server-side prompt agent, or **Hosted Agents** to use the Agent Framework [`FoundryChatClient`](/agent-service/responses-api) to build an ephemeral, in-process agent.
...
<Tabs>
  ...
  <Tab title="Hosted Agents">
    ...
    This sample uses [`FoundryChatClient`](/agent-service/responses-api) from the Microsoft Agent Framework and calls `get_mcp_tool()` to register a hosted MCP server with per-tool approval control. Install the package with `pip install agent-framework-foundry aiohttp`, set the `FOUNDRY_PROJECT_ENDPOINT` and `FOUNDRY_MODEL` environment variables, and sign in with `az login`.
...
- **Private MCP requires Standard Agent Setup**: Private MCP server connectivity is only available with [Standard Agent Setup with private networking](/setup/virtual-networks) (BYO VNet). Basic agent setup doesn't support private MCP endpoints.
...
- [MCP server authentication](/agent-service/mcp-authentication)
- [Build and register a Model Context Protocol (MCP) server](/agent-service/build-your-own-mcp-server)
- [Set up private networking for Foundry Agent Service](/setup/virtual-networks)
- [Configure private link for Foundry](/security/configure-private-link)
