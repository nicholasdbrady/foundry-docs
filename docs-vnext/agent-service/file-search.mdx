...
You can customize file search behavior at runtime, such as specifying which vector store to use per request, by using [structured inputs](/agent-service/structured-inputs).
...
The following code sample shows how to create an agent with the file search tool enabled. You need to upload files and create a vector store before running this code. Select **Prompt Agents** to use the Azure AI Projects SDK to create a server-side prompt agent, or **Hosted Agents** to use the Agent Framework [`FoundryChatClient`](/agent-service/responses-api) to build an ephemeral, in-process agent.
...
<Tabs>
  ...
  <Tab title="Hosted Agents">
    ...
    This sample uses [`FoundryChatClient`](/agent-service/responses-api) from the Microsoft Agent Framework. It uploads a file, creates a vector store, and calls `get_file_search_tool()` to give the agent access to the indexed content. Install the package with `pip install agent-framework-foundry aiohttp`, set the `FOUNDRY_PROJECT_ENDPOINT` and `FOUNDRY_MODEL` environment variables, and sign in with `az login`.
...
| -------- | ---------------- |
| Search existing Azure AI Search indexes | [Azure AI Search tool](/agent-service/ai-search) |
| Search the public web for current information | [Web search tool](/agent-service/web-search) |
...
For background concepts and lifecycle guidance (readiness, deletion behavior, and expiration policies), see [Vector stores for file search](/agent-service/vector-stores).
...
- Features and availability vary by region. See [Azure AI Foundry region support](/foundry-models/region-support).
...
- [Azure AI Search tool](/agent-service/ai-search) - Search existing Azure AI Search indexes from your agents
- [Web search tool](/agent-service/web-search) - Enable agents to search the public web
- [Vector stores for file search](/agent-service/vector-stores) - Understand vector store lifecycle and expiration
- [Structured inputs](/agent-service/structured-inputs) - Parameterize agent definitions at runtime
