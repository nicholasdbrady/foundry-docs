...
Start with the [web search tool](/agent-service/web-search). Learn more about the differences between web search and Grounding with Bing Search (or Grounding with Bing Custom Search) in the [web grounding overview](/agent-service/web-overview).
...
Not all models support Grounding with Bing Search or Grounding with Bing Custom Search. For a full list of models that support these tools, see [Tool support by region and model](/agent-service/tool-best-practice#tool-support-by-region-and-model).
...
- Azure RBAC roles:
  ...
  - **Foundry Project Manager** role to create project connections in Foundry. For more information, see [Role-based access control for Microsoft Foundry](/security/rbac-foundry).
...
1. Add the appropriate connection to your project.
   ...
   For step-by-step instructions, see [Add a new connection to your project](/setup/connections-add).
...
See [best practices](/agent-service/tool-best-practice) for information on optimizing tool usage.
...
<Tabs>
  ...
  <Tab title="Hosted Agents">
    ...
    This sample uses [`FoundryChatClient`](/agent-service/responses-api) from the Microsoft Agent Framework and calls `get_bing_grounding_tool()` to attach Bing grounding via a project connection. Install the package with `pip install agent-framework-foundry aiohttp`, set the `FOUNDRY_PROJECT_ENDPOINT` and `FOUNDRY_MODEL` environment variables, and sign in with `az login`.
...
<Frame>
  <img src="/images/website-citations.png" alt="A screenshot showing citations for Bing search results." />
</Frame>
...
- [Tool use best practices](/agent-service/tool-best-practice) - Learn optimization strategies for agent tools
- [Web search tool](/agent-service/web-search) - Use web search without configuring Bing tool parameters
- [Manage Grounding with Bing in Microsoft Foundry and Azure](/setup/manage-grounding-with-bing) - Control and disable Grounding with Bing features
- [Connect OpenAPI tools to agents](/agent-service/openapi) - Integrate custom APIs with your agents
- [Discover tools in the Foundry Tools (preview)](/agent-service/tool-catalog) - Explore all available agent tools in Foundry Agent Service
