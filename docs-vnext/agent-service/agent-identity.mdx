...
For Foundry-specific RBAC roles and scopes, see [Azure role-based access control in Foundry](/security/rbac-foundry).
...
azd does not configure Container Registry, Application Insights, or custom resource permissions. For production deployments and the complete permission requirements for hosted agents, see [Hosted agent permissions reference](/agent-service/hosted-agent-permissions).
...
When you [publish an agent](/agent-service/agent-applications), you must reassign RBAC permissions to the new agent identity for any resources that the agent needs to access. This reassignment ensures that the published agent maintains appropriate access while operating under its distinct identity.
...
For comprehensive details about all permissions involved in hosted agent deployment, including Azure Container Registry, Application Insights, and multi-resource RBAC configurations, see [Hosted agent permissions reference](/agent-service/hosted-agent-permissions).
...
* **Model Context Protocol (MCP)**: Use your agent's identity to authenticate with MCP servers that support agent identity authentication (preview). For details, see [Model Context Protocol](/agent-service/model-context-protocol) and [MCP server authentication](/agent-service/mcp-authentication).
* **Agent-to-Agent (A2A)**: Enable secure communication between agents by using agent identities (preview). For details, see [Agent-to-Agent tool](/agent-service/agent-to-agent) and [Agent2Agent (A2A) authentication](/agent-service/agent-to-agent-authentication).
...
- [Set up MCP server authentication](/agent-service/mcp-authentication#use-agent-identity-authentication-preview)
...
- **Missing role assignments**: Ensure the agent identity has the required RBAC role on the target resource. For Foundry roles and scopes, see [Azure role-based access control in Foundry](/security/rbac-foundry).
...
- [Model Context Protocol](/agent-service/model-context-protocol)
- [Agent-to-Agent tool](/agent-service/agent-to-agent)
...
* [Azure role-based access control in Foundry](/security/rbac-foundry)
