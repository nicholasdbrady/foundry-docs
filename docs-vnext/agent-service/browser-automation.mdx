...
You're responsible for reviewing and testing your applications and implementing your own responsible AI mitigations. By using the Browser Automation Tool, you are acknowledging that you bear responsibility and liability for any use of it and all outcomes. Use judgment in deciding which credentials you provide to your browser sessions. See the [Foundry Agent Service transparency note](https://learn.microsoft.com/azure/foundry/responsible-ai/agents/transparency-note).
...
Choose how the agent runs before configuring Browser Automation:

| Agent type | Use it when |
| --- | --- |
| **Prompt agent** | You want Foundry to run an agent defined by a model, instructions, and tools. |
| **Hosted agent with Microsoft Agent Framework** | You need custom code with Foundry-managed hosting, scaling, identity, and observability. |
| **Hosted agent with another framework** | You need to keep orchestration built with LangGraph, OpenAI Agents SDK, Semantic Kernel, or custom code. Connect the framework to the Browser Automation toolbox through its MCP client. |

## Choose an authentication type

| Authentication type | Use it when | Considerations |
| --- | --- | --- |
| **API key** | You are prototyping or your organization permits service access tokens. | Foundry stores the key in the project connection. Never put it in source code, prompts, or logs. Rotate it according to your organization's policy. |
| **Project managed identity** | Agents in one Foundry project share a Playwright workspace. | Grant the project identity only the permissions it needs. Use this option as the enterprise default when the connection supports it. |
| **Agent identity** | A hosted agent needs an access boundary separate from other agents in the project. | Grant the deployed agent's dedicated identity access to the downstream resources it needs. |

### Grant Playwright workspace access

Browser Automation uses the **Foundry project managed identity** to access the Playwright workspace, not the hosted agent's dedicated agent identity.

1. Open the Playwright workspace in the Azure portal.
1. Select **Access control (IAM)** > **Add** > **Add role assignment**.
1. Select a custom role containing only the required Playwright permissions. If no custom role is available, assign **Playwright Workspace Contributor** at the Playwright workspace resource scope.
1. Select the Foundry project managed identity, then select **Review + assign**.

## Monitor browser automation sessions

- **Live View** shows the active session while you develop and test.
- **Session recording** captures video, actions, logs, and metadata for completed sessions.
- **Take control** lets a person intervene before the agent continues.

For production investigations, use run summaries, traces, and diagnostic logs instead of continuously monitoring active sessions.
...
The .NET SDK is currently in preview. For more information, see the [quickstart](/get-started/get-started-code).
...
After you run a sample, verify the tool was called by using tracing in Microsoft Foundry. For guidance on validating tool invocation, see [Best practices for using tools in Microsoft Foundry Agent Service](/agent-service/tool-best-practice). If you use streaming, you can also look for `browser_automation_preview_call` events.
...
The following Python example demonstrates how to create an AI agent with browser automation capabilities. Select **Prompt Agents** to use the Azure AI Projects SDK to create a server-side prompt agent, or **Hosted Agents** to use the Agent Framework [`FoundryChatClient`](/agent-service/responses-api) to build an ephemeral, in-process agent.
...
This tool uses a Playwright workspace resource to run browser sessions. Review the Playwright workspace documentation for pricing and usage details. For guidance on optimizing tool usage, see [Best practices for using tools in Microsoft Foundry Agent Service](/agent-service/tool-best-practice).
...
- Use tracing in Microsoft Foundry to confirm whether a tool call occurred. For guidance, see [Best practices for using tools in Microsoft Foundry Agent Service](/agent-service/tool-best-practice).
...
- Remove the project connection if it’s no longer required. For more information, see [Add a connection in Microsoft Foundry](/setup/connections-add).

## Example scenarios

- **Form filling and integration**: Move information between browser-based applications when an API isn't available.
- **Research and data extraction**: Navigate sites, filter results, and compare information across sources.
- **Quality assurance**: Validate end-to-end flows and reproduce reported browser issues.
- **Legacy-system automation**: Automate repetitive interactions with internal or vendor portals.

## Transparency note

Review the [transparency note](/responsible-ai/transparency-note#enabling-autonomous-actions-with-or-without-human-input-through-action-tools) when using this tool. The Browser Automation tool is a tool that can perform real-world browser tasks through natural language prompts, enabling automated browsing activities without human intervention.

Review the [responsible AI considerations](/responsible-ai/transparency-note#considerations-when-choosing-a-use-case) when using this tool.

## Related content

- [Best practices for using tools in Microsoft Foundry Agent Service](/agent-service/tool-best-practice)
- [Computer use tool for agents](/agent-service/computer-use)
- [Add a connection in Microsoft Foundry](/setup/connections-add)
- [Quickstart: Create your first agent](/get-started/get-started-code)
