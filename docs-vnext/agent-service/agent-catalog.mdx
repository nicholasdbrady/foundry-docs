...
- A [model deployment](/foundry-models/create-model-deployments) (for example, `gpt-5-mini`).
...
<Frame>
<img src="/images/agents-catalog-browse.png" alt="Screenshot of the Foundry agent manifests showing 30 agent manifests with search, sort, and tool icons for each manifest." />
</Frame>
...
|---|---|
| **Tools** | The [tools](/agent-service/tool-catalog) the agent uses, such as web search, code interpreter, or SharePoint. Most manifests use one or two tools. |
...
|---|---|---|
| **[Web search](/agent-service/web-search)** | Built-in | Retrieves real-time information from the public web. Most versatile tool with the lowest setup friction. |
| **[Code Interpreter](/agent-service/code-interpreter)** | Built-in | Writes and runs Python code for data analysis, calculations, and chart generation. |
| **[SharePoint](/agent-service/sharepoint) (preview)** | Built-in | Searches and retrieves documents from your SharePoint sites. |
| **[GitHub](/agent-service/model-context-protocol) (via MCP)** | Custom | Accesses repositories, pull requests, issues, and code for developer workflows. Connected through an MCP server. |
| **[Grounding with Bing Search](/agent-service/bing-tools)** | Built-in | Retrieves cited, verifiable facts from the web with source attribution. |
| **[Azure AI Search](/agent-service/ai-search)** | Built-in | Queries knowledge indexes for grounded, domain-specific answers. |
| **[Microsoft Fabric](/agent-service/fabric) (preview)** | Built-in | Connects to your enterprise data warehouse for analytics. |
| **[File Search](/agent-service/file-search)** | Built-in | Analyzes uploaded documents using vector search. |
| **[Browser Automation](/agent-service/browser-automation) (preview)** | Built-in | Interacts with web UIs through natural language prompts. |
| **[OpenAPI tool](/agent-service/openapi)** | Custom | Calls external APIs using an OpenAPI specification. |
...
For more information about configuring tools, see [Agent tools overview](/agent-service/tool-catalog).
...
1. Test your agent in the [agents playground](/foundry-models/concept-playgrounds).
1. When you're satisfied with the results, [publish your agent](/agent-service/agent-applications).
...
- [Quickstart: Create an agent from a manifest](/agent-service/quickstart-agent-catalog)
...
- [Transparency note for Azure Agent Service](/responsible-ai/transparency-note)
