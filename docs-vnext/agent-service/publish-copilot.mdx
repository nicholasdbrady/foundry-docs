...
If you're migrating from a previous publishing model, see [Migrate from Agent Applications to the new agent model](/agent-service/migrate-hosted-agent-preview).
...
For more information on selecting an active agent version and configuring other agent settings, see [Publish and share agents](/agent-service/configure-agent).
...
<Frame>
<img src="/images/publish-dropdown.png" alt="Expanded publish dropdown showing options" />
</Frame>
...
<Frame>
<img src="/images/active-version.png" alt="Screenshot of the active version popup showing Always use latest and specific version options." />
</Frame>
...
<Frame>
<img src="/images/agent-store.png" alt="A screenshot of the Agent store in Microsoft 365 Copilot." />
</Frame>
...
| Publishing fails with identity error | The agent doesn't have a unique identity (`agent.identity` is null) | See the [migration guide](/agent-service/migrate-agent-applications) for steps to resolve this. |
...
- [Configure your agent endpoint and settings](/agent-service/configure-agent)
