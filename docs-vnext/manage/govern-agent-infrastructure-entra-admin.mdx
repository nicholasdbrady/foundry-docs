...
The guidance here focuses on infrastructure-level governance of agents built with [Foundry Agent Service](/agents/development/overview) as a fallback for situations that require direct administrative action.
...
- One of the following Microsoft Entra role assignments:
  - [Microsoft Entra Global Administrator](https://learn.microsoft.com/entra/identity/role-based-access-control/permissions-reference#global-administrator)
  - [Microsoft Entra AI Administrator](https://learn.microsoft.com/entra/identity/role-based-access-control/permissions-reference#ai-administrator)
- If your organization uses [Privileged Identity Management (PIM)](https://learn.microsoft.com/entra/id-governance/privileged-identity-management/pim-configure), activate your role assignment before proceeding.

<Info>

**AI Administrators**: You can't perform the [access elevation procedure](#elevate-access-to-manage-azure-subscriptions) yourself. You should coordinate with the agent owner first, who can either:
...
Developers create **Foundry agents** within a project. Each agent gets a unique identity and endpoint for interaction. Foundry automatically registers each agent with the [Agent 365 registry](https://learn.microsoft.com/microsoft-365/admin/manage/agent-registry) using the agent's unique identity.
...
[Prompt agents](/agents/development/prompt-agent) can also use a common endpoint that serves all agents in the project. Agents that run this way share the project's infrastructure and identity. Don't use project endpoints for production. Foundry doesn't create more registrations in Agent 365 for project endpoints or project identities.
...
For a full description of agent lifecycle operations, see [Manage agents in Foundry Control Plane](/manage/how-to-manage-agents). To learn more about Foundry concepts referenced in this section, see:

- [Microsoft Foundry architecture](/security/architecture) for how Foundry resources map to Azure resources.
- [Agent identity concepts](/agents/development/agent-identity) for how agent identities work in Microsoft Entra ID.
- [Plan and manage Foundry](/setup/planning) for subscription and resource group organization.
...
- [**Stop** and **Start**](/manage/how-to-manage-agents#start-and-stop-agents) operate on individual deployments by deallocating or provisioning compute. They affect the underlying Azure infrastructure and make the agent unavailable across all channels (Teams, Microsoft 365 Copilot, Foundry, APIs).
...
The full resource ID pattern shares a common prefix: `/subscriptions/{sub-id}/resourceGroups/{group}/providers/Microsoft.CognitiveServices/accounts/{account-name}/projects/{project-name}/...`. The key differentiator is the segment after the project: either `agents/{agent-name}` or `applications/{application-name}`.
...
Check the [Activity Log](https://learn.microsoft.com/azure/azure-monitor/platform/activity-log) for the resource. The log shows who took management actions recently. This approach helps you identify current resource managers, even if they don't have privileged role assignments.
...
For the full procedure, see [Elevate access to manage all Azure subscriptions and management groups](https://learn.microsoft.com/azure/role-based-access-control/elevate-access-global-admin).
...
The Foundry RBAC roles were recently renamed. **Foundry User**, **Foundry Owner**, **Foundry Account Owner**, and **Foundry Project Manager** were previously named Azure AI User, Azure AI Owner, Azure AI Account Owner, and Azure AI Project Manager. You might still see the previous names in some places while the rename rolls out. The role IDs and core permissions are unchanged by the rename.
   ...
   **Azure CLI**:
   ...
   az role assignment list --assignee YOUR_EMAIL --scope RESOURCE_SCOPE
...
|---|---|---|
| View | [Foundry User](https://learn.microsoft.com/azure/role-based-access-control/built-in-roles/ai-machine-learning#azure-ai-user)<br />(**Reader** isn't sufficient) | [Reader](https://learn.microsoft.com/azure/role-based-access-control/built-in-roles/general#reader) |
| Stop/Start | _Not supported_ | [Foundry Owner](https://learn.microsoft.com/azure/role-based-access-control/built-in-roles/ai-machine-learning#azure-ai-owner) |
| Delete | [Foundry User](https://learn.microsoft.com/azure/role-based-access-control/built-in-roles/ai-machine-learning#azure-ai-user) | [Foundry Owner](https://learn.microsoft.com/azure/role-based-access-control/built-in-roles/ai-machine-learning#azure-ai-owner) |
...
When you need to intervene, choose the least disruptive action for your situation.

| Agent type | Supported actions in Microsoft 365 Admin Center | In Foundry portal | Via REST API |
| --- | --- | --- | --- |
| Agent applications | Stop, Start | Stop, Start, Delete | Stop, Start, Delete |
| Foundry agents | _None — use another interface_ | Delete _(Stop and Start aren't supported)_ | Delete _(Stop and Start aren't supported)_ |
...
<Tabs>
  <Tab title="Microsoft 365 Admin Center">
    ...
    **Note**: Foundry agent resources don't have a stop API in Microsoft 365 Admin Center. For agent resources that can't continue to run, consider deletion.

  </Tab>
  <Tab title="Foundry portal">

    For the full stop procedure in the portal, see [Start and stop agents](/manage/how-to-manage-agents#start-and-stop-agents).

    **Note**: Foundry agent resources don't have a stop API. For agent resources that can't continue to run, consider deletion.

  </Tab>
  <Tab title="REST API">

    The examples use `az rest` commands to simplify authentication. The easiest way to run these commands is through [Azure Cloud Shell](https://learn.microsoft.com/azure/cloud-shell/overview). Cloud Shell requires no installation and uses your current Azure sign-in session. If you prefer to work locally, [install the Azure CLI](https://learn.microsoft.com/cli/azure/install-azure-cli) and [sign in](https://learn.microsoft.com/cli/azure/authenticate-azure-cli) first.

    #### Agent application

    To fully stop an agent application, you need to stop each of its deployments. First, list the deployments for your agent application:
...
Always prefer **Stop** over **Delete**. Deletion permanently removes Azure resources. If an agent serves multiple tenants, deletion affects all of them.
...
<Tabs>
  ...
  <Tab title="REST API">
    ...
    **Delete a specific deployment** — first obtain the deployment name:
...
- [Manage agents in Foundry Control Plane](/manage/how-to-manage-agents)
