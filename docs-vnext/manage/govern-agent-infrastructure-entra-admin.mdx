...
The guidance here focuses on infrastructure-level governance of agents built with [Foundry Agent Service](/agent-service/overview) as a fallback for situations that require direct administrative action.
...
[Prompt agents](/agent-service/prompt-agent) can also use a common endpoint that serves all agents in the project. Agents that run this way share the project's infrastructure and identity. Don't use project endpoints for production. Foundry doesn't create more registrations in Agent 365 for project endpoints or project identities.
...
- [Agent identity concepts](/agent-service/agent-identity) for how agent identities work in Microsoft Entra ID.
...
|---|---|---|
| View | Foundry User (**Reader** isn't sufficient) | [Reader](https://learn.microsoft.com/azure/role-based-access-control/built-in-roles/general#reader) |
...
When you need to intervene, choose the least disruptive action for your situation. Your choice of action and agent type determines which interface you use to manage the agent.
...
**Microsoft 365 Admin Center (agent applications only)**
...
Deleting an agent or agent application permanently removes Azure resources. If the agent serves multiple tenants, this action affects all of them. Always prefer stopping a deployment over deleting it.

</Warning>

**Foundry portal — Foundry agents**
...
To delete the entire agent application:
...
To delete a specific deployment within an agent application, first list the deployments:
...
Then delete the target deployment:
