...
Replace the placeholders in the following command before running it:
...
Set these environment variables using your resource's key and endpoint:
...
On Windows, restart any programs that need to read the updated environment. On Linux, run `source ~/.bashrc` to apply changes.
