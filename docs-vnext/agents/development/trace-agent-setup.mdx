...
Foundry stores traces in [Azure Application Insights](https://learn.microsoft.com/azure/azure-monitor/app/app-insights-overview) by using [OpenTelemetry semantic conventions](https://opentelemetry.io/docs/specs/semconv/gen-ai/).
...
- For log-based queries, start by assigning the [Log Analytics Reader role](https://learn.microsoft.com/azure/azure-monitor/logs/manage-access).
...
- To manage access at scale, use [Microsoft Entra groups](/security/rbac-foundry#use-microsoft-entra-groups-with-foundry).
</Info>
...
- Foundry also allows for easy [integration](/agents/development/trace-agent-framework) with top agent frameworks.

### Client-side traces with the Microsoft Foundry SDK

Client-side tracing extends server-side visibility into your own application code. Install OpenTelemetry and the Azure SDK tracing plugin:
...
For complete setup instructions covering Python and C#, including console export, content recording, trace context propagation, and custom function tracing, see [Add client-side tracing to Foundry agents](/agents/development/trace-agent-client-side).
...
For more guidance, see [Security and privacy](/agents/development/trace-agent-concept#security-and-privacy).
...
| You see authorization errors when you query or view telemetry | Missing RBAC permissions on Application Insights or Log Analytics | Confirm access in **Access control (IAM)** for the connected resources. For log queries, assign the [Log Analytics Reader role](https://learn.microsoft.com/azure/azure-monitor/logs/manage-access). |
| Client-side traces don't appear | Instrumentation isn't installed or configured | Recheck your package installation and follow the SDK guidance in [Add client-side tracing to Foundry agents](/agents/development/trace-agent-client-side). |
...
- [Add client-side tracing to Foundry agents](/agents/development/trace-agent-client-side)
- [Tracing integrations](/agents/development/trace-agent-framework)
- [Monitor AI agents with the Agent Monitoring Dashboard](/agents/development/how-to-monitor-agents-dashboard)
