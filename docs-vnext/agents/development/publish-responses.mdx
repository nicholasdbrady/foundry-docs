...
- **Publish your agent as an Agent Application**: An Agent Application is a managed Azure resource that wraps your agent with a stable endpoint for external consumption. To publish your agent, see [Publish and share agents in Microsoft Foundry](/agents/development/publish-agent).
...
- Install the `openai` and `azure-identity` packages and authenticate as described in [Prepare your development environment](/developer-experience/install-cli-sdk).
