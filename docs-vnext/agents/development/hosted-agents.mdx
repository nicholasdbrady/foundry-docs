...
| Factor | Microsoft Agent Framework | LangGraph | Custom code |
|--------|--------------------------|-----------|-------------|
| **State management** | Built-in conversation state | Graph-based state with checkpointing | You manage state |
| **Multi-agent orchestration** | Built-in agent-to-agent protocol | Graph-based routing and branching | You implement routing |
| **Streaming** | Supported via hosting adapter | Native streaming support | You implement streaming |
| **Tool calling complexity** | Simple tool registration | Complex tool pipelines with conditional logic | Full flexibility |
| **C# support** | ✅ | ❌ | ✅ |
| **Community ecosystem** | Growing | Large (LangChain ecosystem) | N/A |
| **Learning curve** | Low | Medium | Varies |

<Tip>
Choose **Microsoft Agent Framework** when you need C# support, built-in agent-to-agent communication, or the simplest path to production on Foundry. Choose **LangGraph** when you need complex branching workflows, graph-based state management, or already use the LangChain ecosystem. Choose **custom code** when your agent has unique requirements that don't fit either framework's abstractions.
</Tip>
...
If you use the Foundry SDK, you need to perform these steps independently. For more information, see [Enable tracing in your project](https://learn.microsoft.com/en-us/azure/foundry-classic/how-to/develop/trace-application#enable-tracing-in-your-project).
...
For more information about evaluating agents, see [Evaluate your AI agents](https://learn.microsoft.com/en-us/azure/foundry-classic/how-to/develop/agent-evaluate-sdk) and [Agent evaluators](/observability/agent-evaluators).
