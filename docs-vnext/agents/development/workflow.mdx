...
sequence:
  ...
  - agent: writer
    prompt: "Write a summary based on: {{researcher.output}}"
"""
...
print(f"Workflow created: {agent.name} (version: {agent.version})")
```
...
const agent = await client.agents.createVersion("my-workflow", {
  kind: "workflow",
  workflow: workflowYaml,
});
console.log(`Workflow created: ${agent.name} (version: ${agent.version})`);
```
</CodeGroup>
