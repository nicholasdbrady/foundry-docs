...
If you're deploying for the first time or want the fastest path, use the [Quickstart: Create and deploy a Hosted agent](/agents/development/quickstart-hosted-agent) instead. The quickstart uses the **Azure Developer CLI (azd)** or **VS Code extension**, which handle building, pushing, versioning, and RBAC configuration automatically.
...
* Agent code using a [supported framework](/agents/development/hosted-agents#framework-and-language-support)
...
Before starting deployment, verify that your user account and the project managed identity have the required roles:

```bash
{/* Check your own role assignments on the Foundry resource */}
az role assignment list \
  --assignee $(az ad signed-in-user show --query id -o tsv) \
  --scope /subscriptions/YOUR_SUBSCRIPTION_ID/resourceGroups/YOUR_RG/providers/Microsoft.CognitiveServices/accounts/YOUR_ACCOUNT \
  --output table

{/* Check managed identity ACR pull access */}
PRINCIPAL_ID=$(az cognitiveservices account show \
  --name YOUR_ACCOUNT --resource-group YOUR_RG \
  --query identity.principalId -o tsv)

az role assignment list \
  --assignee $PRINCIPAL_ID \
  --scope /subscriptions/YOUR_SUBSCRIPTION_ID/resourceGroups/YOUR_RG/providers/Microsoft.ContainerRegistry/registries/YOUR_ACR \
  --output table
```

If the managed identity has no role assignment on the container registry, deployment fails with an opaque permission error. Assign the **Container Registry Repository Reader** role before proceeding.
...
1. Verify you have Azure Developer CLI version 1.23.0 or later:
    ...
    To upgrade, see [Install or update the Azure Developer CLI](https://learn.microsoft.com/azure/developer/azure-developer-cli/install-azd).
...
1. Build your Docker image:
    ...
    ```bash
    docker build --platform linux/amd64 -t myagent:20250315-1200 .
    ```

    <Warning>
    Always specify `--platform linux/amd64` when building locally. Without this flag, builds on ARM-based systems (Apple Silicon Macs) produce images that fail at startup with `exec format error`.
    </Warning>

    <Warning>
    Use unique, immutable image tags such as timestamps (`20250315-1200`) or git SHAs — never use `:latest`. Agent versions are immutable once created, and `:latest` tags prevent reliable rollbacks when you need to redeploy a known-good version.
    </Warning>
...
1. Push to Azure Container Registry:
    ...
    docker tag myagent:20250315-1200 myregistry.azurecr.io/myagent:20250315-1200
    docker push myregistry.azurecr.io/myagent:20250315-1200
    ```
...
- Image Generation
- Web Search
- MCP connections (see [Connect to Model Context Protocol servers](/agents/tools/model-context-protocol))
...
- [Agent identity concepts](/agents/development/agent-identity)
- [Publish and share agents](/agents/development/publish-agent)
