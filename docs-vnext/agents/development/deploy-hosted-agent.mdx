...
This article describes the initial public preview hosting backend, which was retired on May 22, 2026. Existing deployments on this backend were not migrated automatically and this workflow no longer works for new agents. Use [Deploy a hosted agent](/agent-service/deploy-hosted-agent) instead, and see [Migrate hosted agents to the refreshed public preview](/agent-service/migrate-hosted-agent-preview) if you have an agent deployed on the old backend.
