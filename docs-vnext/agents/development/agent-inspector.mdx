...
- An initialized hosted agent project. To create one, see [Initialize an agent project](/agent-service/init-agent-project).
- The azd Foundry extensions installed. For installation steps, see [Install the azd Foundry extensions](/developer-experience/install-cli-foundry-extensions).
- An authenticated Azure Developer CLI session. Run `azd auth login` if needed.
- A local agent to inspect. To start one, see [Run a hosted agent locally with the Azure Developer CLI](/agent-service/run-hosted-agent-locally).
...
The inspector only targets a local agent on `http://localhost:8088` by default. It doesn't authenticate to or proxy a deployed Foundry agent. For the deployed agent, use [`azd ai agent invoke`](/agent-service/invoke-hosted-agent) and [`azd ai agent monitor`](/agent-service/monitor-hosted-agent-logs).
...
- [Run a hosted agent locally with the Azure Developer CLI](/agent-service/run-hosted-agent-locally) to start the local agent.
- [Invoke a hosted agent with the Azure Developer CLI](/agent-service/invoke-hosted-agent) for terminal-based local and deployed invocation.
- [Isolate hosted agent sessions per user](/agent-service/isolate-sessions-per-user) to test isolation against a deployed agent.
