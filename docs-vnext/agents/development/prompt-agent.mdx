...
<Tabs>
  ...
  <Tab title="C#">
    ...
    1. Install packages by adding NuGet packages using the .NET CLI in the integrated terminal. These packages use the **Foundry projects (new) API**.
  ...
  <Tab title="REST API">
    ...
    1. Get a temporary access token. It expires in 60–90 minutes; refresh after that.

        ```azurecli
        az account get-access-token --scope https://ai.azure.com/.default
        ```

    1. Save the result as the environment variable `AZURE_AI_AUTH_TOKEN`.
...
In the [Azure portal](https://portal.azure.com), select the resource group, then select **Delete**. Confirm that you want to delete the resource group.
...
- [Use tools with agents](/agents/tools/model-context-protocol)
- [Quickstart: Deploy your first hosted agent](/agents/development/quickstart-hosted-agent)
