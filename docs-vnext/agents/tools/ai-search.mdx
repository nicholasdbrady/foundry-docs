...
conn = client.connections.get("YOUR_AI_SEARCH_CONNECTION_NAME")

agent = client.agents.create_version(
    agent_name="MyAgent",
    definition=PromptAgentDefinition(
        model="YOUR_MODEL_DEPLOYMENT",
        instructions="You are a helpful assistant. Provide citations for answers.",
        tools=[AzureAISearchTool(azure_ai_search=AzureAISearchToolResource(indexes=[
            AISearchIndexResource(project_connection_id=conn.id, index_name="YOUR_INDEX_NAME", query_type=AzureAISearchQueryType.SIMPLE),
        ]))],
    ),
)
```
...
const conn = await project.connections.get("YOUR_AI_SEARCH_CONNECTION_NAME");
...
const agent = await project.agents.createVersion("MyAgent", {
    ...
    tools: [{ type: "azure_ai_search", azure_ai_search: { indexes: [
        { project_connection_id: conn.id, index_name: "YOUR_INDEX_NAME", query_type: "simple" },
    ]}}],
});
```
</CodeGroup>
...
- The latest prerelease package. See the [quickstart](/get-started/get-started-code) for details.
