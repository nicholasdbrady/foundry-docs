...
sidebarTitle: "Azure Language in Foundry Tools"
---
...
This article introduces the Azure Language integrations available in Foundry Tools:
...
- **Language processing**: Access to Azure Language natural language processing (NLP) capabilities, including Named Entity Recognition, Text Analytics for health, Conversational Language Understanding, Custom Question Answering, Language Detection, Sentiment Analysis, Summarization, Key Phrase Extraction, and PII redaction.
...
For connection setup details, see [Add a connection to your project](/setup/connections-add).
...
Some Foundry configurations restrict which MCP servers you can use. For example, network-secured Foundry projects can require publicly accessible MCP servers. For details, see [Connect to Model Context Protocol servers (preview)](/agents/tools/model-context-protocol).
...
The agent, built on Azure Language natural language understanding capabilities, processes user input through layers. The system analyzes messages to understand intentions, then routes requests through appropriate channels based on confidence levels.
...
Before you set up the intent routing agent, make sure you have the following resources and configurations:
...
- **Foundry resource**: An active Foundry resource to host your agent.
- **Project resources**: Create your CLU and CQA projects using a Foundry resource, AI hub resource, or Azure Language resource.
- **Project deployments**: Deploy both a Custom Question Answering (CQA) project and a Conversational Language Understanding (CLU) project.
- **Custom connection setup**: Configure a custom connection between your agent project and the Azure Language resources. For steps, see [Set up a custom connection](#set-up-a-custom-connection).
...
- **Response delivery**: Custom Question Answering (CQA) provides responses drawn from curated knowledge sources, ensuring users receive consistent information that aligns with organizational standards.
- **Knowledge management**: Manage intent definitions in CLU projects and question-answer pairs in CQA projects for oversight of the agent's knowledge base.
- **Fallback processing**: Add retrieval-augmented generation (RAG) to handle edge cases and uncommon questions using approved knowledge sources.
...
The agent combines Foundry Agent Service capabilities with Custom Question Answering (CQA) technology. This integration creates a solution with minimal setup while delivering performance and oversight.
...
Before you set up the exact question answering agent, make sure you have:

- **Azure subscription**: [Create one for free](https://azure.microsoft.com/free/cognitive-services). You need Contributor or Owner role on the resource group.
- **Foundry resource**: An active Foundry resource to host your agent.
- **Project resources**: A CQA project using a Foundry resource, AI hub resource, or Azure Language resource.
- **Project deployment**: A deployed Custom Question Answering (CQA) project.
- **Custom connection setup**: A custom connection between your agent project and the Azure Language resources. For steps, see [Set up a custom connection](#set-up-a-custom-connection).
...
- **Agent Service integration**: Integrates Agent Service with Custom Question Answering capabilities within Azure Language services. This integration eliminates complex configuration requirements and provides access to enterprise security and monitoring features.
- **No-code deployment**: Deploy and configure the agent through the Foundry visual interface without writing custom code. This approach enables business stakeholders to participate in knowledge base creation and maintenance.
- **Knowledge management**: Manage question-answer pairs in CQA projects, ensuring response accuracy and organizational control.
- **Deterministic answering**: Returns exact verbatim responses as defined in the CQA project answers, ensuring consistent and controllable responses.
- **Fallback processing**: Add retrieval-augmented generation (RAG) to handle queries outside the predefined knowledge base using approved organizational content sources.

## Set up a custom connection

Both the intent routing agent and the exact question answering agent require a custom connection between your agent project and Azure Language resources. Follow these steps:
...
For detailed connection instructions, see [Add a connection to your project](/setup/connections-add).
...
|-------|---------------|------------|
| Authentication errors | Incorrect connection key | Confirm the connection uses the `Ocp-Apim-Subscription-Key` header name and that the value matches your Azure Language resource key. |
| Agent doesn't use CLU or CQA | Misconfigured connection | Confirm your CLU and CQA projects are deployed and that the agent is connected to the correct resources. |
| Low confidence or irrelevant responses | Insufficient training data | Review CLU intent training data and CQA question-answer pairs, then redeploy your projects. |
| MCP server returns 404 or connection error | Incorrect endpoint or API version | Verify the endpoint URL uses the correct Foundry resource name and API version. Confirm the Azure Language resource is in a supported region. |
| `azd ai agent init` command fails | Outdated Azure Developer CLI | Make sure you have the latest version of the [Azure Developer CLI](https://learn.microsoft.com/azure/developer/azure-developer-cli/install-azd) installed and that you're signed in with `azd auth login`. |
...
- [Tool catalog](/agents/tools/tool-catalog)
