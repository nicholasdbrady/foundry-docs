...
from azure.ai.projects.models import PromptAgentDefinition, MemorySearchPreviewTool

client = AIProjectClient(endpoint="YOUR_PROJECT_ENDPOINT", credential=DefaultAzureCredential())

tool = MemorySearchPreviewTool(
    memory_store_name="YOUR_MEMORY_STORE_NAME",
    scope="user_123",
)
...
const agent = await project.agents.createVersion("MyAgent", {
    ...
    model: "YOUR_MODEL_DEPLOYMENT",
    instructions: "You are a helpful assistant that remembers user preferences.",
    tools: [{ type: "memory_search_preview", memory_store_name: "YOUR_MEMORY_STORE_NAME", scope: "user_123" }],
});
```
</CodeGroup>

By creating and managing memory stores, you can build agents that retain user preferences, maintain conversation history, and deliver personalized experiences.
...
This article explains how to create, manage, and use memory stores. For conceptual information, see [Memory in Foundry Agent Service](/agents/tools/what-is-memory).
...
- A [Microsoft Foundry project](/setup/create-projects) with [authorization and permissions](#authorization-and-permissions) configured.
- [Chat model deployment](/models/capabilities/create-model-deployments) (for example, `gpt-5.2`) in your project.
- [Embedding model deployment](/models/capabilities/embeddings) (for example, `text-embedding-3-small`) in your project.
- For Python examples:
  - Python 3.8 or later with a [configured environment](/get-started/get-started-code)
...
For more information about user profile and chat summary memories, see [Memory types](/agents/tools/what-is-memory#memory-types).
...
- [Memory store REST API reference](/api-sdk/foundry-project-rest-preview)
- [Memory in Foundry Agent Service](/agents/tools/what-is-memory)
- [Foundry Agent Service quotas and limits](/agents/development/limits-quotas-regions)
