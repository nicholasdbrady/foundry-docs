...
<Note>
For information on optimizing tool usage, see [best practices](/agents/tools/tool-best-practice).
</Note>
...
- **Using a multi-agent workflow**: When Agent A calls Agent B through a workflow or other multi-agent orchestration, Agent B takes full responsibility for answering the user. Agent A is out of the loop. Agent B handles all subsequent user input. For more information, see [Build a workflow in Microsoft Foundry](/agents/development/workflow).
...
For details about supported authentication approaches, see [Agent2Agent (A2A) authentication](/agents/tools/agent-to-agent-authentication).
...
<Note>
The .NET and Java SDKs are currently in preview. See the [quickstart](/get-started/get-started-code) for details.
</Note>
...
- [Agent2Agent (A2A) authentication](/agents/tools/agent-to-agent-authentication)
- [Build a workflow in Microsoft Foundry](/agents/development/workflow)
- [Best practices for tools](/agents/tools/tool-best-practice)
