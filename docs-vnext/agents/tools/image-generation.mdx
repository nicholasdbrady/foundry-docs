...
<Info>
- The image generation tool requires the `gpt-image-1` model. See the [Azure OpenAI transparency note](/responsible-ai/transparency-note) for limitations and responsible AI considerations.
...
- A Foundry project.
- A basic or standard agent environment. See [agent environment setup](/setup/environment-setup).
...
1. Confirm your region and model support for image generation. See [Best practices for using tools in Microsoft Foundry Agent Service](/agents/tools/tool-best-practice).
1. Configure the environment variables listed in the prerequisites.

## Code examples

Before you start, install the `azure-ai-projects` package (version 2.0.0b4 or later). For package installation instructions, see the [quickstart](/get-started/get-started-code).
...
Content filtering can block image generation if the service detects unsafe content in your prompt. For more information, see [Content filter](https://learn.microsoft.com/en-us/azure/foundry-classic/foundry-models/concepts/content-filter).

<Tip>
For a thorough look at how you can tweak your text prompts to generate different kinds of images, see [Image prompt engineering techniques](/models/catalog/gpt-4-v-prompt-engineering).
</Tip>
...
| Prompt doesn't produce an image | Content filtering blocked the request | Check content filtering logs. See [Content filter](https://learn.microsoft.com/en-us/azure/foundry-classic/foundry-models/concepts/content-filter) for guidelines on acceptable prompts. |
| Tool not available | Regional or model limitation | Confirm the image generation tool is available in your region and with your orchestrator model. See [Best practices for using tools](/agents/tools/tool-best-practice). |
...
- [Image generation in Azure OpenAI](/models/catalog/dall-e)
- [Responses API in Azure OpenAI](/models/capabilities/responses)
