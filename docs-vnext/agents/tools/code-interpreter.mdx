...
agent = client.agents.create_version(
    ...
    definition=PromptAgentDefinition(
        ...
        tools=[CodeInterpreterTool(container=CodeInterpreterContainerAuto())],
    ),
)
```
...
- Latest prerelease SDK package installed (`azure-ai-projects>=2.0.0b4` for Python). See the [quickstart](/get-started/get-started-code) for installation steps.
...
For the current list of supported regions and models for Code Interpreter, see [Best practices for using tools in Microsoft Foundry Agent Service](/agents/tools/tool-best-practice).
...
For the latest SDK support status, see [Best practices for using tools in Microsoft Foundry Agent Service](/agents/tools/tool-best-practice).
...
| Python package not available. | Code Interpreter has a fixed set of packages. | Code Interpreter includes common data science packages. For custom packages, use [Custom code interpreter](/agents/tools/custom-code-interpreter). |
...
For examples of conversation and file cleanup patterns, see [Web search tool (preview)](/agents/tools/web-search) and [File search tool for agents](/agents/tools/file-search).

## Related content

- [Best practices for using tools in Microsoft Foundry Agent Service](/agents/tools/tool-best-practice)
- [Custom code interpreter tool for agents (preview)](/agents/tools/custom-code-interpreter)
