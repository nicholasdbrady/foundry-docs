...
<Warning>
**Use the 2.x preview SDK.** This quickstart requires `azure-ai-projects>=2.0.0b4`, which must be installed with the `--pre` flag. Running `pip install azure-ai-projects` without `--pre` installs the stable 1.x package, which targets the legacy Foundry classic experience. Code written for 2.x is **not compatible** with 1.x. See the install step below and [SDK overview](/api-sdk/sdk-overview) for details.
</Warning>
...
* The required language runtimes, global tools, and Visual Studio Code extensions as described in [Prepare your development environment](/developer-experience/install-cli-sdk).
...
<Tabs>
  ...
  <Tab title="REST API">
    ...
    Follow along below or get the code:
    <Card title="Get the code" icon="arrow-right" href="https://github.com/azure-ai-foundry/foundry-samples/tree/main/samples/REST/quickstart" />
...
<Tip>
**Starting a new project?** Install the 2.x preview package — it's the current active API for new Foundry projects. The 2.x preview package targets the new Foundry portal at [ai.azure.com](https://ai.azure.com). The 1.x GA package targets Foundry classic (legacy). Code using 2.x is not compatible with 1.x. See [SDK overview](/api-sdk/sdk-overview) for details.
</Tip>
...
<Tabs>
  ...
  <Tab title="Java">
    ...
    1. Add these Maven dependencies to your `pom.xml`. These packages use the **Foundry projects (new) API** (preview).
        ...
        ```groovy
        implementation 'com.azure:azure-ai-agents:1.0.0-beta.3'
...
<Tip>
Code uses **Azure AI Projects 2.x** and is incompatible with Azure AI Projects 1.x. [See the Foundry (classic) documentation](https://learn.microsoft.com/en-us/azure/foundry-classic/index.yml)  for the Azure AI Projects 1.x version.
</Tip>
