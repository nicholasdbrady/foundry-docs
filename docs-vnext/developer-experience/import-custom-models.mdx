...
This custom model import guide uses the Fireworks on Foundry integration. For an overview of available catalog models, supported architectures, data privacy, and limitations, see [Use Fireworks models on Foundry](/developer-experience/enable-fireworks-models).
...
* The **Fireworks on Foundry** preview feature enabled in your subscription. For setup steps, see [Use Fireworks models on Foundry](/developer-experience/enable-fireworks-models#enable-fireworks-on-foundry).
* The **Cognitive Services Contributor** role or equivalent permissions on the Foundry resource to create and manage deployments. For more information, see [Azure role based access control](https://learn.microsoft.com/azure/foundry/concepts/rbac-foundry#permissions-for-each-built-in-role).
* [Azure Developer CLI](https://learn.microsoft.com/azure/developer/azure-developer-cli/install-azd) (`azd`) installed locally. The import workflow uses `azd` to upload model weights.

## Region availability

Support for deploying custom models is available in all global [Azure regions](https://learn.microsoft.com/azure/reliability/regions-list) except for Azure Government cloud environments.
...
1. Configure the deployment:
   ...
   * **Provisioned throughput units**: allocate the number of provisioned throughput units (PTUs) for the deployment. For more information, see [Provisioned throughput concepts](/foundry-models/provisioned-throughput).
...
Use the following examples to automate parts of the deployment workflow after the custom model is registered. Each example deploys the custom model with `80` units of Global Provisioned throughput. Replace placeholders with your details.
...
az cognitiveservices account deployment create \
  --name "YOUR_FOUNDRY_RESOURCE_NAME" \
  --resource-group "YOUR_RESOURCE_GROUP" \
  --deployment-name "YOUR_DEPLOYMENT_NAME" \
  --model-name "YOUR_REGISTERED_MODEL_NAME" \
  --model-version "1" \
...
1. Open the [Foundry Playground](/foundry-models/concept-playgrounds).
...
| Deployment fails | Confirm you have sufficient quota and that the [Fireworks preview feature](/developer-experience/enable-fireworks-models#enable-fireworks-on-foundry) is enabled and registered in your subscription. |
...
For more troubleshooting guidance, see [Troubleshoot Fireworks on Foundry](/developer-experience/enable-fireworks-models#troubleshoot-fireworks-on-foundry).

## Related content

* [Fireworks models on Foundry](/developer-experience/enable-fireworks-models)
...
* [Configure keyless authentication with Microsoft Entra ID](/security/configure-entra-id)
