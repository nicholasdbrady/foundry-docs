...
- The [azd Foundry extensions installed](/developer-experience/install-cli-foundry-extensions).
- An authenticated `azd` session.
- A Microsoft Foundry project endpoint for the commands you want to run. For more information, see [Set the azd project context](/developer-experience/cli-project-context).
- Optional: a deployed hosted agent when you need to invoke an agent endpoint. For setup, see [Deploy a hosted agent](/agents/development/deploy-hosted-agent).

## Start with the Microsoft Foundry Skill

Coding agents work best when they already know the `azd ai` conventions. The [Microsoft Foundry Skill](/developer-experience/use-microsoft-foundry-skill) gives a coding agent that knowledge: it generates correct `azd ai` commands and Foundry wiring, and it applies the practices in this article -- setting the project context, passing `--no-prompt`, and requesting `--output json` for structured results. Point your coding agent at the skill first, then use the patterns in the rest of this article to review and harden what it produces.
...
For the full explanation of how the CLI resolves the endpoint, including the precedence order, see [Set the azd project context](/developer-experience/cli-project-context).
...
Use this shape when one repository's CI needs to call an agent owned by a different repository, or when an MCP server fronts several agents and only knows their endpoint URLs.
...
For secrets that shouldn't live in a local `.env` file, store them in a Foundry project connection and reference them with a `${{connections.<name>.credentials.<field>}}` placeholder.
...
For the full explanation, including how the standalone context interacts with in-project work, see [Set the azd project context](/developer-experience/cli-project-context).
...
- On failure, prefer parsing the structured suggestion in the error output to decide next steps. For example, a "No Foundry project endpoint resolved" error implies you should run `azd ai project set`, or set `FOUNDRY_PROJECT_ENDPOINT`, before retrying.
...
- [Set the azd project context](/developer-experience/cli-project-context) to understand how the CLI resolves the Foundry project endpoint.
- [Deploy a hosted agent with the Azure Developer CLI](/agents/development/deploy-hosted-agent) for patterns that deploy agents from CI.
- [Invoke a hosted agent with the Azure Developer CLI](/agent-service/invoke-hosted-agent) for full `azd ai agent invoke` options, including `--agent-endpoint`.
