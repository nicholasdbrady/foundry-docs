...
- Optional: a deployed hosted agent when you need to invoke an agent endpoint. For setup, see [Deploy a hosted agent](/agents/deploy-hosted-agent).

## Start with the Microsoft Foundry Skill

Coding agents work best when they already know the `azd ai` conventions. The [Microsoft Foundry Skill](/get-started/use-microsoft-foundry-skill) gives a coding agent that knowledge: it generates correct `azd ai` commands and Foundry wiring, and it applies the practices in this article -- setting the project context, passing `--no-prompt`, and requesting `--output json` for structured results. Point your coding agent at the skill first, then use the patterns in the rest of this article to review and harden what it produces.
...
Use this shape when one repository's CI needs to call an agent owned by a different repository, or when an MCP server fronts several agents and only knows their endpoint URLs. For the full set of `invoke` options, see [Invoke a hosted agent](/agents/invoke-hosted-agent).
...
For secrets that shouldn't live in a local `.env` file, store them in a Foundry project connection and reference them with a `${{connections.<name>.credentials.<field>}}` placeholder. See [Run a hosted agent locally](/agents/run-hosted-agent-locally) for the full local-run surface.
...
- [Set up CI/CD for hosted agents with the Azure Developer CLI](/agents/set-up-ci-cd-cli) for patterns that run `azd ai` in pipelines.
- [Invoke a hosted agent](/agents/invoke-hosted-agent) for full `azd ai agent invoke` options, including `--agent-endpoint`.
