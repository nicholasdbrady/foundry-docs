...
| Chat history stores | `langchain_azure_ai.chat_message_histories` | Persist and replay chat history across sessions. Use memory-powered histories to retrieve consolidated past chat history. [See detailed examples](/developer-experience/langchain-memory). |
...
```bash
export OPENAI_BASE_URL="https://YOUR_RESOURCE.services.ai.azure.com/openai/v1"
...
You can also configure clients specifically. As an example, `AzureAIOpenAIApiChatModel` as a representative pattern:
...
# Option C: Use an API key.
model_with_key = AzureAIOpenAIApiChatModel(
  endpoint=os.environ["OPENAI_BASE_URL"],
...
<Card title="Use Foundry Agent Service with LangGraph" icon="arrow-right" href="/developer-experience/langchain-agents" />
