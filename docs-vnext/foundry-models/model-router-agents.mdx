...
For general model router concepts, see the [model router overview](/foundry-models/model-router). For deployment steps, see [Use model router](/foundry-models/model-router).

## Prerequisites

- A Microsoft Foundry project with a model router deployment. See [Deploy a model router model](/foundry-models/model-router#deploy-a-model-router-model).
...
- **Switch routing mode** — Use Quality mode for critical agents (legal, medical) or Cost mode for high-volume agents (classification, triage). See [Change the routing mode](/foundry-models/model-router#optional-change-the-routing-mode).
- **Constrain the model pool** — Use model subset to limit which models the router can select. See [Route to a model subset](/foundry-models/model-router#optional-route-to-a-model-subset).
...
- [How model router works](/foundry-models/model-router-how-it-works)
