...
| [GPT-6 series](/foundry-models/models-sold-directly-by-azure#gpt-6) | **NEW** `gpt-6-astra` |
...
For regional and deployment-category availability, see [Region availability for Foundry Models sold by Azure](/foundry-models/models-sold-directly-by-azure-region-availability).

### Capabilities

| Model ID | APIs and features | Context window | Maximum output | Training data |
| --- | --- | ---: | ---: | --- |
| `gpt-6-astra` (2026-09-03) | Chat Completions and [Responses API](/foundry-models/responses); streaming; structured outputs; text and image input; functions, tools, and parallel tool calling; [reasoning effort and verbosity](/foundry-models/reasoning); computer use | 1,050,000 total<br /><br />922,000 input<br />128,000 output | 128,000 | April 2026 |

<Note>
Some [quota tiers](/foundry-models/quotas-limits) require a quota request before you can deploy `gpt-6-astra`. Tier 5 and Tier 6 subscriptions have quota by default. Standard pay-as-you-go deployments use separate pricing categories for short- and long-context requests based on the input token count.
</Note>
...
| `DeepSeek-V3.2-Speciale` | chat-completion <br /> [(with reasoning content)](/foundry-models/use-chat-reasoning) | - **Input:** text (128,000 tokens) <br /> - **Output:** text (128,000 tokens) <br /> - **Languages:** `en` and `zh` <br />  - **Tool calling:** No <br /> - **Response formats:** Text, JSON | - Global standard (all regions) |
| `DeepSeek-V3.2` | chat-completion <br /> [(with reasoning content)](/foundry-models/use-chat-reasoning) | - **Input:** text (128,000 tokens) <br /> - **Output:** text (128,000 tokens) <br /> - **Languages:** `en` and `zh` <br />  - **Tool calling:** No <br /> - **Response formats:** Text, JSON | - Global standard (all regions) |
| `DeepSeek-V3.1` | chat-completion <br /> [(with reasoning content)](/foundry-models/use-chat-reasoning) | - **Input:** text (131,072 tokens) <br /> - **Output:** text (131,072 tokens) <br /> - **Languages:** `en` and `zh` <br />  - **Tool calling:** Yes <br /> - **Response formats:** Text, JSON | - Global standard (all regions) |
| `DeepSeek-R1-0528` | chat-completion <br /> [(with reasoning content)](/foundry-models/use-chat-reasoning) | - **Input:** text (163,840 tokens) <br /> - **Output:** text (163,840 tokens) <br /> - **Languages:** `en` and `zh` <br />  - **Tool calling:** No <br /> - **Response formats:** Text | - Global standard (all regions) <br /> - Global provisioned (all regions)|
