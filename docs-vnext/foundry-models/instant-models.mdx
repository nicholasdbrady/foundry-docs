...
- A Foundry project in **West US 3** (the only supported region for instant models during preview). If you need to create a project, see [Create a project](/setup/create-projects).
...
<Tabs>
  ...
  <Tab title="Foundry portal">
    ...
    1. On the Home page of your project, select **Test in playground**. (This might instead be labeled **Explore playgrounds**).
        ...
        {/* 
        <img src="/images/playground.png" alt="Screenshot of Test in playground card with description to test models and get API key and endpoint." />
        */}
...
| Need reserved capacity or [predictable throughput](/foundry-models/deployment-types) | Deployment |
| Require [provisioned throughput (PTU)](/foundry-models/provisioned-throughput) | Deployment |
| Need [data residency](/foundry-models/deployment-types) in a specific region | Deployment |
...
| Fine-grained [quota](/foundry-models/quota) partitioning across teams | Deployment |
| [Fine-tuned models](/foundry-models/fine-tune-cli) | Deployment |
...
For more details on how global and regional quotas interact, see [Manage and increase quotas](/foundry-models/quota).
...
All instant models use default [guardrails](/guardrails/guardrails-overview) and content filters. However, you can't configure custom guardrails or Responsible AI (RAI) policies on a per-model basis for instant models. You can set a default RAI policy at the account level through the API, but that policy applies uniformly to all instant models. If you need different content filtering policies for individual models, use a deployment.
...
- [Guardrails](/guardrails/guardrails-overview), custom RAI policies, and content filters aren't configurable for instant models.
...
- [Manage quotas for Foundry resources](/foundry-models/quota)
