...
1. From the [model catalog](https://ai.azure.com/catalog/) select a [reasoning model](/foundry-models/reasoning) such as `gpt-6-astra`, `gpt-6-sol`, `gpt-5.3-codex`, `gpt-5.2-codex`, `gpt-5.1-codex-max`, `gpt-5.1-codex`, `gpt-5.1-codex-mini`, [`gpt-5-codex`](https://ai.azure.com/catalog/models/gpt-5-codex), [`gpt-5`](https://ai.azure.com/catalog/models/gpt-5), [`gpt-5-mini`](https://ai.azure.com/catalog/models/gpt-5-mini), or [`gpt-5-nano`](https://ai.azure.com/catalog/models/gpt-5-nano).
...
Codex CLI versions `0.152.1` and `0.153.0` are validated with `gpt-6-astra` on Azure OpenAI, including [prompt caching](/foundry-models/prompt-caching). Codex CLI version `0.155.1` is validated with `gpt-6-sol` and `gpt-6-luna` on Azure OpenAI. These are validated versions, not minimum supported versions.
...
1. Copy the following text to use the [v1 Responses API](/foundry-models/responses). With the [v1 API](/api-sdk/api-version-lifecycle) you no longer need to pass `api-version`, but you must include `/v1` in the `base_url` path. You can't pass your API key as a string directly to `env_key`. `env_key` must point to an environment variable. Update your `base_url` with your resource name:
