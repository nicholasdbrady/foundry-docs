...
- [Model routing with the Responses API](/foundry-models/responses-model-routing) — Use a single API to route to any model automatically or by name.
- [A2A agent endpoints (preview)](/agent-service/enable-agent-to-agent-endpoint) — Expose agents as Agent2Agent endpoints so other agents can discover and call them.
- [Instant models (preview)](/foundry-models/instant-models) — Call any supported model by name without creating a deployment first.
- [Voice agents (preview)](/agent-service/build-voice-agent) — Build voice-enabled agents with hosted agents.
- [Routines (preview)](/agent-service/routines) — Automate multi-step agent workflows with reusable routines.
- [Fabric IQ (preview)](/agent-service/fabric-iq) — Connect agents to Microsoft Fabric data.
- [Work IQ (preview)](/agent-service/work-iq) — Connect agents to Microsoft 365 content.
...
{/* Disposable issue #629 relevant skip-ci proof. */}
