...
For general model router concepts, see the [model router overview](/models/catalog/model-router). For deployment steps, see [Use model router](/models/catalog/model-router).

## Prerequisites

- A Microsoft Foundry project with a model router deployment. See [Deploy a model router model](/models/catalog/model-router#deploy-a-model-router-model).
...
- **Switch routing mode** — Use Quality mode for critical agents (legal, medical) or Cost mode for high-volume agents (classification, triage). See [Change the routing mode](/models/catalog/model-router#optional-change-the-routing-mode).
- **Constrain the model pool** — Use model subset to limit which models the router can select. See [Route to a model subset](/models/catalog/model-router#optional-route-to-a-model-subset).
...
- [How model router works](/models/catalog/model-router-how-it-works)
