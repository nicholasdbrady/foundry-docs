---
title: "GPT Realtime 2.x overview"
description: "Learn about the generally available GPT Realtime 2.1 models for low-latency, interactive voice experiences."
---
...
- `gpt-realtime-2.1` (version `2026-07-07`) improves silence and noise handling over `gpt-realtime-2`.
- `gpt-realtime-2.1-mini` (version `2026-07-07`) is a smaller variant in the 2.x series.

Both models support Global Standard and Data Zone Standard deployments. For region availability, see [Region availability for Foundry Models sold by Azure](/foundry-models/models-sold-directly-by-azure-region-availability).
...
```text
wss://YOUR_RESOURCE_NAME.openai.azure.com/openai/v1/realtime?model=YOUR_DEPLOYMENT_NAME
```

The GA endpoint doesn't use a date-based API version or an `api-version` query parameter. For complete examples, see [Use the GPT Realtime API via WebSockets](/models/catalog/realtime-audio-websockets) and [Use the GPT Realtime API via WebRTC](/models/catalog/realtime-audio-webrtc).
...
Instruction following is stricter than in earlier realtime models. If your system prompt contains narrow wording, such as distinguishing "order ID" from "confirmation code," broaden or rephrase instructions to match real user phrasing.

## Known limitations

Currently, GPT Realtime 2.x models don't support the `truncation` property in the `session.update` payload. To manage token usage and costs in an ongoing conversation, use `conversation.item.truncate` or `conversation.item.delete` events.

Realtime models have separate audio-token and concurrent-session quotas. Review [Azure OpenAI quotas and limits](/foundry-models/quotas-limits) before production deployment.
...
- [Use the GPT Realtime API via WebSockets](/models/catalog/realtime-audio-websockets)
- [Use the GPT Realtime API via WebRTC](/models/catalog/realtime-audio-webrtc)
- [Use the GPT Realtime API via SIP](/models/catalog/realtime-audio-sip)
