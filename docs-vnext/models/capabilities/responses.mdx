...
---
Use the Azure OpenAI Responses API to generate stateful, multi-turn responses. It brings together capabilities from chat completions and the Assistants API in one unified experience. The Responses API also supports the `computer-use-preview` model that powers [Computer use](https://learn.microsoft.com/en-us/azure/foundry-classic/openai/how-to/computer-use).
...
<Tabs>
  ...
  <Tab title="REST API">
    ...
    The REST examples use `AZURE_OPENAI_AUTH_TOKEN` for Microsoft Entra ID authentication. For details on required scopes and token acquisition options, see [Azure OpenAI REST API reference](/api-sdk/latest).
...
To retrieve a stored response by ID.
...
The output confirms that even though the first input question was not passed directly to the `second_response` call, the `previous_response_id` gives the model full context of the prior exchange.
...
The Code Interpreter tool enables models to write and execute Python code in a secure, sandboxed environment. It supports data processing, file generation (including charts), iterative debugging, and visual reasoning (for example, image cropping and zooming in models like o3 and o4-mini). It is especially useful for data analysis, mathematical computation, and code generation.
...
The MCP tool works only in the Responses API, and is available across all newer models (gpt-4o, gpt-4.1, and reasoning models). You only pay for tokens used when importing tool definitions or making tool calls.
...
Review all data being shared with remote MCP servers and optionally log it for auditing.
...
- [Azure service tags](https://learn.microsoft.com/azure/virtual-network/service-tags-overview) are currently not supported for MCP client traffic.
</Info>
...
For examples of how to use reasoning models with the responses API see the [reasoning models guide](/models/capabilities/reasoning#reasoning-summary).

## Computer use

Computer use with Playwright has moved to the [dedicated computer use model guide](https://learn.microsoft.com/en-us/azure/foundry-classic/openai/how-to/computer-use#playwright-integration)

## Responses API

### API support

- [v1 API is required for access to the latest features](/api-sdk/api-version-lifecycle#api-evolution)
...
Not every model is available in the regions supported by the responses API. Check the [models page](/models/catalog/models-sold-directly-by-azure) for model region availability.
...
- [Responses API reference documentation](https://learn.microsoft.com/azure/ai-foundry/openai/reference-preview-latest)
...
- [API version lifecycle](/api-sdk/api-version-lifecycle)
- [Azure OpenAI REST API reference](/api-sdk/latest)
