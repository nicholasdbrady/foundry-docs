# Retrieve a response

Retrieve a previously created response by ID.

## Request

`GET /responses/{responseId}`

### Path parameters

| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `responseId` | string | Required | Identifier of the response to retrieve. |

### Query parameters

| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `api-version` | string | Required | API version. Use `2025-05-01`. |

### Headers

| Header | Type | Required | Description |
| --- | --- | --- | --- |
| `Authorization` | string | Conditional | Bearer access token. Use this header or `api-key`. |
| `api-key` | string | Conditional | Project API key. Use this header or `Authorization`. |
| `Accept` | string | Required | Use `application/json`. |

### Request body

This operation does not accept a request body.

### Example request

```bash
curl "https://YOUR_ENDPOINT/api/projects/YOUR_PROJECT_NAME/responses/RESP_123?api-version=2025-05-01" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Accept: application/json"
```

## Response

### Response body

| Field | Type | Description |
| --- | --- | --- |
| `id` | string | Unique identifier for the response. |
| `object` | string | Object type. This value is `response`. |
| `status` | string | Current lifecycle state of the response. |
| `created_at` | string | Timestamp when the response was created. |
| `output` | array | Generated output items, if available. |
| `usage` | object | Usage totals when generation has started. |

### Example response

```json
{
  "id": "resp_01JX8V6C4FQ2R6P4Z5W8N9K7M1",
  "object": "response",
  "created_at": "2025-05-21T14:22:31Z",
  "status": "completed",
  "output": [
    {
      "id": "msg_01JX8V6D0S4H2T6H6EW4P2Q4ME",
      "type": "message",
      "role": "assistant",
      "content": [
        {
          "type": "output_text",
          "text": "The invoice is for $4,250.00, due on 2025-05-31, and is still pending payment."
        }
      ]
    }
  ],
  "usage": {
    "input_tokens": 87,
    "output_tokens": 24,
    "total_tokens": 111
  }
}
```

## Errors

| Status | Code | Description |
| --- | --- | --- |
| `401` | `unauthorized` | The bearer token or API key is missing, expired, or invalid. |
| `404` | `not_found` | No response resource exists with the specified `responseId`. |
| `429` | `rate_limit_exceeded` | The request exceeded a project or deployment rate limit. |
