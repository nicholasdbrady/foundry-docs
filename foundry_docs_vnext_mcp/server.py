...
Serves the docs-vnext/ content set for A/B/C/D comparison testing against
the current foundry-docs server, Microsoft Docs remote MCP, and Mintlify MCP.

Reuses core modules (indexer, chunker, retry, telemetry) from foundry_docs_mcp.
"""
...
from foundry_docs_mcp.telemetry import Telemetry, emit_feedback, instrument_search, setup_telemetry
...
def _resolve_docs_dir() -> Path:
    """Find the docs-vnext directory, checking env var and common locations."""
    if env := os.environ.get("FOUNDRY_VNEXT_DOCS_DIR"):
        return Path(env)
    candidate = PROJECT_ROOT / "docs-vnext"
    if candidate.is_dir():
        return candidate
    if (fallback := Path("/app/docs-vnext")).is_dir():
        return fallback
    return candidate


def _resolve_docs_json() -> Path:
    if env := os.environ.get("FOUNDRY_VNEXT_DOCS_JSON"):
        return Path(env)
    candidate = PROJECT_ROOT / "docs-vnext" / "docs.json"
    if candidate.exists():
        return candidate
    if (fallback := Path("/app/docs-vnext/docs.json")).exists():
        return fallback
    return candidate
...
async def load_docs(server):
    ...
    if os.environ.get("AZURE_SEARCH_ENDPOINT"):
        try:
            ...
            logger.info("Azure AI Search hybrid mode enabled for docs-vnext")
        except Exception as exc:
            logger.warning("Failed to initialize Azure hybrid mode; using local search fallback: %s", exc)

    telemetry = setup_telemetry("foundry-docs-vnext")
...
mcp = FastMCP(
    "foundry-docs-vnext",
    instructions=(
        "Microsoft Foundry documentation server (vnext) with ~250+ docs covering agents, "
        ...
        "Resource templates:\n"
        "  docs://vnext/page/{path}  — read a doc directly by URI\n"
        "  docs://vnext/navigation   — full navigation structure"
    ),
    version="0.1.0",
    lifespan=load_docs,
)
...
@mcp.tool(
    ...
    annotations={
        "title": "Search Foundry Documentation (vnext)",
        "readOnlyHint": True,
        "openWorldHint": False,
    },
)
async def search_docs(
    ...
    *,
    ...
    """
    await _log(ctx, "info", f"Searching docs-vnext for: {query}")
...
@mcp.tool(
    ...
    annotations={
        "title": "Submit Search Feedback (vnext)",
        "readOnlyHint": False,
        "openWorldHint": False,
    },
)
async def submit_search_feedback(
    ...
    *,
    ...
    await _log(ctx, "info", "Recorded search feedback event (vnext)")
    return json.dumps(
        {
...
@mcp.tool(
    tags={"read", "content"},
    annotations={
        "title": "Get Documentation Page (vnext)",
        "readOnlyHint": True,
        "openWorldHint": False,
    },
)
...
@mcp.tool(
    ...
    annotations={
        "title": "List Documentation Sections (vnext)",
        "readOnlyHint": True,
        "openWorldHint": False,
    },
)
...
@mcp.resource(
    "docs://vnext/navigation",
    name="Navigation Structure (vnext)",
    description="Full Mintlify navigation with all sections, groups, and page paths (docs-vnext)",
...
@mcp.resource(
    "docs://vnext/page/{section}/{page}",
    name="Documentation Page (vnext)",
    description=(
        "Read a documentation page by section and page name (docs-vnext). "
        "Example: docs://vnext/page/get-started/quickstart-create-foundry-resources"
    ),
...
@mcp.resource(
    "docs://vnext/page/{section}/{subsection}/{page}",
    name="Documentation Page (vnext, nested)",
    description=(
        "Read a documentation page with a nested path (docs-vnext). "
        "Example: docs://vnext/page/agents/development/overview"
    ),
    mime_type="text/markdown",
    annotations={"readOnlyHint": True, "idempotentHint": True},
)
...
def explain_concept(concept: str) -> list[Message]:
    return [
        Message(
            ...
            "Use the foundry-docs-vnext MCP tools to ground every answer in official docs.",
            role="assistant",
        ),
        Message(
...
def build_agent(agent_type: str, tools: str = "") -> list[Message]:
    ...
    return [
        Message(
            ...
            "Use the foundry-docs-vnext MCP tools to provide accurate, up-to-date guidance.",
            role="assistant",
        ),
        Message(
...
def compare_options(option_a: str, option_b: str, context: str = "") -> list[Message]:
    ...
    return [
        Message(
            ...
            "Use the foundry-docs-vnext MCP tools to ground comparisons in official docs.",
            role="assistant",
        ),
        Message(
